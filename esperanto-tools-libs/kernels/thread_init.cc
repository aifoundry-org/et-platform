...
extern "C" void thread_init()
{
    ...
    if (get_lane_id() == 0 && is_t0)
    {
        ...
        for (uint64_t barrier_addr = ATOMIC_REGION; barrier_addr < ATOMIC_REGION + 0x100; barrier_addr += 8)
        {
        ...
        uint64_t shire_smem_addr = BLOCK_SHARED_REGION + BLOCK_SHARED_REGION_SIZE_PER_SHIRE * shire_id;
    ...
    if ( is_t0 )
    {
        evict_range_wait( (void*)LAUNCH_PARAMS_AREA_BASE, CACHE_LINE_SIZE, CACHEOP_DEST_LVL_L2 );
    }
    ...
    if (intra_shire_barrier(FLB_BOOT, MINIONS_PER_SHIRE, MINIONS_PER_SHIRE))
    {
        ...
        evict_range_wait( (void*)LAUNCH_PARAMS_AREA_BASE, CACHE_LINE_SIZE, CACHEOP_DEST_LVL_MEM );

        // Inform maxion about initialization completion. Only one thread is doing this
...
extern "C" void thread_launch(LaunchParams_t *launch_params_p)
{
    ...
    if ( is_raw_kernel )
    {
        ...
        RawKernelFunction_t raw_kernel_fun = (RawKernelFunction_t)launch_params_p->kernel_pc;
    ...
    } else
    {
        ...
        if ( is_t0 )
        {
            KernelWorkItemFunction_t kernel_wi_fun = (KernelWorkItemFunction_t)launch_params_p->kernel_pc;

            KernelDispatchFunction(launch_params_p->args, &launch_params_p->state, kernel_wi_fun);
        }
    }

    if ( is_t0 )
    {
        evict_range_wait( launch_params_p, sizeof(LaunchParams_t) + launch_params_p->args_size, CACHEOP_DEST_LVL_L2 );
    }
    ...
    if (intra_shire_barrier(FLB_FINAL, MINIONS_PER_SHIRE, MINIONS_PER_SHIRE))
    {
        ...
        evict_range_wait( launch_params_p, sizeof(LaunchParams_t) + launch_params_p->args_size, CACHEOP_DEST_LVL_MEM );
