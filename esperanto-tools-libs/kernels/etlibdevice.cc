...
extern "C" void KernelDispatchFunction(void *args,
   ...
   *
   * Each CUDA Thread Block will take whole number of shires. So if block size
   * is 1 then only one hw thread in a shire will be active, i.e. execute some
   * work item. All work items in a block should run simultaneously in order to
   * be possible to implement intra-block synchronization, i.e. CUDA
   * __syncthreads(). So in each point of time some last shires may by
   * completely inactive and some last hw threads in each-block-last-shire may
   * also be inactive.
   */
  unsigned grid_size =
      state_p->grid_size_x * state_p->grid_size_y * state_p->grid_size_z;
  unsigned block_size =
      state_p->block_size_x * state_p->block_size_y * state_p->block_size_z;
  unsigned shires_per_block =
      align_up(block_size, (unsigned)MINIONS_PER_SHIRE) / MINIONS_PER_SHIRE;
  unsigned blocks_per_device =
      SHIRES_COUNT / shires_per_block; // or simul_blocks
  unsigned active_shires = shires_per_block * blocks_per_device;
  unsigned non_active_minions_per_block =
      (shires_per_block * MINIONS_PER_SHIRE) - block_size;
  ...
  if (thisThreadShireId < active_shires) {
    ...
    unsigned thisThreadLocalId =
        thread_id % (shires_per_block * MINIONS_PER_SHIRE);
    unsigned thisThreadLocalShireId = thisThreadLocalId / MINIONS_PER_SHIRE;
    unsigned thisShireActiveMinions =
        (thisThreadLocalShireId == shires_per_block - 1) // last shire in block?
            ? MINIONS_PER_SHIRE - non_active_minions_per_block
            : MINIONS_PER_SHIRE;
    if (thisThreadLocalId < block_size) {
      for (unsigned block_id = thisThreadSimulBlockId; block_id < grid_size;
           block_id += blocks_per_device) {
        ...
        wi_state.local_id_y =
            (local_id / state_p->block_size_x) % state_p->block_size_y;
        wi_state.local_id_z =
            (local_id / state_p->block_size_x) / state_p->block_size_y;

        wi_state.block_id_x = (block_id % state_p->grid_size_x);
        wi_state.block_id_y =
            (block_id / state_p->grid_size_x) % state_p->grid_size_y;
        wi_state.block_id_z =
            (block_id / state_p->grid_size_x) / state_p->grid_size_y;
   ...
   * Each hw thread will execute wiPerThread work items (CUDA threads), except
   * maybe the last one or several.
   */
  unsigned grid_size =
      state_p->grid_size_x * state_p->grid_size_y * state_p->grid_size_z;
  unsigned block_size =
  ...
  // assert(MINIONS_COUNT * (wiPerThread-1) < global_size <= MINIONS_COUNT *
  // wiPerThread)

  unsigned thisThreadGlobalIdFirst = thread_id * wiPerThread;
  unsigned thisThreadGlobalIdEnd = thisThreadGlobalIdFirst + wiPerThread;
  thisThreadGlobalIdEnd = (global_size < thisThreadGlobalIdEnd)
                              ? global_size
                              : thisThreadGlobalIdEnd;

  KernelWorkItemState_t wi_state;

  for (unsigned global_id = thisThreadGlobalIdFirst;
       global_id < thisThreadGlobalIdEnd; global_id++) {
...
extern "C" char *__et_get_smem_vars_storage(KernelRuntimeState_t *rt_p,
  ...
  unsigned leading_shire_id =
      align_down(get_shire_id(), wi_p->shires_per_block);
  uint64_t block_smem_addr =
      BLOCK_SHARED_REGION +
      BLOCK_SHARED_REGION_SIZE_PER_SHIRE * leading_shire_id;

  // if kernel uses block shared mem (and therefore unconditionally called this
  // function at the beginnig) then we must assure that threads from consecutive
  // blocks (binned to this leading shire) will not execute simultaneously
...
extern "C" void llvm_nvvm_barrier0(KernelRuntimeState_t *rt_p,
  ...
  if (intra_shire_barrier(FLB_SWI, wi_p->this_shire_active_minions, 0)) {
    ...
    // synchronize them on block shared memory (if there are more than one shire
    // in block)
    if (wi_p->shires_per_block > 1) {
       ...
       * some threads will (indefinitelly) wait zero in counter while some
       * another threads may get ahead and manage to +1 in next logical barrier.
       */
      asm volatile("fence");
...
extern "C" unsigned llvm_nvvm_read_ptx_sreg_tid_x(KernelRuntimeState_t *rt_p,
                                                  KernelWorkItemState_t *wi_p) {
  return wi_p->local_id_x;
}
extern "C" unsigned llvm_nvvm_read_ptx_sreg_tid_y(KernelRuntimeState_t *rt_p,
                                                  KernelWorkItemState_t *wi_p) {
  return wi_p->local_id_y;
}
extern "C" unsigned llvm_nvvm_read_ptx_sreg_tid_z(KernelRuntimeState_t *rt_p,
                                                  KernelWorkItemState_t *wi_p) {
  return wi_p->local_id_z;
}

extern "C" unsigned
llvm_nvvm_read_ptx_sreg_ctaid_x(KernelRuntimeState_t *rt_p,
                                KernelWorkItemState_t *wi_p) {
  return wi_p->block_id_x;
}
extern "C" unsigned
llvm_nvvm_read_ptx_sreg_ctaid_y(KernelRuntimeState_t *rt_p,
                                KernelWorkItemState_t *wi_p) {
  return wi_p->block_id_y;
}
extern "C" unsigned
llvm_nvvm_read_ptx_sreg_ctaid_z(KernelRuntimeState_t *rt_p,
                                KernelWorkItemState_t *wi_p) {
  return wi_p->block_id_z;
}

extern "C" unsigned
llvm_nvvm_read_ptx_sreg_ntid_x(KernelRuntimeState_t *rt_p,
                               KernelWorkItemState_t *wi_p) {
  return rt_p->block_size_x;
}
extern "C" unsigned
llvm_nvvm_read_ptx_sreg_ntid_y(KernelRuntimeState_t *rt_p,
                               KernelWorkItemState_t *wi_p) {
  return rt_p->block_size_y;
}
extern "C" unsigned
llvm_nvvm_read_ptx_sreg_ntid_z(KernelRuntimeState_t *rt_p,
                               KernelWorkItemState_t *wi_p) {
  return rt_p->block_size_z;
}

extern "C" unsigned
llvm_nvvm_read_ptx_sreg_nctaid_x(KernelRuntimeState_t *rt_p,
                                 KernelWorkItemState_t *wi_p) {
  return rt_p->grid_size_x;
}
extern "C" unsigned
llvm_nvvm_read_ptx_sreg_nctaid_y(KernelRuntimeState_t *rt_p,
                                 KernelWorkItemState_t *wi_p) {
  return rt_p->grid_size_y;
}
extern "C" unsigned
llvm_nvvm_read_ptx_sreg_nctaid_z(KernelRuntimeState_t *rt_p,
                                 KernelWorkItemState_t *wi_p) {
...
extern "C" float llvm_nvvm_atomic_load_add_f32_p0f32(float *address,
                                                     float val) {
  ETCC_STUB();
  return 0;
}
...
extern "C" int __nv_min(int a, int b) { return device_min(a, b); }

extern "C" int __nv_max(int a, int b) { return device_max(a, b); }
...
extern "C" float __nv_expf(float a) { return device_expf(a); }
...
extern "C" void __nv_sincosf(float a, float *o1_p, float *o2_p) { ETCC_STUB(); }
