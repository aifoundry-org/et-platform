...
typedef void (*KernelWorkItemFunction_t)(void* args, KernelRuntimeState_t* state_p, KernelWorkItemState_t* wi_state_p);
...
// like libc roundf: round x to the nearest integer, but round halfway cases away from zero
...
AUX_FUN_ATTRS int device_min(int a, int b)
{
    return a < b ? a : b;
}

AUX_FUN_ATTRS int device_max(int a, int b)
{
    return a > b ? a : b;
}
