...
#define etrtHostAllocPortable               0x01  /**< Pinned memory accessible by all contexts */
...
#define etrtHostRegisterDefault             0x00  /**< Default host memory registration flag */
#define etrtHostRegisterPortable            0x01  /**< Pinned memory accessible by all contexts */
#define etrtHostRegisterMapped              0x02  /**< Map registered memory into device space */
...
#define etrtStreamNonBlocking               0x01  /**< Stream does not synchronize with stream 0 (the NULL stream) */
...
#define etrtEventInterprocess               0x04  /**< Event is suitable for interprocess use. etrtEventDisableTiming must be set */

#define etrtDeviceScheduleAuto              0x00  /**< Device flag - Automatic scheduling */
#define etrtDeviceScheduleSpin              0x01  /**< Device flag - Spin default scheduling */
#define etrtDeviceScheduleYield             0x02  /**< Device flag - Yield default scheduling */
#define etrtDeviceScheduleBlockingSync      0x04  /**< Device flag - Use blocking synchronization */
#define etrtDeviceScheduleMask              0x07  /**< Device schedule flags mask */
#define etrtDeviceMapHost                   0x08  /**< Device flag - Support mapped pinned allocations */
#define etrtDeviceLmemResizeToMax           0x10  /**< Device flag - Keep local memory allocation after launch */
#define etrtDeviceMask                      0x1f  /**< Device flags mask */

#define etrtIpcMemLazyEnablePeerAccess      0x01  /**< Automatically enable peer access between remote devices as needed */

#define etrtMemAttachGlobal                 0x01  /**< Memory can be accessed by any stream on any device*/
#define etrtMemAttachHost                   0x02  /**< Memory cannot be accessed by any stream on any device */
#define etrtMemAttachSingle                 0x04  /**< Memory can only be accessed by a single stream on the associated device */

#define etrtOccupancyDefault                0x00  /**< Default behavior */
#define etrtOccupancyDisableCachingOverride 0x01  /**< Assume global caching is enabled and cannot be automatically turned off */

#define etrtCpuDeviceId                     ((int)-1) /**< Device id that represents the CPU */
#define etrtInvalidDeviceId                 ((int)-2) /**< Device id that represents an invalid device */

/**
 * @brief  Flag to make kernel launch obey pre-synch rules
 *
 * If set, each kernel launched as part of ::etrtLaunchCooperativeKernelMultiDevice only
 * waits for prior work in the stream corresponding to that Device to complete before the
 ...
 * ::etrtLaunchCooperativeKernelMultiDevice will only wait for the kernel launched on
 * the Device corresponding to that stream to complete before it begins execution.
 */
...
enum etrtError {
    ...
    /**
     * The device function being invoked (usually via ::etrtLaunchKernel()) was not
     * previously configured via the ::etrtConfigureCall() function.
     */
     ...
     * This leaves the process in an inconsistent state and any further ET Runtime work
     * will return the same error. To continue using the ET stack, the process must be
     ...
     * work will return the same error. To continue using the ET stack, the process
     ...
     * launched via either ::etrtLaunchCooperativeKernel or ::etrtLaunchCooperativeKernelMultiDevice
     * exceeds the maximum number of blocks as allowed by ::etrtOccupancyMaxActiveBlocksPerMultiprocessor
     * or ::etrtOccupancyMaxActiveBlocksPerMultiprocessorWithFlags times the number of multiprocessors
     * as specified by the device attribute ::etrtDevAttrMultiProcessorCount.
     */
...
enum etrtMemcpyKind {
    ...
    etrtMemcpyDefault             =   4       /**< Direction of the transfer is inferred from the pointer values. Requires unified virtual addressing */
};
...
struct etrtDeviceProp {
    ...
    int    canMapHostMemory;           /**< Device can map host memory with etrtHostAlloc/etrtHostGetDevicePointer */
    ...
    int    multiDeviceBoardGroupID;    /**< Unique identifier for a group of devices on the same multi-device board */
    int    hostNativeAtomicSupported;  /**< Link between the device and the host supports native atomic operations */
    int    singleToDoublePrecisionPerfRatio; /**< Ratio of single precision performance (in floating-point operations per second) to double precision performance */
    int    pageableMemoryAccess;       /**< Device supports coherently accessing pageable memory without calling etrtHostRegister on it */
    int    concurrentManagedAccess;    /**< Device can coherently access managed memory concurrently with the CPU */
    int    computePreemptionSupported; /**< Device supports Compute Preemption */
    int    canUseHostPointerForRegisteredMem; /**< Device can access host registered memory at the same virtual address as the CPU */
    int    cooperativeLaunch;          /**< Device supports launching cooperative kernels via ::etrtLaunchCooperativeKernel */
    int    cooperativeMultiDeviceLaunch; /**< Device can participate in cooperative kernels launched via ::etrtLaunchCooperativeKernelMultiDevice */
