...
//
// test_load_bw_open_8b kernel test:
// Invokes the load_bw kernel and sends it a tensor
// with pointers. Each minion will access 1000 addresses.
// Minions are grouped in groups of 8 based on (minion_id mod 128).
// Within the group memory accesses go to the sam MS/MC/Bank, and if they arrive clsoe to each other
// we can have open page accesses. Due to the unpredictability of the time arrival we do not know in advance
// how many open accesses we are going to have, one of the purposes of this test is to gauge that.
TEST_F(PCIEKernelLaunchTest, load_bw_open_8b_kernel) {
  ...
  // Rows are random.
  // Minions with the same (id mod 128) go to the same bank/ms/mc, and to different columns
  // If these requests arrive around the same time we have a high chance of open page access.
  uint64_t input_array[NUM_ADDRESSES];
  generator.seed(seed_val);

  for (uint64_t i=0; i < NUM_ITERATIONS; i++) {
    uint64_t row_val = row_distr(generator);
    for (uint64_t m=0; m < NUM_SHIRES * NUM_MINIONS_PER_SHIRE; m++) {
      uint64_t col_val = m / 128;
      uint64_t bank_mc_ms_val = m % 128;
      uint64_t idx = m * NUM_ITERATIONS + i;
      input_array[idx] = ((row_val << 18) | (col_val << 13) | (bank_mc_ms_val << 6)) & 0xFFFFFFFFC0ULL;
      input_array[idx] = input_array[idx] + START_ADDRESS;
    }
  }
