...
//
// test_load_bw_closed_4b kernel test:
// Invokes the load_bw kernel and sends it a tensor
// with random pointers + the constraint that they go to odd MC banks.
// Each minion will access 1000 random addresses
// resulting into closed page requests equally distributes across all memory controllers.
TEST_F(PCIEKernelLaunchTest, load_bw_closed_4b_kernel) {
  ...
  // To access 4 banks set bit 10 to 1 to go to odd banks only
  ...
  for (uint32_t i=0; i < NUM_ADDRESSES; i++) {
    ...
    bank_mc_ms_val = bank_mc_ms_val | 0x010ULL;
