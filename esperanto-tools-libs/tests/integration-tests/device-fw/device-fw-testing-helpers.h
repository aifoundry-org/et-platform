...
bool Test_Trace_verify_ring_buffer_data(uint8_t *trace_buffers,
                                        size_t buffer_size,
                                        ...
                                        int test_case_nm)
{
    ...
    for (; i < num_of_buffers; i++)
    {
        ...
        while (tail != head)
        {
            ...
            case ::device_api::non_privileged::TRACE_EVENT_ID_TEXT_STRING:
            {
                ...
                tail += ALIGN(offsetof(::device_api::non_privileged::trace_string_t,
                                       msg) +
                                  trace_string->size,
                              8);
                break;
    ...
    if ((test_status == true) && ((all_minions_msg_counter == trace_buffer_counter) || (all_minions_msg_counter == (max_str_events_per_buff * trace_buffer_counter))))
    {
        return true;
    }
    else
    {
        RTDEBUG << "Failed: test_status: " << test_status << " All minion messages counter: " << all_minions_msg_counter << "trace_buffer_counter: " << trace_buffer_counter;
        return false;
    }
}
