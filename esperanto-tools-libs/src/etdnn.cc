...
template <class A>
std::string setArgsAlphaBetaAndFixKernelName(A* args,
...
/**
 * Check that tensor dense packed in memory in specific format (without holes in memory).
 */
static bool
isTensorInDenseFormat( EtDnnTensorStruct*     tensorStruct,
                       etdnnTensorFormat_t    format)
{
    ...
    switch ( format )
    {
        case ETDNN_TENSOR_NCHW:
            ...
            nStride = tensorStruct->sizes[W_index] * tensorStruct->sizes[H_index] * tensorStruct->sizes[C_index];
            break;
        case ETDNN_TENSOR_NHWC:
            cStride = 1;
            wStride = tensorStruct->sizes[C_index];
            hStride = tensorStruct->sizes[C_index] * tensorStruct->sizes[W_index];
            nStride = tensorStruct->sizes[C_index] * tensorStruct->sizes[W_index] * tensorStruct->sizes[H_index];
            break;
...
etdnnGetVersion()
{
    return ETDNN_VERSION;
}

EXAPI etdnnStatus_t
etdnnCreate(etdnnHandle_t *handle)
{
...
EXAPI etdnnStatus_t
etdnnDestroy(etdnnHandle_t handle)
{
...
EXAPI etdnnStatus_t
etdnnSetStream(etdnnHandle_t handle, etrtStream_t streamId)
{
    EtDnnContext *ctx = (EtDnnContext *)handle;
...
EXAPI etdnnStatus_t
etdnnGetStream(etdnnHandle_t handle, etrtStream_t *streamId)
{
    EtDnnContext *ctx = (EtDnnContext *)handle;
 ...
 *
 * In contradistinction to cudnnSetTensor4dDescriptor this function receives explicitly passed stride values
 */
EXAPI etdnnStatus_t
etdnnSetTensor4dDescriptorEx( etdnnTensorDescriptor_t     tensorDesc,
                              etdnnDataType_t             dataType,
                              int                         n,
                              int                         c,
                              int                         h,
 ...
 */
EXAPI etdnnStatus_t
etdnnSetTensor4dDescriptor( etdnnTensorDescriptor_t     tensorDesc,
                            etdnnTensorFormat_t         format,
                            etdnnDataType_t             dataType,
                            int                         n,
                            int                         c,
                            ...
                            int                         w)
{
    ...
    return etdnnSetTensor4dDescriptorEx( tensorDesc, dataType, n, c, h, w, nStride, cStride, hStride, wStride);
}
 ...
 */
EXAPI etdnnStatus_t
etdnnSetTensorNdDescriptor( etdnnTensorDescriptor_t tensorDesc,
                            etdnnDataType_t         dataType,
                            int                     nbDims,
                            const int               *dimA,
                            const int               *strideA)
{
    ...
    // NOTE: need to check correctness for dim > 4. For now there's no appropriate test case.
    if ( nbDims != 4 )
    {
        abort();
    }
 ...
 */
EXAPI etdnnStatus_t
etdnnSetTensorNdDescriptorEx( etdnnTensorDescriptor_t tensorDesc,
                              etdnnTensorFormat_t     format,
                              etdnnDataType_t         dataType,
                              int                     nbDims,
                              const int               *dimA)
{
    // There's some doubt with this function. Better leave in unimplemented until we meet it's call in real test case.
    abort();
}
 ...
 */
EXAPI etdnnStatus_t
etdnnSetFilter4dDescriptor( etdnnFilterDescriptor_t    filterDesc,
                            etdnnDataType_t            dataType,
                            etdnnTensorFormat_t        format,
                            int                        k,
                            int                        c,
 ...
 */
EXAPI etdnnStatus_t
etdnnSetConvolution2dDescriptor( etdnnConvolutionDescriptor_t    convDesc,
                                 int                             pad_h,
                                 int                             pad_w,
                                 int                             u,
                                 int                             v,
...
EXAPI etdnnStatus_t
etdnnSetConvolutionGroupCount( etdnnConvolutionDescriptor_t convDesc,
                               int groupCount)
{
 ...
 */
EXAPI etdnnStatus_t
etdnnSetActivationDescriptor( etdnnActivationDescriptor_t         activationDesc,
 ...
 */
EXAPI etdnnStatus_t
etdnnSetPooling2dDescriptor( etdnnPoolingDescriptor_t    poolingDesc,
 ...
 */
EXAPI etdnnStatus_t
etdnnActivationForward( etdnnHandle_t handle,
                        ...
                        void                           *y)
{
    ...
    ActivationForward_KernelArgs_t args_struct = { ad->mode, ad->nan_prop, (float) ad->coef, 
                                                   xt->dataType, xt->dimensions,
                                                   xt->sizes[N_index], xt->sizes[C_index], xt->sizes[H_index], xt->sizes[W_index],
                                                   xt->strides[N_index], xt->strides[C_index], xt->strides[H_index], xt->strides[W_index],
                                                   yt->strides[N_index], yt->strides[C_index], yt->strides[H_index], yt->strides[W_index],
                                                   x, y, { 0 }, { 0 }};
    ...
    kernel_name = setArgsAlphaBetaAndFixKernelName(&args_struct, kernel_name, args_struct.data_type, alpha, beta);

    dim3 gridDim(defaultGridDim1D(args_struct.n*args_struct.c*args_struct.h*args_struct.w));
...
EXAPI etdnnStatus_t
etdnnActivationLinear( etdnnHandle_t handle,
                       etdnnActivationMode_t           mode,
                       const etdnnTensorDescriptor_t   xDesc,
                       const void                     *x,
                       const etdnnTensorDescriptor_t   yDesc,
                       void                           *y)
{
    ...
    int len = yt->sizes[N_index]*yt->sizes[C_index]*yt->sizes[H_index]*yt->sizes[W_index];
 ...
 */
EXAPI etdnnStatus_t
etdnnPoolingForward( etdnnHandle_t handle,
                     etdnnPoolingDescriptor_t       poolingDesc,
                     const void                     *alpha,
                     const etdnnTensorDescriptor_t   xDesc,
                     const void                     *x,
                     const void                     *beta,
...
//    }
    ...
    PoolingForward_KernelArgs_t args_struct = { ps->mode, ps->nan_prop, ps->window_h, ps->window_w,
                                                ps->vertical_pad, ps->horizontal_pad, ps->vertical_stride, ps->horizontal_stride,
                                                xt->dataType, xt->dimensions,
                                                xt->sizes[N_index], xt->sizes[C_index], xt->sizes[H_index], xt->sizes[W_index],
                                                xt->strides[N_index], xt->strides[C_index], xt->strides[H_index], xt->strides[W_index],
                                                yt->sizes[N_index], yt->sizes[C_index], yt->sizes[H_index], yt->sizes[W_index],
                                                yt->strides[N_index], yt->strides[C_index], yt->strides[H_index], yt->strides[W_index],
                                                x, y, { 0 }, { 0 },
                                                xt->scale, xt->offset, yt->scale, yt->offset };
    ...
    dim3 gridDim(defaultGridDim1D(args_struct.out_n*args_struct.out_c*args_struct.out_h*args_struct.out_w));
    dim3 blockDim(defaultBlockDim1D());
    etrtConfigureCall(gridDim, blockDim, 0, reinterpret_cast<etrtStream_t>(ctx->etStream));
    etrtSetupArgument(&args_struct, sizeof(args_struct), 0);
    etrtLaunch(nullptr, kernel_name.c_str());
 ...
 */
EXAPI etdnnStatus_t
etdnnAddTensor( etdnnHandle_t                    handle,
                const void                       *alpha,
                ...
                void                             *C)
{
    ...
    if ( at->dataType != ct->dataType
         || at->dimensions != ct->dimensions
         || at->dimensions < 1)
    {
        return ETDNN_STATUS_BAD_PARAM;
    }
    ...
    AddTensor_KernelArgs_t args_struct = { at->dataType, at->dimensions,
                                           at->sizes[N_index], at->sizes[C_index], at->sizes[H_index], at->sizes[W_index], 0,
                                           ct->sizes[N_index], ct->sizes[C_index], ct->sizes[H_index], ct->sizes[W_index], 0,
                                           at->strides[N_index], at->strides[C_index], at->strides[H_index], at->strides[W_index], 0,
                                           ct->strides[N_index], ct->strides[C_index], ct->strides[H_index], ct->strides[W_index], 0,
                                           A, C, { 0 }, { 0 }};

    std::string kernel_name = setArgsAlphaBetaAndFixKernelName(&args_struct, "AddTensor", args_struct.data_type, alpha, beta);
...
EXAPI etdnnStatus_t
etdnnDeriveBNTensorDescriptor( etdnnTensorDescriptor_t         derivedBnDesc,
                               const etdnnTensorDescriptor_t   xDesc,
                               etdnnBatchNormMode_t            mode)
{
    ...
    etdnnDataType_t derived_type = (tx->dataType == ETDNN_DATA_HALF) ? ETDNN_DATA_FLOAT : tx->dataType;

    switch ( mode )
    {
      case ETDNN_BATCHNORM_PER_ACTIVATION:
        etdnnSetTensor4dDescriptor(derivedBnDesc, ETDNN_TENSOR_NCHW, derived_type, 1, tx->sizes[C_index], tx->sizes[H_index], tx->sizes[W_index]);
        break;
      case ETDNN_BATCHNORM_SPATIAL:
      case ETDNN_BATCHNORM_SPATIAL_PERSISTENT:
        etdnnSetTensor4dDescriptor(derivedBnDesc, ETDNN_TENSOR_NCHW, derived_type, 1, tx->sizes[C_index], 1, 1);
        break;
      default:
        return ETDNN_STATUS_BAD_PARAM;
    }
...
EXAPI etdnnStatus_t
etdnnBatchNormalizationForwardInference( etdnnHandle_t                    handle,
                                         etdnnBatchNormMode_t             mode,
                                         const void                      *alpha,
                                         const void                      *beta,
                                         const etdnnTensorDescriptor_t    xDesc,
                                         ...
                                         double                           epsilon)
{
    ...
    if ( bnt->sizes[N_index] != 1
         || bnt->sizes[C_index] != xt->sizes[C_index] )
    {
        return ETDNN_STATUS_BAD_PARAM;
    }
    ...
    BatchNormalizationForwardInference_KernelArgs_t args_struct = { xt->dataType, bnt->dataType, xt->dimensions, mode,
                                                                    xt->sizes[N_index], xt->sizes[C_index], xt->sizes[H_index], xt->sizes[W_index],
                                                                    xt->strides[N_index], xt->strides[C_index], xt->strides[H_index], xt->strides[W_index],
                                                                    yt->strides[N_index], yt->strides[C_index], yt->strides[H_index], yt->strides[W_index],
                                                                    bnt->strides[N_index], bnt->strides[C_index], bnt->strides[H_index], bnt->strides[W_index],
                                                                    x, y, bnScale, bnBias, estimatedMean, estimatedVariance,
                                                                    { 0 }, { 0 }, (float)epsilon};

    std::string kernel_name = setArgsAlphaBetaAndFixKernelName(&args_struct, "BatchNormalizationForwardInference", args_struct.data_type, alpha, beta);
...
EXAPI etdnnStatus_t
etdnnGetConvolutionForwardAlgorithm( etdnnHandle_t                      handle,
                                     const etdnnTensorDescriptor_t      xDesc,
...
EXAPI etdnnStatus_t
etdnnGetConvolutionForwardWorkspaceSize( etdnnHandle_t                           handle,
                                         const   etdnnTensorDescriptor_t         xDesc,
                                         const   etdnnFilterDescriptor_t         wDesc,
                                         const   etdnnConvolutionDescriptor_t    convDesc,
                                         const   etdnnTensorDescriptor_t         yDesc,
 ...
 */
EXAPI etdnnStatus_t
etdnnBiasedConvolutionForward( etdnnHandle_t                       handle,
                               const void                         *alpha,
                               const etdnnTensorDescriptor_t       xDesc,
                               const void                         *x,
                               const etdnnFilterDescriptor_t       wDesc,
                               ...
                               void                               *y)
{
    ...
    if ( xt->dataType != wt->dataType
         || yt->dataType != wt->dataType )
    {
        return ETDNN_STATUS_BAD_PARAM;
    }
    ...
    if ( wt->h == 1 && wt->w == 1 // Optimization for 1x1 filter size
         && xt->dataType == ETDNN_DATA_FLOAT
         && (*(float*)alpha) == 1.0f
         && (*(float*)beta) == 0.0f
         && conv->pad_h == 0 && conv->pad_w == 0
         && conv->dilation_h == 1 && conv->dilation_w == 1
         && conv->u == 1 && conv->v == 1
         && isTensorInDenseFormat(xt, ETDNN_TENSOR_NHWC)
         && isTensorInDenseFormat(yt, ETDNN_TENSOR_NHWC)
         && wt->format == ETDNN_TENSOR_NHWC
         && is_aligned((uintptr_t)x, 64)
         && is_aligned((uintptr_t)w, 64)
         && is_aligned((uintptr_t)y, 64)
         && is_aligned(xt->sizes[C_index]*sizeof(float), 64)
         && is_aligned(wt->k*sizeof(float), 64))
    {
        ...
        etiMatMulTensor(reinterpret_cast<etrtStream_t>(ctx->etStream),
                        m, n, k,
                        (const float*)x, k, false,
                        (const float*)w, k, true,
                        (float*)y, n);

        if ( bias != 0 )
        {
            etiBatchedAdd(reinterpret_cast<etrtStream_t>(ctx->etStream),
                          yt->sizes[N_index]*yt->sizes[H_index]*yt->sizes[W_index],
                          yt->sizes[C_index],
                          (float*)y, (float*)bias, (float*)y);
        }
        return ETDNN_STATUS_SUCCESS;
    }
         ...
         && is_aligned(wt->h*wt->w*wt->k*sizeof(float), 64))
    {
         ...
         * 1. MatMul into workspace, which caches all pairwise dot products of feature maps,
         ...
         * C - intermediate matrix m x n with cached pairwise dot products of feature maps.
         */
        int m = xt->sizes[H_index] * xt->sizes[W_index] * xt->sizes[N_index];
        int n = wt->h * wt->w * wt->k;
        int k = xt->sizes[C_index];
        ...
        if (neededWorkSpaceSizeInBytes <= workSpaceSizeInBytes)
        {
            ...
            ConvTailNHWC_KernelArgs_t args_struct = { wt->h, wt->w,
                                                      xt->sizes[C_index], xt->sizes[H_index], xt->sizes[W_index],
                                                      yt->sizes[C_index], yt->sizes[H_index], yt->sizes[W_index],
                                                      yt->sizes[N_index],
                                                      conv->pad_h, conv->pad_w, conv->u, conv->v, conv->dilation_h, conv->dilation_w,
                                                      workSpace, bias, y,
                                                      xt->scale, xt->offset, wt->scale, wt->offset, conv->biasScale, conv->biasOffset,
                                                      yt->scale, yt->offset };
            dim3 gridDim(defaultGridDim1D(args_struct.batch_size*args_struct.out_c*args_struct.out_h*args_struct.out_w));
            dim3 blockDim(defaultBlockDim1D());
            etrtConfigureCall(gridDim, blockDim, 0, reinterpret_cast<etrtStream_t>(ctx->etStream));
            etrtSetupArgument(&args_struct, sizeof(args_struct), 0);
         ...
         && is_aligned(wt->h*wt->w*wt->k*sizeof(int32_t), 64))
    {
        ...
        if (neededWorkSpaceSizeInBytes <= workSpaceSizeInBytes)
        {
            etiMatMulTensorInt8To32(reinterpret_cast<etrtStream_t>(ctx->etStream),
                                    m, n, k,
                                    (const int8_t*)x, k, false,
                                    (const int8_t*)w, k, true,
                                    (int32_t*)workSpace, n);
    ...
    ConvolutionForward_KernelArgs_t args_struct = { xt->dataType, wt->format,
                                                    wt->k, wt->c, wt->h, wt->w,
                                                    xt->sizes[N_index], xt->sizes[C_index], xt->sizes[H_index], xt->sizes[W_index],
                                                    xt->strides[N_index], xt->strides[C_index], xt->strides[H_index], xt->strides[W_index],
                                                    yt->sizes[N_index], yt->sizes[C_index], yt->sizes[H_index], yt->sizes[W_index],
                                                    yt->strides[N_index], yt->strides[C_index], yt->strides[H_index], yt->strides[W_index],
                                                    conv->pad_h, conv->pad_w, conv->u, conv->v, conv->dilation_h, conv->dilation_w,
                                                    workSpace, workSpaceSizeInBytes, x, w, bias, y,
                                                    { 0 }, { 0 },
                                                    xt->scale, xt->offset, wt->scale, wt->offset, conv->biasScale, conv->biasOffset,
                                                    yt->scale, yt->offset };

    std::string kernel_name = setArgsAlphaBetaAndFixKernelName(&args_struct, "ConvolutionForward", args_struct.data_type, alpha, beta);
 ...
 */
EXAPI etdnnStatus_t
etdnnConvolutionForward( etdnnHandle_t                       handle,
                         const void                         *alpha,
                         const etdnnTensorDescriptor_t       xDesc,
                         const void                         *x,
                         const etdnnFilterDescriptor_t       wDesc,
                         ...
                         void                               *y)
{
    return etdnnBiasedConvolutionForward(handle, alpha, xDesc, x, wDesc, w, convDesc, NULL, algo, workSpace, workSpaceSizeInBytes, beta, yDesc, y);
}

EXAPI etdnnStatus_t
etdnnSoftmaxForward( etdnnHandle_t                    handle,
                     ...
                     void                            *y)
{
    ...
    SoftmaxForward_KernelArgs_t args_struct = { xt->dataType, mode, xt->dimensions,
                                                xt->sizes[N_index], xt->sizes[C_index], xt->sizes[H_index], xt->sizes[W_index],
                                                xt->strides[N_index], xt->strides[C_index], xt->strides[H_index], xt->strides[W_index],
                                                yt->strides[N_index], yt->strides[C_index], yt->strides[H_index], yt->strides[W_index],
                                                x, y, { 0 }, { 0 }};
    ...
    if ( mode == ETDNN_SOFTMAX_MODE_INSTANCE )
    {
        ...
        // For Block we request whole number of Shires, but no more than max possible Block size.
        // Each batch (n-dimension) is handled independently from each other in separate Blocks.
        unsigned reduce_len = args_struct.c*args_struct.h*args_struct.w;
        dim3 gridDim(args_struct.n);
        dim3 blockDim(std::min<unsigned>(align_up(reduce_len, MINIONS_PER_SHIRE), MINIONS_COUNT));
        etrtConfigureCall(gridDim, blockDim, 0, reinterpret_cast<etrtStream_t>(ctx->etStream));
        etrtSetupArgument(&args_struct, sizeof(args_struct), 0);
        etrtLaunch(nullptr, kernel_name.c_str());
    } else
...
EXAPI etdnnStatus_t
etdnnTransformTensor( etdnnHandle_t                  handle,
                      const void                    *alpha,
                      const etdnnTensorDescriptor_t  xDesc,
                      const void                    *x,
                      const void                    *beta,
                      ...
                      void                          *y)
{
    ...
    TransformTensor_KernelArgs_t args_struct = { xt->dataType, xt->dimensions,
                                                 xt->sizes[N_index], xt->sizes[C_index], xt->sizes[H_index], xt->sizes[W_index],
                                                 xt->strides[N_index], xt->strides[C_index], xt->strides[H_index], xt->strides[W_index],
                                                 yt->strides[N_index], yt->strides[C_index], yt->strides[H_index], yt->strides[W_index],
                                                 x, y, { 0 }, { 0 }};

    std::string kernel_name = setArgsAlphaBetaAndFixKernelName(&args_struct, "TransformTensor", args_struct.data_type, alpha, beta);
...
EXAPI etdnnStatus_t
etdnnSplat( etdnnHandle_t                    handle,
            const void                       *val,
            const etdnnTensorDescriptor_t    yDesc,
            void                             *y)
{
...
EXAPI etdnnStatus_t
etdnnArithOp( etdnnHandle_t                    handle,
              etdnnArithOp_t                   op,
              const etdnnTensorDescriptor_t    xDesc,
              const void *                     x,
              const etdnnTensorDescriptor_t    yDesc,
              ...
              void *                           z)
{
    ...
    if ( zt->dimensions != 4
         || xt->dimensions != zt->dimensions
         || yt->dimensions != zt->dimensions
         || xt->dataType != zt->dataType
    ...
    for ( int i = 0; i < xt->dimensions; i++ )
    {
        if ( yt->sizes[i] != zt->sizes[i]
             || xt->sizes[i] != zt->sizes[i] )
        {
            abort();
        }
    }
    ...
    int len = zt->sizes[N_index]*zt->sizes[C_index]*zt->sizes[H_index]*zt->sizes[W_index];
    dim3 gridDim(defaultGridDim1D(len));
    dim3 blockDim(defaultBlockDim1D());
    etrtConfigureCall(gridDim, blockDim, 0, reinterpret_cast<etrtStream_t>(ctx->etStream));
    ...
    {
        QuantizedArithOp_KernelArgs_t kernel_args = { op, len, (int8_t*)x, (int8_t*)y, (int8_t*)z,
                                                      xt->scale, yt->scale, zt->scale,
                                                      xt->offset, yt->offset, zt->offset };
        etrtSetupArgument(&kernel_args, sizeof(kernel_args), 0);
    } else {
        etrtSetupArgument( &len, 4, 0);
        etrtSetupArgument( &x, 8, 8);
...
EXAPI etdnnStatus_t
etdnnExtractTensor( etdnnHandle_t                    handle,
                    const etdnnTensorDescriptor_t    xDesc,
                    const void *                     x,
                    const etdnnTensorDescriptor_t    yDesc,
                    void *                           y,
...
EXAPI etdnnStatus_t
etdnnInsertTensor( etdnnHandle_t                    handle,
                   const etdnnTensorDescriptor_t    xDesc,
                   const void *                     x,
                   const etdnnTensorDescriptor_t    yDesc,
                   void *                           y,
...
EXAPI etdnnStatus_t
etdnnLocalResponseNormalization( etdnnHandle_t                    handle,
                                 const etdnnTensorDescriptor_t    xDesc,
                                 const void *                     x,
                                 const etdnnTensorDescriptor_t    yDesc,
                                 void *                           y,
                                 ...
                                 const void *                     beta)
{
    ...
    if ( xt->dimensions != scalet->dimensions
         || yt->dimensions != scalet->dimensions
         || xt->dimensions != 4 )
    {
        abort();
    }
    ...
    for ( int i = 0; i < xt->dimensions; i++ )
    {
        if ( xt->sizes[i] != scalet->sizes[i]
             || yt->sizes[i] != scalet->sizes[i] )
        {
            abort();
        }
    }
    ...
    if ( xt->dataType != ETDNN_DATA_FLOAT
         || xt->dataType != scalet->dataType
         || yt->dataType != scalet->dataType )
    {
        abort();
    }

    LRN_KernelArgs_t args_struct = { xt->dataType, xt->dimensions,
                                     xt->sizes[N_index], xt->sizes[C_index], xt->sizes[H_index], xt->sizes[W_index],
                                     xt->strides[N_index], xt->strides[C_index], xt->strides[H_index], xt->strides[W_index],
                                     yt->strides[N_index], yt->strides[C_index], yt->strides[H_index], yt->strides[W_index],
                                     scalet->strides[N_index], scalet->strides[C_index], scalet->strides[H_index], scalet->strides[W_index],
                                     x, y, scale,
                                     halfWindowSize, { 0 }, { 0 }, { 0 }};
...
EXAPI etdnnStatus_t
etdnnSelect( etdnnHandle_t                    handle,
             const etdnnTensorDescriptor_t    xDesc,
             const void *                     x,
             const etdnnTensorDescriptor_t    yDesc,
             const void *                     y,
             ...
             void *                           z)
{
    ...
    if ( zt->dimensions != 4
         || xt->dimensions != zt->dimensions
         || yt->dimensions != zt->dimensions
         || condt->dimensions != zt->dimensions
         || xt->dataType != zt->dataType
         || yt->dataType != zt->dataType
    ...
    if ( zt->dataType != ETDNN_DATA_FLOAT
         || xt->dataType != zt->dataType
         || yt->dataType != zt->dataType
         || condt->dataType != zt->dataType )
    {
