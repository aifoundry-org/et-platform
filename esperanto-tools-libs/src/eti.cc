 ...
 */
void
etiMatMulTensor(etrtStream_t stream,
                int m,
                int n,
                int k,
                const float *A, int lda, bool is_transa,
                const float *B, int ldb, bool is_transb,
                float *C, int ldc)
{
    {
        GetDev dev;
     ...
     * Note also that Esperanto tensor instructions have internally row-major layout;
     */
    ...
    dim3 gridDim(defaultGridDim1D((align_up(m, 16) / 16) * (align_up(n, 16) / 16)));
    dim3 blockDim(defaultBlockDim1D());
    etrtConfigureCall(gridDim, blockDim, 0, stream);

    MatMulTensor_KernelArgs_t args_struct = { m, n, k,
                                              A, lda, is_transa,
                                              B, ldb, is_transb,
                                              C, ldc };
...
void
etiMatMulTensorInt8To32(etrtStream_t stream,
                        int m,
                        int n,
                        int k,
                        ...
                        int32_t *C, int ldc)
{
    ...
    MatMulTensorInt8To32_KernelArgs_t args_struct = { m, n, k,
                                                      A, lda, is_transa,
                                                      B, ldb, is_transb,
                                                      C, ldc };
    etrtSetupArgument(&args_struct, sizeof(args_struct), 0);
...
void
etiBatchedAdd(etrtStream_t stream,
              int n,
              int slice_size,
              const float* batch,
              const float* slice,
              float* dest)
{
    ...
    BatchedAdd_KernelArgs_t args_struct = { n, slice_size,
                                            batch,
                                            slice,
                                            dest };
    etrtSetupArgument(&args_struct, sizeof(args_struct), 0);
...
void
etiQuantizedBatchedAdd(etrtStream_t stream,
                       int n,
                       int slice_size,
                       ...
                       int8_t* dest, float destScale, int32_t destOffset)
{
    ...
    QuantizedBatchedAdd_KernelArgs_t args_struct = { n, slice_size,
