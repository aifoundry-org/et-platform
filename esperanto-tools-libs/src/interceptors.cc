...
static int interceptor_log(const char *fmt, ...)
{
...
template <typename Ret, typename... Args> Ret InterceptFunction(const char* dlsym_name, Ret (*internal_fun_p)(Args...), Args... args)
{
...
/*
 * Note: __cudaRegisterFunction is called during DSO linking (before our global variables constructors),
 *       so in __cudaRegisterFunction we may only store data to statically initialized data types (POD).
 */
...
static const std::string &find_registered_function_name(const void *hostFun)
{
    ...
    std::call_once(flag, []() {
        ...
        {
            gRegisteredFunctions.insert({gRegisteredFunctionsPtr[i], gRegisteredFunctionsName[i]});
        }
    });
...
{
    void **fatCubinHandle = InterceptFunction<void**>("__cudaRegisterFatBinary", __etrtRegisterFatBinary, fatCubin);
...
{
    InterceptFunction<void>("__cudaUnregisterFatBinary", __etrtUnregisterFatBinary, fatCubinHandle);

//    interceptor_log("__cudaUnregisterFatBinary fcbhandle:%p\n", fatCubinHandle);
}

EXAPI void __cudaRegisterFunction(
        void   **fatCubinHandle,
  const char    *hostFun,
        char    *deviceFun,
...
//    interceptor_log("__cudaRegisterFunction fcbhandle:%p %p %s %d %p %p %p %p %p [%s]\n",
//                    fatCubinHandle, hostFun, deviceName, thread_limit, tid, bid, bDim, gDim, wSize,
//                    demangle(deviceName).c_str());
...
//                    fatCubinHandle, hostVar, deviceName, ext, size, constant, global);
}
...
{
    const char* res = InterceptFunction<const char*>("cudaGetErrorString", etrtGetErrorString, error);
...
{
    etrtError_t res = InterceptFunction<etrtError_t>("cudaGetDeviceCount", etrtGetDeviceCount, count);
    interceptor_log("cudaGetDeviceCount err:%d count:%d\n", res, *count);
    return res;
}

EXAPI etrtError_t cudaGetDeviceProperties(struct etrtDeviceProp *prop, int device)
{
    etrtError_t res = InterceptFunction<etrtError_t>("cudaGetDeviceProperties", etrtGetDeviceProperties, prop, device);
    interceptor_log("cudaGetDeviceProperties err:%d device:%d major:%d minor:%d name:%s totalGlobalMem:%lu maxThreadsPerBlock:%d\n",
                    res, device, prop->major, prop->minor, prop->name, prop->totalGlobalMem, prop->maxThreadsPerBlock);
    return res;
}
...
{
    etrtError_t res = InterceptFunction<etrtError_t>("cudaGetDevice", etrtGetDevice, device);
...
{
    etrtError_t res = InterceptFunction<etrtError_t>("cudaSetDevice", etrtSetDevice, device);
...
{
    etrtError_t res = InterceptFunction<etrtError_t>("cudaMallocHost", etrtMallocHost, ptr, size);
...
{
    etrtError_t res = InterceptFunction<etrtError_t>("cudaFreeHost", etrtFreeHost, ptr);
...
{
    etrtError_t res = InterceptFunction<etrtError_t>("cudaMalloc", etrtMalloc, devPtr, size);
...
{
    etrtError_t res = InterceptFunction<etrtError_t>("cudaFree", etrtFree, devPtr);
    interceptor_log("cudaFree err:%d devPtr:%p\n", res, devPtr);
    return res;
}

EXAPI etrtError_t cudaPointerGetAttributes(struct etrtPointerAttributes *attributes, const void *ptr)
{
    etrtError_t res = InterceptFunction<etrtError_t>("cudaPointerGetAttributes", etrtPointerGetAttributes, attributes, ptr);
    interceptor_log("cudaPointerGetAttributes err:%d ptr:%p memType:%d, device:%d, devPtr:%p hostPtr:%p isManaged:%d\n",
                    res, ptr, attributes->memoryType, attributes->device, attributes->devicePointer, attributes->hostPointer, attributes->isManaged);
    return res;
}

EXAPI etrtError_t cudaStreamCreate(etrtStream_t *pStream)
{
    etrtError_t res = InterceptFunction<etrtError_t>("cudaStreamCreate", etrtStreamCreate, pStream);
 ...
 * ::cuStreamCreate
 */
EXAPI etrtError_t cudaStreamCreateWithFlags(etrtStream_t *pStream, unsigned int flags)
{
    etrtError_t res = InterceptFunction<etrtError_t>("cudaStreamCreateWithFlags", etrtStreamCreateWithFlags, pStream, flags);
...
{
    etrtError_t res = InterceptFunction<etrtError_t>("cudaStreamDestroy", etrtStreamDestroy, stream);
    interceptor_log("cudaStreamDestroy err:%d stream:%p\n", res, stream);
    return res;
}

EXAPI etrtError_t cudaMemcpy(void *dst, const void *src, size_t count, enum etrtMemcpyKind kind)
{
    etrtError_t res = InterceptFunction<etrtError_t>("cudaMemcpy", etrtMemcpy, dst, src, count, kind);
    interceptor_log("cudaMemcpy err:%d dst:%p src:%p count:%lu kind:%d\n", res, dst, src, count, kind);
    return res;
}

EXAPI etrtError_t cudaMemcpyAsync(void *dst, const void *src, size_t count, enum etrtMemcpyKind kind, etrtStream_t stream /*__dv(0)*/)
{
    etrtError_t res = InterceptFunction<etrtError_t>("cudaMemcpyAsync", etrtMemcpyAsync, dst, src, count, kind, stream);
    interceptor_log("cudaMemcpyAsync err:%d dst:%p src:%p count:%lu kind:%d, stream:%p\n", res, dst, src, count, kind, stream);
    return res;
}

EXAPI etrtError_t cudaMemset( void* devPtr, int value, size_t count )
{
    etrtError_t res = InterceptFunction<etrtError_t>("cudaMemset", etrtMemset, devPtr, value, count);
    interceptor_log("cudaMemset err:%d devPtr:%p value:%d count:%lu \n", res, devPtr, value, count);
    return res;
}
...
{
    etrtError_t res = InterceptFunction<etrtError_t>("cudaStreamSynchronize", etrtStreamSynchronize, stream);
...
{
    etrtError_t res = InterceptFunction<etrtError_t>("cudaGetLastError", etrtGetLastError);
...
{
    etrtError_t res = InterceptFunction<etrtError_t>("cudaEventCreate", etrtEventCreate, event);
 ...
 * ::cuEventCreate
 */
EXAPI etrtError_t cudaEventCreateWithFlags(etrtEvent_t *event, unsigned int flags)
{
    etrtError_t res = InterceptFunction<etrtError_t>("cudaEventCreateWithFlags", etrtEventCreateWithFlags, event, flags);
...
{
    etrtError_t res = InterceptFunction<etrtError_t>("cudaEventQuery", etrtEventQuery, event);
...
{
    etrtError_t res = InterceptFunction<etrtError_t>("cudaEventRecord", etrtEventRecord, event, stream);
    interceptor_log("cudaEventRecord err:%d event:%p stream:%p\n", res, event, stream);
    return res;
}

EXAPI etrtError_t cudaStreamWaitEvent(etrtStream_t stream, etrtEvent_t event, unsigned int flags)
{
    etrtError_t res = InterceptFunction<etrtError_t>("cudaStreamWaitEvent", etrtStreamWaitEvent, stream, event, flags);
...
{
    etrtError_t res = InterceptFunction<etrtError_t>("cudaEventSynchronize", etrtEventSynchronize, event);
    interceptor_log("cudaEventSynchronize err:%d event:%p\n", res, event);
    return res;
}

EXAPI etrtError_t cudaEventElapsedTime(float* ms, etrtEvent_t start, etrtEvent_t end)
{
    etrtError_t res = InterceptFunction<etrtError_t>("cudaEventElapsedTime", etrtEventElapsedTime, ms, start, end);
...
{
    etrtError_t res = InterceptFunction<etrtError_t>("cudaEventDestroy", etrtEventDestroy, event);
 ...
 */
EXAPI etrtError_t cudaConfigureCall(dim3 gridDim, dim3 blockDim, size_t sharedMem /*__dv(0)*/, etrtStream_t stream /*__dv(0)*/)
{
    etrtError_t res = InterceptFunction<etrtError_t>("cudaConfigureCall", etrtConfigureCall, gridDim, blockDim, sharedMem, stream);
    interceptor_log("cudaConfigureCall err:%d gridDim:(%d,%d,%d) blockDim:(%d,%d,%d) sharedMem:%lu stream:%p\n",
                    res, gridDim.x, gridDim.y, gridDim.z, blockDim.x, blockDim.y, blockDim.z, sharedMem, stream);
    return res;
}

EXAPI etrtError_t cudaSetupArgument(const void *arg, size_t size, size_t offset)
{
    etrtError_t res = InterceptFunction<etrtError_t>("cudaSetupArgument", etrtSetupArgument, arg, size, offset);

    char arg_val_str[256] = "";
    switch (size) {
      case 1: sprintf(arg_val_str, "arg_val:%d(0x%x)", *(uint8_t*)arg, *(uint8_t*)arg); break;
      case 2: sprintf(arg_val_str, "arg_val:%d(0x%x)", *(uint16_t*)arg, *(uint16_t*)arg); break;
      case 4: sprintf(arg_val_str, "arg_val:%d(0x%x)", *(int*)arg, *(int*)arg); break;
      case 8: sprintf(arg_val_str, "arg_val:%lld(0x%llx)", *(long long *)arg, *(long long *)arg); break;
    }
...
EXAPI etrtError_t cudaLaunch(const void *func)
{
    ...
    etrtError_t err = use_cuda() ? cudaLaunch_p(func) : etrtLaunch(func, find_registered_function_name(func).c_str());

    const std::string &name = find_registered_function_name(func);
    interceptor_log("cudaLaunch err:%d hostFun:%p [-> %s [%s]]\n", err, func, name.c_str(), demangle(name).c_str());
...
{
    etblasStatus_t res = InterceptFunction<etblasStatus_t>("cublasCreate_v2", etblasCreate_v2, handle);
...
{
    etblasStatus_t res = InterceptFunction<etblasStatus_t>("cublasDestroy_v2", etblasDestroy_v2, handle);
    interceptor_log("cublasDestroy_v2 status:%d handle:%p\n", res, handle);
    return res;
}

EXAPI etblasStatus_t cublasSetPointerMode_v2(etblasHandle_t handle, etblasPointerMode_t mode)
{
    etblasStatus_t res = InterceptFunction<etblasStatus_t>("cublasSetPointerMode_v2", etblasSetPointerMode_v2, handle, mode);
    interceptor_log("cublasSetPointerMode_v2 status:%d handle:%p mode:%d\n", res, handle, mode);
    return res;
}

EXAPI etblasStatus_t cublasSetStream_v2(etblasHandle_t handle, etrtStream_t streamId)
{
    etblasStatus_t res = InterceptFunction<etblasStatus_t>("cublasSetStream_v2", etblasSetStream_v2, handle, streamId);
    interceptor_log("cublasSetStream_v2 status:%d handle:%p stream:%p\n", res, handle, streamId);
    return res;
}

EXAPI etblasStatus_t cublasSgemm_v2(etblasHandle_t handle,
                                    ...
                                    int ldc)
{
    etblasStatus_t res = InterceptFunction<etblasStatus_t>("cublasSgemm_v2", etblasSgemm_v2,
                                                           handle, transa, transb, m, n, k, alpha, A, lda, B, ldb, beta, C, ldc);
    interceptor_log("cublasSgemm_v2 status:%d handle:%p transa:%d transb:%d m:%d n:%d k:%d, alpha:%p A:%p lda:%d B:%p ldb:%d beta:%p C:%p ldc:%d\n",
                    res, handle, transa, transb, m, n, k, alpha, A, lda, B, ldb, beta, C, ldc);
    return res;
}
...
{
    etdnnStatus_t res = InterceptFunction<etdnnStatus_t>("cudnnCreate", etdnnCreate, handle);
...
{
    etdnnStatus_t res = InterceptFunction<etdnnStatus_t>("cudnnDestroy", etdnnDestroy, handle);
    interceptor_log("cudnnDestroy status:%d handle:%p\n", res, handle);
    return res;
}

EXAPI etdnnStatus_t cudnnSetStream(etdnnHandle_t handle, etrtStream_t streamId)
{
    etdnnStatus_t res = InterceptFunction<etdnnStatus_t>("cudnnSetStream", etdnnSetStream, handle, streamId);
    interceptor_log("cudnnSetStream status:%d handle:%p stream:%p\n", res, handle, streamId);
    return res;
}

EXAPI etdnnStatus_t cudnnGetStream(etdnnHandle_t handle, etrtStream_t *streamId)
{
    etdnnStatus_t res = InterceptFunction<etdnnStatus_t>("cudnnGetStream", etdnnGetStream, handle, streamId);
 ...
 */
EXAPI etdnnStatus_t cudnnCreateTensorDescriptor( etdnnTensorDescriptor_t *tensorDesc)
{
    etdnnStatus_t res = InterceptFunction<etdnnStatus_t>("cudnnCreateTensorDescriptor", etdnnCreateTensorDescriptor, tensorDesc);
...
*/
EXAPI etdnnStatus_t cudnnDestroyTensorDescriptor(etdnnTensorDescriptor_t tensorDesc)
{
    etdnnStatus_t res = InterceptFunction<etdnnStatus_t>("cudnnDestroyTensorDescriptor", etdnnDestroyTensorDescriptor, tensorDesc);
...
*/
EXAPI etdnnStatus_t cudnnSetTensor4dDescriptorEx( etdnnTensorDescriptor_t     tensorDesc,
                                                  ...
                                                  int                         wStride)
{
    etdnnStatus_t res = InterceptFunction<etdnnStatus_t>("cudnnSetTensor4dDescriptorEx", etdnnSetTensor4dDescriptorEx,
                                                         tensorDesc, dataType, n, c, h, w, nStride, cStride, hStride, wStride);
    interceptor_log("cudnnSetTensor4dDescriptor status:%d tensorDesc:%p dataType:%d n:%d c:%d h:%d w:%d nStride:%d cStride:%d hStride:%d wStride:%d\n",
                    res, tensorDesc, dataType, n, c, h, w, nStride, cStride, hStride, wStride);
    return res;
}
...
/*
    ...
    The total size of the tensor descriptor exceeds the maximim limit of 2 Giga-elements.
*/
EXAPI etdnnStatus_t cudnnSetTensor4dDescriptor( etdnnTensorDescriptor_t tensorDesc,
                                                ...
                                                int                     w)
{
    etdnnStatus_t res = InterceptFunction<etdnnStatus_t>("cudnnSetTensor4dDescriptor", etdnnSetTensor4dDescriptor,
                                                         tensorDesc, format, dataType, n, c, h, w);
    interceptor_log("cudnnSetTensor4dDescriptor status:%d tensorDesc:%p format:%d dataType:%d n:%d c:%d h:%d w:%d\n",
...
/*
    ...
    The resources could not be allocated.
*/
EXAPI etdnnStatus_t cudnnCreateFilterDescriptor( etdnnFilterDescriptor_t *filterDesc)
{
    etdnnStatus_t res = InterceptFunction<etdnnStatus_t>("cudnnCreateFilterDescriptor", etdnnCreateFilterDescriptor, filterDesc);
...
/*
    ...
    The object was destroyed successfully.
*/
EXAPI etdnnStatus_t cudnnDestroyFilterDescriptor( etdnnFilterDescriptor_t filterDesc)
{
    etdnnStatus_t res = InterceptFunction<etdnnStatus_t>("cudnnDestroyFilterDescriptor", etdnnDestroyFilterDescriptor, filterDesc);
 ...
 */
EXAPI etdnnStatus_t cudnnSetFilter4dDescriptor( etdnnFilterDescriptor_t    filterDesc,
                                                etdnnDataType_t            dataType,
                                                etdnnTensorFormat_t        format,
                                                int                        k,
                                                int                        c,
                                                ...
                                                int                        w)
{
    etdnnStatus_t res = InterceptFunction<etdnnStatus_t>("cudnnSetFilter4dDescriptor", etdnnSetFilter4dDescriptor,
                                                         filterDesc, dataType, format, k, c, h, w);
    interceptor_log("cudnnSetFilter4dDescriptor status:%d filterDesc:%p dataType:%d format:%d k:%d c:%d h:%d w:%d\n", res, filterDesc, dataType, format, k, c, h, w);
    return res;
}
...
/*
    ...
    The resources could not be allocated.
 */
EXAPI etdnnStatus_t cudnnCreateConvolutionDescriptor( etdnnConvolutionDescriptor_t *convDesc)
{
    etdnnStatus_t res = InterceptFunction<etdnnStatus_t>("cudnnCreateConvolutionDescriptor", etdnnCreateConvolutionDescriptor, convDesc);
...
/*
    ...
    The object was destroyed successfully.
*/
EXAPI etdnnStatus_t cudnnDestroyConvolutionDescriptor( etdnnConvolutionDescriptor_t convDesc)
{
    etdnnStatus_t res = InterceptFunction<etdnnStatus_t>("cudnnDestroyConvolutionDescriptor", etdnnDestroyConvolutionDescriptor, convDesc);
 ...
 */
EXAPI etdnnStatus_t cudnnSetConvolution2dDescriptor( etdnnConvolutionDescriptor_t    convDesc,
                                                     int                             pad_h,
                                                     int                             pad_w,
                                                     int                             u,
                                                     int                             v,
...
{
    etdnnStatus_t res = InterceptFunction<etdnnStatus_t>("cudnnSetConvolution2dDescriptor", etdnnSetConvolution2dDescriptor,
                                                         convDesc, pad_h, pad_w, u, v, dilation_h, dilation_w, mode, computeType);
    interceptor_log("cudnnSetConvolution2dDescriptor status:%d convDesc:%p pad_h:%d pad_w:%d u:%d v:%d dilation_h:%d dilation_w:%d mode:%d computeType:%d\n",
                    res, convDesc, pad_h, pad_w, u, v, dilation_h, dilation_w, mode, computeType);
    return res;
}

EXAPI etdnnStatus_t cudnnCreateActivationDescriptor( etdnnActivationDescriptor_t *activationDesc)
{
    etdnnStatus_t res = InterceptFunction<etdnnStatus_t>("cudnnCreateActivationDescriptor", etdnnCreateActivationDescriptor, activationDesc);
    interceptor_log("cudnnCreateActivationDescriptor status:%d activationDesc:%p\n", res, *activationDesc);
    return res;
}

EXAPI etdnnStatus_t cudnnDestroyActivationDescriptor( etdnnActivationDescriptor_t activationDesc)
{
    etdnnStatus_t res = InterceptFunction<etdnnStatus_t>("cudnnDestroyActivationDescriptor", etdnnDestroyActivationDescriptor, activationDesc);
    interceptor_log("cudnnDestroyActivationDescriptor status:%d activationDesc:%p\n", res, activationDesc);
    return res;
}

EXAPI etdnnStatus_t cudnnSetActivationDescriptor( etdnnActivationDescriptor_t         activationDesc,
                                                  etdnnActivationMode_t               mode,
                                                  etdnnNanPropagation_t               reluNanOpt,
                                                  double                              coef)
{
    etdnnStatus_t res = InterceptFunction<etdnnStatus_t>("cudnnSetActivationDescriptor", etdnnSetActivationDescriptor,
                                                         activationDesc, mode, reluNanOpt, coef);
    interceptor_log("cudnnSetActivationDescriptor status:%d activationDesc:%p mode:%d reluNanOpt:%d coef:%f\n", res, activationDesc, mode, reluNanOpt, coef);
    return res;
}

EXAPI etdnnStatus_t cudnnActivationForward( etdnnHandle_t handle,
                                            ...
                                            void                           *y)
{
    etdnnStatus_t res = InterceptFunction<etdnnStatus_t>("cudnnActivationForward", etdnnActivationForward,
                                                          handle, activationDesc, alpha, xDesc, x, beta, yDesc, y);
    interceptor_log("cudnnActivationForward status:%d handle:%p activationDesc:%p alpha:%p xDesc:%p x:%p beta:%p yDesc:%p y:%p\n",
                    res, handle, activationDesc, alpha, xDesc, x, beta, yDesc, y);
    return res;
}

EXAPI etdnnStatus_t cudnnCreatePoolingDescriptor( etdnnPoolingDescriptor_t *poolingDesc)
{
    etdnnStatus_t res = InterceptFunction<etdnnStatus_t>("cudnnCreatePoolingDescriptor", etdnnCreatePoolingDescriptor, poolingDesc);
    interceptor_log("cudnnCreatePoolingDescriptor status:%d poolingDesc:%p\n", res, *poolingDesc);
    return res;
}

EXAPI etdnnStatus_t cudnnDestroyPoolingDescriptor( etdnnPoolingDescriptor_t poolingDesc)
{
    etdnnStatus_t res = InterceptFunction<etdnnStatus_t>("cudnnDestroyPoolingDescriptor", etdnnDestroyPoolingDescriptor, poolingDesc);
    interceptor_log("cudnnDestroyPoolingDescriptor status:%d poolingDesc:%p\n", res, poolingDesc);
    return res;
}

EXAPI etdnnStatus_t cudnnSetPooling2dDescriptor( etdnnPoolingDescriptor_t    poolingDesc,
...
{
    etdnnStatus_t res = InterceptFunction<etdnnStatus_t>("cudnnSetPooling2dDescriptor", etdnnSetPooling2dDescriptor,
    ...
    interceptor_log("cudnnSetPooling2dDescriptor status:%d poolingDesc:%p mode:%d maxpoolingNanOpt:%d windowHeight:%d windowWidth:%d verticalPadding:%d horizontalPadding:%d verticalStride:%d horizontalStride:%d\n",
                    res, poolingDesc, mode, maxpoolingNanOpt, windowHeight, windowWidth, verticalPadding, horizontalPadding, verticalStride, horizontalStride);
    return res;
}

EXAPI etdnnStatus_t cudnnPoolingForward( etdnnHandle_t                    handle,
                                         ...
                                         void                            *y)
{
    etdnnStatus_t res = InterceptFunction<etdnnStatus_t>("cudnnPoolingForward", etdnnPoolingForward,
                                                          handle, poolingDesc, alpha, xDesc, x, beta, yDesc, y);
    interceptor_log("cudnnPoolingForward status:%d handle:%p poolingDesc:%p alpha:%p xDesc:%p x:%p beta:%p yDesc:%p y:%p\n",
                    res, handle, poolingDesc, alpha, xDesc, x, beta, yDesc, y);
    return res;
}

EXAPI etdnnStatus_t cudnnAddTensor( etdnnHandle_t                    handle,
                                    const void                       *alpha,
                                    ...
                                    void                             *C)
{
    etdnnStatus_t res = InterceptFunction<etdnnStatus_t>("cudnnAddTensor", etdnnAddTensor,
                                                          handle, alpha, aDesc, A, beta, cDesc, C);
    interceptor_log("cudnnAddTensor status:%d handle:%p alpha:%p aDesc:%p A:%p beta:%p cDesc:%p C:%p\n",
 ...
 */
EXAPI etdnnStatus_t cudnnSetTensorNdDescriptor( etdnnTensorDescriptor_t tensorDesc,
                                                etdnnDataType_t         dataType,
                                                int                     nbDims,
                                                const int               *dimA,
                                                const int               *strideA)
{
    etdnnStatus_t res = InterceptFunction<etdnnStatus_t>("cudnnSetTensorNdDescriptor", etdnnSetTensorNdDescriptor,
                                                         tensorDesc, dataType, nbDims, dimA, strideA);
    interceptor_log( "cudnnSetTensorNdDescriptor status:%d tensorDesc:%p dataType:%d nbDims:%d\n", res, tensorDesc, dataType, nbDims);
    return res;
}
 ...
 */
EXAPI etdnnStatus_t cudnnSetTensorNdDescriptorEx( etdnnTensorDescriptor_t tensorDesc,
                                                  etdnnTensorFormat_t     format,
                                                  etdnnDataType_t         dataType,
                                                  int                     nbDims,
                                                  const int               *dimA)
{
    etdnnStatus_t res = InterceptFunction<etdnnStatus_t>("cudnnSetTensorNdDescriptorEx", etdnnSetTensorNdDescriptorEx,
                                                         tensorDesc, format, dataType, nbDims, dimA);
    interceptor_log( "cudnnSetTensorNdDescriptorEx status:%d tensorDesc:%p format:%d dataType:%d nbDims:%d\n", res, tensorDesc, format, dataType, nbDims);
    return res;
}

EXAPI etdnnStatus_t cudnnDeriveBNTensorDescriptor( etdnnTensorDescriptor_t         derivedBnDesc,
                                                   const etdnnTensorDescriptor_t   xDesc,
                                                   etdnnBatchNormMode_t            mode)
{
    etdnnStatus_t res = InterceptFunction<etdnnStatus_t>("cudnnDeriveBNTensorDescriptor", etdnnDeriveBNTensorDescriptor,
                                                         derivedBnDesc, xDesc, mode);
    interceptor_log( "cudnnDeriveBNTensorDescriptor status:%d derivedBnDesc:%p xDesc:%p mode:%d\n", res, derivedBnDesc, xDesc, mode);
    return res;
}

EXAPI etdnnStatus_t cudnnBatchNormalizationForwardInference( etdnnHandle_t                    handle,
                                                             etdnnBatchNormMode_t             mode,
                                                             const void                      *alpha,
                                                             const void                      *beta,
                                                             const etdnnTensorDescriptor_t    xDesc,
                                                             ...
                                                             double                           epsilon)
{
    etdnnStatus_t res = InterceptFunction<etdnnStatus_t>("cudnnBatchNormalizationForwardInference", etdnnBatchNormalizationForwardInference,
                                                         handle, mode, alpha, beta, xDesc, x, yDesc, y,
                                                         bnScaleBiasMeanVarDesc, bnScale, bnBias,
                                                         estimatedMean, estimatedVariance, epsilon);
    interceptor_log("cudnnBatchNormalizationForwardInference status:%d handle:%p mode:%d alpha:%p beta:%p xDesc:%p x:%p yDesc:%p y:%p"
                    " bnScaleBiasMeanVarDesc:%p bnScale:%p bnBias:%p estimatedMean:%p estimatedVariance:%p epsilon:%f\n",
                    res, handle, mode, alpha, beta, xDesc, x, yDesc, y, bnScaleBiasMeanVarDesc, bnScale, bnBias, estimatedMean, estimatedVariance, epsilon);
    return res;
}

EXAPI etdnnStatus_t cudnnGetConvolutionForwardAlgorithm( etdnnHandle_t                      handle,
                                                         const etdnnTensorDescriptor_t      xDesc,
...
{
    etdnnStatus_t res = InterceptFunction<etdnnStatus_t>("cudnnGetConvolutionForwardAlgorithm", etdnnGetConvolutionForwardAlgorithm,
                                                         handle, xDesc, wDesc, convDesc, yDesc, preference, memoryLimitInBytes, algo);
    interceptor_log("cudnnGetConvolutionForwardAlgorithm status:%d handle:%p xDesc:%p wDesc:%p convDesc:%p yDesc:%p preference:%d memoryLimitInBytes:%ld algo:%d\n",
                    res, handle, xDesc, wDesc, convDesc, yDesc, preference, memoryLimitInBytes, *algo);
    return res;
}


EXAPI etdnnStatus_t cudnnGetConvolutionForwardWorkspaceSize( etdnnHandle_t                           handle,
                                                             const   etdnnTensorDescriptor_t         xDesc,
                                                             const   etdnnFilterDescriptor_t         wDesc,
                                                             const   etdnnConvolutionDescriptor_t    convDesc,
                                                             const   etdnnTensorDescriptor_t         yDesc,
                                                             ...
                                                             size_t                                 *sizeInBytes)
{
    etdnnStatus_t res = InterceptFunction<etdnnStatus_t>("cudnnGetConvolutionForwardWorkspaceSize", etdnnGetConvolutionForwardWorkspaceSize,
                                                         handle, xDesc, wDesc, convDesc, yDesc, algo, sizeInBytes);
    interceptor_log("cudnnGetConvolutionForwardWorkspaceSize status:%d handle:%p xDesc:%p wDesc:%p convDesc:%p yDesc:%p algo:%d sizeInBytes:%ld\n",
                    res, handle, xDesc, wDesc, convDesc, yDesc, algo, *sizeInBytes);
    return res;
}

EXAPI etdnnStatus_t cudnnConvolutionForward( etdnnHandle_t                       handle,
                                             const void                         *alpha,
                                             const etdnnTensorDescriptor_t       xDesc,
                                             const void                         *x,
                                             const etdnnFilterDescriptor_t       wDesc,
                                             ...
                                             void                               *y)
{
    etdnnStatus_t res = InterceptFunction<etdnnStatus_t>("cudnnConvolutionForward", etdnnConvolutionForward,
                                                         handle, alpha, xDesc, x, wDesc, w, convDesc, algo,
                                                         workSpace, workSpaceSizeInBytes, beta, yDesc, y);
    interceptor_log("cudnnConvolutionForward status:%d handle:%p alpha:%p xDesc:%p x:%p wDesc:%p w:%p convDesc:%p algo:%d workSpace:%p workSpaceSizeInBytes:%ld beta:%p yDesc:%p y:%p\n",
                    res, handle, alpha, xDesc, x, wDesc, w, convDesc, algo, workSpace, workSpaceSizeInBytes, beta, yDesc, y);
    return res;
}

EXAPI etdnnStatus_t cudnnSoftmaxForward( etdnnHandle_t                    handle,
                                         ...
                                         void                            *y)
{
    etdnnStatus_t res = InterceptFunction<etdnnStatus_t>( "cudnnSoftmaxForward", etdnnSoftmaxForward,
                                                          handle, algorithm, mode, alpha, xDesc, x, beta, yDesc, y);
    interceptor_log("cudnnSoftmaxForward status = %d handle = %p algorithm = %d, mode = %d, alpha = %p, xDesc = %p, x = %p, beta = %p, yDesc = %p, y = %p\n",
                    res, handle, algorithm, mode, alpha, xDesc, x, beta, yDesc, y);
    return res;
}

EXAPI etdnnStatus_t cudnnSetConvolutionGroupCount( etdnnConvolutionDescriptor_t convDesc,
                                                   int groupCount)
{
    etdnnStatus_t res = InterceptFunction<etdnnStatus_t>( "cudnnSetConvolutionGroupCount", etdnnSetConvolutionGroupCount,
                                                          convDesc, groupCount);
    interceptor_log("cudnnSetConvolutionGroupCount status = %d convDesc = %p gc = %d\n", res, convDesc, groupCount);
    return res;
}
