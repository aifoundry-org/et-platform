...
public:
    ...
    virtual void execute(CardProxy *card_proxy) { THROW("Unexpected EtAction::execute()"); }
...
public:
    ...
    void observerWait();
    bool isExecuted() { std::lock_guard<std::mutex> lk(observer_mutex); return executed; }
};
...
public:
    EtActionEventWaiter(EtActionEvent* event):event_to_wait_(event) { assert( event_to_wait_ != nullptr ); event_to_wait_->incRefCounter(); }
...
public:
    EtActionConfigure(const void *devMemRegionPtr, size_t devMemRegionSize,
                      const void *kernelsDevMemRegionPtr, size_t kernelsDevMemRegionSize)
            : devMemRegionPtr(devMemRegionPtr), devMemRegionSize(devMemRegionSize)
            , kernelsDevMemRegionPtr(kernelsDevMemRegionPtr), kernelsDevMemRegionSize(kernelsDevMemRegionSize) {}
    virtual void execute(CardProxy *card_proxy);
...
public:
    EtActionLaunch(dim3 gridDim, dim3 blockDim, const std::vector<uint8_t> &args_buff, uintptr_t kernel_pc, const std::string &kernel_name)
            : gridDim(gridDim), blockDim(blockDim), args_buff(args_buff), kernel_pc(kernel_pc), kernel_name(kernel_name) {}
    virtual void execute(CardProxy *card_proxy);
};
...
class EtStream
{
    bool is_blocking_;
  public:
    EtStream(bool is_blocking):is_blocking_(is_blocking) {
        init();
    }
