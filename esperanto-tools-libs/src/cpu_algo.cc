 ...
 * C = α * op(A) * op(B) + β * C
 *
 * A, B, C - in column-major format (as in Fortran, i.e. first index (=index of row) is dense)
 *
 ...
 * C     - m x n
 *
 * ld[abc] - leading dimension of a two-dimensional array used to store the matrix [ABC]
 * ld[abc] may be bigger than corresponding matrix dimension, it could be useful in cases
 * when matrix rows/columns lies in memory in non-consecutive manner (sub-matrix, for example)
 */
...
void cpuMatMulTensor(MatMulTensor_KernelArgs_t* args_p)
{
    for (int i = 0; i < args_p->m; i++) {
        for (int j = 0; j < args_p->n; j++) {
            ...
            float sum = 0.0f;
            for (int l = 0; l < args_p->k; l++) {
                sum += matElRM(args_p->A, args_p->lda, args_p->is_transa, i, l) * matElRM(args_p->B, args_p->ldb, args_p->is_transb, l, j);
            }
            cEl = sum;
        }
    }
...
void cpuQuantizedMatMul(QuantizedMatMul_KernelArgs_t* args_p)
{
    for (int i = 0; i < args_p->m; i++) {
        for (int j = 0; j < args_p->n; j++) {
            ...
            cEl = quantization::clip<int32_t, int8_t>(std::round(args_p->invScale * sum + args_p->offsetc));
        }
    }
}
            ...
            for ( int h = 0 ; h < args_p->out_h ; h++ )
            {
                for ( int w = 0 ; w < args_p->out_w ; w++ )
                {
                    ...
                    dest[out_idx] = quantization::clip<int32_t, int8_t>(
                        std::round(float(conv_sum) * (matMulScale / args_p->outScale) + args_p->outOffset));
                }
            }
        }
    }
        ...
        for ( int c = 0 ; c < args_p->filter_k ; c++ )
        {
            ...
            for ( int h = 0 ; h < args_p->out_h ; h++ )
            {
                for ( int w = 0 ; w < args_p->out_w ; w++ )
                {
                    ...
                    {
                        // Scale the bias to match the scale of the matrix multiplication.
                        int32_t B = std::round(float(((int32_t*)args_p->b)[c] - args_p->biasOffset) *
                                               (args_p->biasScale / matMulScale));
                        conv_sum += B;
                    }
...
void cpuBatchNormalizationForwardInferenceFloat(BatchNormalizationForwardInference_KernelArgs_t* args_p)
{
#define ELEM_IN(_n, _c, _h, _w) tensor4dEl((const float*)args_p->x, _n, _c, _h, _w, args_p->in_nStride, args_p->in_cStride, args_p->in_hStride, args_p->in_wStride)
#define ELEM_SCALE(_n, _c, _h, _w) tensor4dEl((const float*)args_p->bnScale, _n, _c, _h, _w, args_p->bn_nStride, args_p->bn_cStride, args_p->bn_hStride, args_p->bn_wStride)
#define ELEM_BIAS(_n, _c, _h, _w) tensor4dEl((const float*)args_p->bnBias, _n, _c, _h, _w, args_p->bn_nStride, args_p->bn_cStride, args_p->bn_hStride, args_p->bn_wStride)
#define ELEM_MEAN(_n, _c, _h, _w) tensor4dEl((const float*)args_p->estimatedMean, _n, _c, _h, _w, args_p->bn_nStride, args_p->bn_cStride, args_p->bn_hStride, args_p->bn_wStride)
#define ELEM_VAR(_n, _c, _h, _w) tensor4dEl((const float*)args_p->estimatedVariance, _n, _c, _h, _w, args_p->bn_nStride, args_p->bn_cStride, args_p->bn_hStride, args_p->bn_wStride)
#define ELEM_OUT(_n, _c, _h, _w) tensor4dEl((float*)args_p->y, _n, _c, _h, _w, args_p->out_nStride, args_p->out_cStride, args_p->out_hStride, args_p->out_wStride)
        ...
        for ( int c = 0 ; c < args_p->c ; c++ )
        {
            ...
            for ( int h = 0 ; h < args_p->h ; h++ )
            {
                ...
                for ( int w = 0 ; w < args_p->w ; w++ )
                {
                    ...
                    float result = (scale * (in_elem - mean) / sqrtf(args_p->epsilon + var) + bias);
...
/**
 * y = α * (bnScale * (x - estimatedMean) / sqrt(epsilon + estimatedVariance) + bnBias) + β * y
 */
    ...
    for ( int n = 0 ; n < args_p->out_n ; n++ )
    {
        for ( int c = 0 ; c < args_p->out_c ; c++ )
        {
            ...
            for ( int h = 0 ; h < args_p->out_h ; h++,y+=args_p->vertical_stride )
            {
                ...
                for ( int w = 0 ; w < args_p->out_w ; w++,x+=args_p->horizontal_stride )
                {
...
{
#define ELEM_IN( _d0, _d1, _d2, _d3) tensor4dEl((const T*)args_p->x, _d0, _d1, _d2, _d3, args_p->x_strides[0], args_p->x_strides[1], args_p->x_strides[2], args_p->x_strides[3])
#define ELEM_OUT( _d0, _d1, _d2, _d3) tensor4dEl((T*)args_p->y, _d0, _d1, _d2, _d3, args_p->y_strides[0], args_p->y_strides[1], args_p->y_strides[2], args_p->y_strides[3])
...
void cpuQuantizedBatchedAdd(QuantizedBatchedAdd_KernelArgs_t* args_p)
{
    ...
    for ( int b = 0; b < args_p->n; b++ )
    {
        for ( int s = 0; s < args_p->slice_size; s++,i++ )
        {
            float batchVal = quantization::dequantize(args_p->batch[i], args_p->batchScale, args_p->batchOffset);
            float sliceVal = quantization::dequantize(args_p->slice[s], args_p->sliceScale, args_p->sliceOffset);
            float destVal = batchVal + sliceVal;
            args_p->dest[i] = quantization::quantize(destVal, args_p->destScale, args_p->destOffset);
        }
    }
}
...
#define ELEM_IN(_n, _c, _h, _w) tensor4dEl((const float*)args_p->x, _n, _c, _h, _w, args_p->in_nStride, args_p->in_cStride, args_p->in_hStride, args_p->in_wStride)
#define ELEM_OUT(_n, _c, _h, _w) tensor4dEl((float*)args_p->y, _n, _c, _h, _w, args_p->out_nStride, args_p->out_cStride, args_p->out_hStride, args_p->out_wStride)
#define ELEM_SCALE(_n, _c, _h, _w) tensor4dEl((float*)args_p->scale, _n, _c, _h, _w, args_p->scale_nStride, args_p->scale_cStride, args_p->scale_hStride, args_p->scale_wStride)
...
void cpuGather( Gather_KernelArgs_t * args_p)
{
    ...
    {        
        memcpy( dest_p + slice_size * i, data_p + args_p->indices[i] * slice_size, slice_size);
    }
}
...
void cpuQuantizedArithOp(QuantizedArithOp_KernelArgs_t *args_p)
{
    ...
    {
        float aVal = quantization::dequantize(args_p->a[i], args_p->aScale, args_p->aOffset);
        float bVal = quantization::dequantize(args_p->b[i], args_p->bScale, args_p->bOffset);
        float yVal;
        switch (args_p->op)
        {
            ...
            case ETDNN_TENSOR_ARITH_OP_MAX: yVal = std::max(aVal, bVal); break;
            case ETDNN_TENSOR_ARITH_OP_MIN: yVal = std::min(aVal, bVal); break;
            default:
                abort();
        }
        args_p->y[i] = quantization::quantize(yVal, args_p->yScale, args_p->yOffset);
    }
}
