...
{
    void *host_base = mmap(nullptr, kHostMemRegionSize, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    ...
    void *dev_base = mmap(kDevMemBaseAddr, kDevMemRegionSize, PROT_NONE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    ...
    void *kernels_dev_base = mmap(kKernelsDevMemBaseAddr, kKernelsDevMemRegionSize, PROT_NONE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    PERROR_IF(kernels_dev_base == MAP_FAILED);
    THROW_IF(kernels_dev_base != kKernelsDevMemBaseAddr, "Cannot allocate kernels dev memory.");
    kernels_dev_mem_region.reset( new EtMemRegion(kernels_dev_base, kKernelsDevMemRegionSize));
    ...
    EtActionEvent *actionEvent = nullptr;
    {
        ...
        actionConfigure = new EtActionConfigure(dev_base, kDevMemRegionSize, kernels_dev_base, kKernelsDevMemRegionSize);
    ...
    if (actionConfigure->isLocalMode()) {
        PERROR_IF(mprotect(dev_base, kDevMemRegionSize, PROT_READ | PROT_WRITE) == -1);
        PERROR_IF(mprotect(kernels_dev_base, kKernelsDevMemRegionSize, PROT_READ | PROT_WRITE) == -1);
    }
