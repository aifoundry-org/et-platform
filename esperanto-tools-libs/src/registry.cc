...
static constexpr size_t UPPER_BOUND_FOR_KERNELS_ELF_SIZE =
    1024 * 1024 * 1024; // 1GB
...
/*
 * Note: __etrtRegisterFunction is called during DSO linking (before our global
 * variables constructors), so in __etrtRegisterFunction we may only store data
 * to statically initialized data types (POD).
 */
static constexpr size_t kRegisteredFatbinWrappersMax = 1024;
static const __fatBinC_Wrapper_t
    *gRegisteredFatbinWrappers[kRegisteredFatbinWrappersMax];
 ...
 * Then they are stay unmodified, so after lazy_init_once() we access to them
 * without synchronization.
 */
static std::vector<std::unique_ptr<EtKernelsElfDescr>> gElfDescrStorage;
static std::unordered_map<const void *, EtKernelHostFunDescr>
    gRegisteredFunctions;

static void lazy_init_once() {
  std::unordered_map<const __fatBinC_Wrapper_t *, const EtKernelsElfDescr *>
      wrapper_2_elf_descr;
  ...
  for (size_t i = 0; i < gRegisteredFatbinWrappersNum; i++) {
    ...
    if ((uint32_t)*fatbin_wrapper_p->data == ELF_MAGIC) {
      ...
      parse_elf(elf_descr->elf_p, &elf_descr->elf_size,
                &elf_descr->kernel_offset, nullptr);
  ...
  for (size_t i = 0; i < gRegisteredFunctionsNum; i++) {
    ...
    const __fatBinC_Wrapper_t *fatbin_wrapper_p =
        gRegisteredFunctionsFatbinWrapper[i];
...
void parse_elf(const void *elf_p, size_t *elf_size_p,
  ...
  struct readmembuf : std::streambuf {
    readmembuf(char *base, size_t n) { this->setg(base, base, base + n); }
  ...
  // Sanity checks.
  THROW_IF(reader.get_machine() != EM_RISCV,
           "Kernels ELF machine is not EM_RISCV.");
  THROW_IF(reader.get_class() != ELFCLASS64,
   ...
   */
  size_t file_size = reader.get_sections_offset() +
      ...
      for (Elf_Xword sym_idx = 0; sym_idx < sym_num; ++sym_idx) {
        ...
        if (type == STT_FUNC && bind == STB_GLOBAL) {
          ...
          if (!kernel_name.empty()) {
            fprintf(stderr,
                    "Esperanto kernel: offset = 0x%lx, sym name = %s, kernel "
                    "name = %s [%s]\n",
                    (size_t)value, name.c_str(), kernel_name.c_str(),
                    demangle(kernel_name).c_str());
            if (kernel_offset_p) {
              assert(value);
