...
void TestDevMgmtApiSyncCmds::setTraceConfigure(bool singleDevice) {
  getDM_t dmi = getInstance();
  ASSERT_TRUE(dmi);
  DeviceManagement& dm = (*dmi)(devLayer_.get());
  ...
  for (int deviceIdx = 0; deviceIdx < deviceCount; deviceIdx++) {
    ...
    ASSERT_EQ(dm.serviceRequest(deviceIdx, device_mgmt_api::DM_CMD::DM_CMD_SET_DM_TRACE_CONFIG, input_buff, input_size,
