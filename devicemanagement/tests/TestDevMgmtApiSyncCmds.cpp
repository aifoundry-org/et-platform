...
void TestDevMgmtApiSyncCmds::initDevErrorEvent() {
  getDM_t dmi = getInstance();
  ASSERT_TRUE(dmi);
  DeviceManagement& dm = (*dmi)(devLayer_.get());
...
void TestDevMgmtApiSyncCmds::checkDevErrorEvent(std::vector<std::string> list, bool isCheckList) {
  if (isParallelRun()) {
  ...
  for (const auto& [deviceIdx, eventsAtStart] : eventsAtStartMap_) {
    auto eventsNow = DevErrorEvent(devLayer_->getDeviceAttribute(deviceIdx, "err_stats/ce_count") +
                                   devLayer_->getDeviceAttribute(deviceIdx, "err_stats/uce_count"));
    EXPECT_FALSE((eventsNow - eventsAtStart).hasFailure(list, isCheckList))
