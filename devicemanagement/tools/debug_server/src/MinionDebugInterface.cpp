...
MinionDebugInterface::MinionDebugInterface(uint8_t device_idx, uint64_t shire_id, uint64_t thread_mask,
                                           uint64_t bp_timeout) {
...
int MinionDebugInterface::invokeDmServiceRequest(uint8_t code, const char* input_buff, const uint32_t input_size,
  ...
  retval = dm.serviceRequest(0, code, input_buff, input_size, output_buff, output_size, hst_latency.get(),
                             dev_latency.get(), 70000);
...
void MinionDebugInterface::stall() {
  ...
  DV_LOG(INFO) << "MDI::stall.."
               << "Shire ID:" << this->shire_id << "Thread Mask:" << std::hex << this->thread_mask << std::endl;
...
void MinionDebugInterface::step() {
  ...
  auto retval = invokeDmServiceRequest(DM_CMD_MDI_ENABLE_SINGLE_STEP, (char*)&mdi_cmd, sizeof(struct mdi_bp_control_t),
...
void MinionDebugInterface::insertBreakpoint(uint64_t addr) {
  ...
  DV_LOG(INFO) << "MDI::insertBreakpoint Hart ID:" << mdi_cmd.hart_id << "BP addr:" << std::hex << mdi_cmd.bp_address
               << std::endl;

  auto retval = invokeDmServiceRequest(DM_CMD_MDI_SET_BREAKPOINT, (char*)&mdi_cmd, sizeof(struct mdi_bp_control_t),
                                       (char*)&status, sizeof(uint32_t));
...
void MinionDebugInterface::removeBreakpoint(uint64_t addr) {
  ...
  DV_LOG(INFO) << "MDI::removeBreakpoint Hart ID:" << mdi_cmd.hart_id << "BP addr:" << std::hex << mdi_cmd.bp_address
               << std::endl;

  auto retval = invokeDmServiceRequest(DM_CMD_MDI_UNSET_BREAKPOINT, (char*)&mdi_cmd, sizeof(struct mdi_bp_control_t),
                                       (char*)&status, sizeof(uint32_t));
...
uint64_t MinionDebugInterface::selectHart(std::uint64_t shire_id, std::uint64_t thread_mask) {
  ...
  DV_LOG(INFO) << "MDI::selectHart ->"
               << "Shire ID: " << mdi_cmd.shire_id << "Thread Mask: " << std::hex << mdi_cmd.thread_mask << std::endl;

  auto retval = invokeDmServiceRequest(DM_CMD_MDI_SELECT_HART, (char*)&mdi_cmd, sizeof(struct mdi_hart_selection_t),
...
uint64_t MinionDebugInterface::unselectHart(std::uint64_t shire_id, std::uint64_t thread_mask) {
  ...
  DV_LOG(INFO) << "MDI::unselectHart ->"
               << "Shire ID: " << mdi_cmd.shire_id << "Thread Mask: " << std::hex << mdi_cmd.thread_mask << std::endl;

  auto retval = invokeDmServiceRequest(DM_CMD_MDI_UNSELECT_HART, (char*)&mdi_cmd, sizeof(struct mdi_hart_selection_t),
                                       (char*)&output_buff, sizeof(uint64_t));
...
void MinionDebugInterface::haltHart() {
  ...
  auto retval = invokeDmServiceRequest(DM_CMD_MDI_HALT_HART, (char*)&mdi_cmd, sizeof(struct mdi_hart_control_t),
...
void MinionDebugInterface::resumeHart() {
  ...
  auto retval = invokeDmServiceRequest(DM_CMD_MDI_RESUME_HART, (char*)&mdi_cmd, sizeof(struct mdi_hart_control_t),
                                       (char*)&output_buff, sizeof(uint32_t));
...
uint32_t MinionDebugInterface::hartStatus() {
  ...
  auto retval = invokeDmServiceRequest(DM_CMD_MDI_GET_HART_STATUS, (char*)&mdi_cmd, sizeof(struct mdi_hart_control_t),
...
uint64_t MinionDebugInterface::readReg(std::uint32_t num) {
  ...
  if (num < RISCV_PC_INDEX) {
    ...
    DV_LOG(INFO) << "Read GPR "
                 << "Hart ID: " << mdi_gpr_read.hart_id << " GPR Index: " << mdi_gpr_read.gpr_index << std::endl;

    auto retval = invokeDmServiceRequest(DM_CMD_MDI_READ_GPR, (char*)&mdi_gpr_read, sizeof(struct mdi_gpr_read_t),
    ...
    DV_LOG(INFO) << "Read CSR "
                 << "Hart ID: " << mdi_csr_read.hart_id << " CSR Index: " << mdi_csr_read.csr_name << std::endl;

    auto retval = invokeDmServiceRequest(DM_CMD_MDI_READ_CSR, (char*)&mdi_csr_read, sizeof(struct mdi_csr_read_t),
                                         (char*)&regval, sizeof(uint64_t));
...
void MinionDebugInterface::writeReg(std::uint32_t num, uint64_t value) {
  ...
  if (num < RISCV_PC_INDEX) {
    ...
    DV_LOG(INFO) << "MDI:: writeReg GPR ->"
                 << " Reg num: " << num << "value: " << std::hex << value << std::endl;
    ...
    auto retval = invokeDmServiceRequest(DM_CMD_MDI_WRITE_GPR, (char*)&mdi_gpr_write, sizeof(struct mdi_gpr_write_t),
                                         (char*)&dummy, sizeof(uint64_t));
  } else {

    DV_LOG(INFO) << "MDI:: writeReg CSR ->"
                 << " Reg num: " << num << "value: " << std::hex << value << std::endl;
    ...
    auto retval = invokeDmServiceRequest(DM_CMD_MDI_WRITE_CSR, (char*)&mdi_csr_write, sizeof(struct mdi_csr_write_t),
                                         (char*)&dummy, sizeof(uint64_t));
  }
}
...
int MinionDebugInterface::readMem(uint8_t* out, uint64_t addr, std::uint32_t len) {
  ...
  bool validAttr = getAttributesForAddr(addr, &access_initiator, &access_type);
  if (validAttr) {
    ...
    while (len >= MDI_MEM_READ_LENGTH_BYTES_8) {
      ...
      DV_LOG(INFO) << "8 byte read ->"
                   << " ADDRESS: " << std::hex << mdi_cmd_req.address << " LENGTH: " << std::hex << mdi_cmd_req.size
                   << std::endl;

      auto retval = invokeDmServiceRequest(DM_CMD_MDI_READ_MEM, (char*)&mdi_cmd_req, sizeof(mdi_cmd_req), (char*)out,
                                           mdi_cmd_req.size);
...
int MinionDebugInterface::writeMem(uint8_t* src, uint64_t addr, std::uint32_t len) {
  ...
  DV_LOG(INFO) << "MDI::writeMem"
               << " address: 0x" << std::hex << mdi_cmd.address << " len: " << std::hex << len << std::endl;

  /* If the addresses are 64-bit aligned */
  if (!((mdi_cmd.address) & 0x7)) {
    while (len >= MDI_MEM_WRITE_LENGTH_BYTES_8) {

      DV_LOG(INFO) << "64 bit aligned"
                   << " address: 0x" << std::hex << mdi_cmd.address << " len: " << std::hex << len << std::endl;
      memcpy((char*)&mdi_cmd.data, src, sizeof(uint64_t));
      mdi_cmd.size = sizeof(uint64_t);
      auto retval = invokeDmServiceRequest(DM_CMD_MDI_WRITE_MEM, (char*)&mdi_cmd, sizeof(mdi_cmd), (char*)&status,
                                           sizeof(uint64_t));
  ...
  /* If the addresses are 16-bit aligned */
    ...
    while (len >= MDI_MEM_WRITE_LENGTH_BYTES_2) {
      ...
      DV_LOG(INFO) << "16 bit aligned writeMem"
                   << " address: 0x" << std::hex << mdi_cmd.address << " len: " << std::hex << len << std::endl;
  ...
  while (len) {
    ...
    DV_LOG(INFO) << "byte aligned"
                 << " address: 0x" << std::hex << mdi_cmd.address << " len: " << std::hex << len << std::endl;
    memcpy((char*)&mdi_cmd.data, src, sizeof(uint8_t));
    mdi_cmd.size = sizeof(uint8_t);
    auto retval =
      invokeDmServiceRequest(DM_CMD_MDI_WRITE_MEM, (char*)&mdi_cmd, sizeof(mdi_cmd), (char*)&status, sizeof(uint64_t));
    DV_LOG(INFO) << "DM retval:" << std::hex << retval << " status: " << std::hex << status << std::endl;
