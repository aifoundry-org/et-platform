        ...
        cm_iface_message_t *const message)
{
    ...
    sw_timer_idx = SW_Timer_Create_Timeout(
        &mm_to_cm_iface_multicast_timeout_cb, 0, 
        TIMEOUT_MM_CM_MSG);
    ...
    else
    {
        ...
        asm volatile("fence");
        evict(to_L3,mm_to_cm_broadcast_message_buffer_ptr,
            sizeof(*message));
        WAIT_CACHEOPS;

        /* Configure broadcast message control data */
        atomic_store_global_32
            (&mm_to_cm_broadcast_message_ctrl_ptr->count, 0);

        /* Send IPI to receivers. Upper 32 Threads of Shire 32 also 
        run Worker FW */
        broadcast_ipi_trigger
            (dest_shire_mask & 0xFFFFFFFFu, 0xFFFFFFFFFFFFFFFFu);
        if (dest_shire_mask & (1ULL << MASTER_SHIRE))
            syscall(SYSCALL_IPI_TRIGGER_INT, 0xFFFFFFFF00000000u, 
            MASTER_SHIRE, 0);

        shire_count = (uint32_t)__builtin_popcountll(dest_shire_mask);

        /* Wait until all the receiver Shires have ACK'd, 1 per Shire.
        Then it's safe to send another broadcast message. Also, wait 
        ...
        do 
        {
            ...
            /* Read the global timeout flag to see for 
            MM->CM message timeout */
            timeout_flag = atomic_compare_and_exchange_local_32
                (&MM_CM_CB.timeout_flag, 1, 0);

        } while ((atomic_load_global_32
            (&mm_to_cm_broadcast_message_ctrl_ptr->count) != 
            shire_count) && (timeout_flag == 0));
