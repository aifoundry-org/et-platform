...
static size_t ring_buffer_avail_space(struct buffer_header_t *rbuffer,
                                        size_t buffer_size)
{
    return ((rbuffer->head < rbuffer->tail) ? rbuffer->tail - rbuffer->head :
...
static trace_status_e ring_buffer_check_space(struct buffer_header_t *rbuffer,
                                         size_t buffer_size, size_t event_size)
{
...
void *ring_buffer_alloc_space(uint16_t hartid, size_t size)
{
    ...
    struct buffer_header_t *rbuffer_header =
        (struct buffer_header_t *)(ALIGN(DEVICE_MRT_TRACE_BASE +
            sizeof(struct trace_control_t) + trace_ctrl->buffer_size * hartid,
            TRACE_BUFFER_REGION_ALIGNEMNT));
    trace_status_e status = ring_buffer_check_space(rbuffer_header,
        DEVICE_MRT_BUFFER_LENGTH(trace_ctrl->buffer_size), size);
    ...
    if (TRACE_STATUS_SUCCESS != status)
    {
        ...
        size_t avail_size = ring_buffer_avail_space(rbuffer_header,
                             DEVICE_MRT_BUFFER_LENGTH(trace_ctrl->buffer_size));
        ...
        if (size > (DEVICE_MRT_BUFFER_LENGTH(trace_ctrl->buffer_size) -
        ...
        {
            msg_hdr = (struct message_header_t *)
                ((void *)(rbuffer_header->buffer + rbuffer_header->tail));
