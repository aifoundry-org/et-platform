...
int64_t RINGBUFFER_write(volatile ringbuffer_t* restrict const ringbuffer_ptr, const void* restrict const buffer, uint32_t length)
{
...
int64_t RINGBUFFER_read(volatile ringbuffer_t* restrict const ringbuffer_ptr, void* restrict const buffer, uint32_t length)
{
...
uint64_t RINGBUFFER_free(volatile const ringbuffer_t* const ringbuffer_ptr)
{
    ...
    return (head_index >= tail_index) ? (RINGBUFFER_LENGTH - 1U) - (head_index - tail_index) : tail_index - head_index - 1U;
}
...
uint64_t RINGBUFFER_used(volatile const ringbuffer_t* const ringbuffer_ptr)
{
    ...
    return (head_index >= tail_index) ? head_index - tail_index : (RINGBUFFER_LENGTH + head_index - tail_index);
}
