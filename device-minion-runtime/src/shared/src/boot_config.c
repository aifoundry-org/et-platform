...
static const uint64_t MEM_SPACE_LUT[16] = {
    [BOOT_CONFIG_MEM_SPACE_RESERVED0]         = 0ULL,
    [BOOT_CONFIG_MEM_SPACE_R_SP_CRU]          = R_SP_CRU_BASEADDR,
    [BOOT_CONFIG_MEM_SPACE_R_PCIE_ESR]        = R_PCIE_ESR_BASEADDR,
    [BOOT_CONFIG_MEM_SPACE_R_PCIE_APB_SUBSYS] = R_PCIE_APB_SUBSYS_BASEADDR,
    [BOOT_CONFIG_MEM_SPACE_R_PCIE0_DBI_SLV]   = R_PCIE0_DBI_SLV_BASEADDR,
    [BOOT_CONFIG_MEM_SPACE_TEST]              = (uint64_t)&test_mem[0],
...
static volatile uint32_t* get_addr_and_mask(
    ...
    uint32_t * mask)
{
    if (cmd->dw0.B.memSpace <= BOOT_CONFIG_MEM_SPACE_RESERVED0 || cmd->dw0.B.memSpace >= BOOT_CONFIG_MEM_SPACE_EOL) {
        return NULL;
    }
    ...
    for (size_t i = 0; i < commands_count; ++i) {
        ...
        switch (cmd->dw0.B.opCode) {
            ...
            case CONFIG_OP_CODE_WRITE:
            {
                ...
                *regPtr = cmd->dw1.B.value & mask; //TODO: this does not prevent bad actor from setting bits to 0. Is this ok?
            }   
            break;
            ...
            case CONFIG_OP_CODE_POLL:
            {
                ...
                while (((*regPtr & mask) != cmd->dw1.B.value) && loopCounter) {
                    --loopCounter;
                }
...
static const uint32_t test_seq_invalid_mem_space_0[] = {
    ...
    0x1 << 28 | BOOT_CONFIG_MEM_SPACE_RESERVED0 << 24 | 0, //offset = 0
    0x00000000,                                       //writeVal
    0x00000000,                                       //mask
    //terminator
    0xFFFFFFFF,
...
static const uint32_t test_seq_invalid_mem_space_1[] = {
    //mem write an invalid mem space
    0x1 << 28 | BOOT_CONFIG_MEM_SPACE_EOL << 24 | 0,       //offset = 0
    0x00000000,                                       //writeVal
    0x00000000,                                       //mask
    //terminator
    0xFFFFFFFF,
...
static const uint32_t test_seq_valid_mem1[] = {
    ...
    0x1 << 28 | BOOT_CONFIG_MEM_SPACE_TEST << 24 | 4, //offset = 4
    0x00FF0000,                                  //writeVal
    0x00110000,                                  //mask
