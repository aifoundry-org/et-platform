...
#define M_CODE_REGION_BASE \
    0x8000000000ULL /* MCODE region is  2M from 0x8000000000 to 0x80001FFFFF */
#define M_CODE_REGION_SIZE 0x0000200000ULL

#define M_DATA_REGION_BASE \
    0x8000200000ULL /* MDATA region is  6M from 0x8000200000 to 0x80007FFFFF */
#define M_DATA_REGION_SIZE 0x0000600000ULL

#define S_CODE_REGION_BASE \
    0x8000800000ULL /* SCODE region is  8M from 0x8000800000 to 0x8000FFFFFF */
#define S_CODE_REGION_SIZE 0x0000800000ULL

#define S_DATA_REGION_BASE \
    0x8001000000ULL /* SDATA region is 48M from 0x8001000000 to 0x8003FFFFFF */
#define S_DATA_REGION_SIZE 0x0003000000ULL

#define U_CODE_REGION_BASE \
    0x8004000000ULL /* UCODE region is  4K from 0x8004000000 to 0x8004001000 */
...
#define FW_MMODE_STACK_SCRATCH_REGION_SIZE \
    64 /* Used by trap handler. 64B is the offset to distribute stack bases across memory controllers. */
#define FW_MMODE_STACK_SIZE \
    (1024 +                 \
     FW_MMODE_STACK_SCRATCH_REGION_SIZE) // Large enough for re-entrant M-mode traps, but small enough so all 2112 stacks + data fit in 6MB region
...
#define FW_MINION_FW_BOOT_CONFIG \
    CACHE_LINE_ALIGN(S_DATA_REGION_BASE + 0x2000000) /* SDATA_REGION_BASE + 32M */
#define FW_MINION_FW_BOOT_CONFIG_SIZE 64

#define FW_WORKER_TO_MASTER_MESSAGE_FLAGS \
    CACHE_LINE_ALIGN(FW_MINION_FW_BOOT_CONFIG + FW_MINION_FW_BOOT_CONFIG_SIZE)
#define FW_WORKER_TO_MASTER_MESSAGE_FLAGS_SIZE (NUM_SHIRES * MESSAGE_FLAG_SIZE)

#define FW_WORKER_TO_MASTER_MESSAGE_BUFFERS \
    CACHE_LINE_ALIGN(FW_WORKER_TO_MASTER_MESSAGE_FLAGS + FW_WORKER_TO_MASTER_MESSAGE_FLAGS_SIZE)
#define FW_WORKER_TO_MASTER_MESSAGE_BUFFERS_SIZE (NUM_HARTS * MESSAGE_BUFFER_SIZE)

#define FW_MASTER_TO_WORKER_MESSAGE_BUFFERS \
    CACHE_LINE_ALIGN(FW_WORKER_TO_MASTER_MESSAGE_BUFFERS + FW_WORKER_TO_MASTER_MESSAGE_BUFFERS_SIZE)
#define FW_MASTER_TO_WORKER_MESSAGE_BUFFERS_SIZE (NUM_HARTS * MESSAGE_BUFFER_SIZE)

#define FW_MASTER_TO_WORKER_BROADCAST_MESSAGE_BUFFER \
    CACHE_LINE_ALIGN(FW_MASTER_TO_WORKER_MESSAGE_BUFFERS + FW_MASTER_TO_WORKER_MESSAGE_BUFFERS_SIZE)
#define FW_MASTER_TO_WORKER_BROADCAST_MESSAGE_BUFFER_SIZE MESSAGE_BUFFER_SIZE

#define FW_MASTER_TO_WORKER_BROADCAST_MESSAGE_CTRL \
    CACHE_LINE_ALIGN(FW_MASTER_TO_WORKER_BROADCAST_MESSAGE_BUFFER + FW_MASTER_TO_WORKER_BROADCAST_MESSAGE_BUFFER_SIZE)
...
#define FW_SMODE_STACK_SCRATCH_REGION_SIZE \
    64 /* Used by trap handler. 64B is the offset to distribute stack bases across memory controllers. */
#define FW_SMODE_STACK_SIZE \
    (4096 + FW_SMODE_STACK_SCRATCH_REGION_SIZE) /* (4K + 64B) stack * 2112 stacks = 8580KB */

#ifndef __ASSEMBLER__
// Ensure the shared message buffers don't overlap with the S-stacks
static_assert((FW_MASTER_TO_WORKER_BROADCAST_MESSAGE_CTRL +
               FW_MASTER_TO_WORKER_BROADCAST_MESSAGE_CTRL_SIZE) <
