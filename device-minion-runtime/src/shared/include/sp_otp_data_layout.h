...
typedef struct OTP_CHICKEN_BITS_s {
    union {
        struct {
            ...
            uint32_t VaultIP_Cock_Switch_Chicken_Bit : 2;
...
typedef struct OTP_FLASH_CONFIGURATION_OVERRIDE_0_s {
    union {
        struct {
            ...
            uint32_t SKIP_RDID : 1;             // 1 - skip the RDID command, 0 - issue the RDID command
            uint32_t SKIP_RDSFDP : 1;           // 1 - skip the RDSFDP command, 0 - issue the RDID command
            uint32_t reserved : 2;
...
typedef struct OTP_FLASH_CONFIGURATION_OVERRIDE_1_s {
    union {
        struct {
            ...
            uint32_t FLASH_GET_PROGRAM_STATUS_DONE_VALUE : 8;   // (when STATUS & MASK) == DONE_VALUE, the programming operation is completed
