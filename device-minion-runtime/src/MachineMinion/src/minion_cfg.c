...
static int64_t minion_configure_cold_boot_pll(uint64_t shire_mask, uint8_t lvdpll_strap)
{
    ...
    lvdpll_mode = freq_to_mode(800, lvdpll_strap);
    status = pll_normalize_and_turn_of_normalization(lvdpll_mode, shire_mask, get_pll_conf_reg()->num_shires);
    if (status != 0)
    {
        return status;
    }
    ...
    get_pll_conf_reg()->minion_norm_freq = gs_lvdpll_settings[(lvdpll_mode-1)].values[14] * 800;

    lvdpll_mode = freq_to_mode(INITIAL_MINION_FREQ, lvdpll_strap);
    status = update_minion_pll_freq_quick(lvdpll_mode, shire_mask, get_pll_conf_reg()->num_shires);
    if (status != 0)
    {
        return status;
    }
    get_pll_conf_reg()->minion_current_freq = (uint64_t)mode_to_freq(lvdpll_mode, minion_lvdpll_strap);
#else
    ...
    get_pll_conf_reg()->minion_norm_freq = INITIAL_MINION_FREQ;
#endif
...
#else
    status = dvfs_update_minion_pll_mode(freq_to_mode((uint16_t)freq, get_pll_conf_reg()->minion_lvdpll_strap),
                get_pll_conf_reg()->booted_shire_mask, get_pll_conf_reg()->num_shires, MM_DVFS_POLL_FOR_LOCK);
#endif
