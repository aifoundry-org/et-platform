...
#------------------------------------------------------------------------------
function(jinja_generate_api_cpp SRCS HDRS)
    if(NOT ARGN)
        message(SEND_ERROR "Error: jinja_generate_api_cpp() called without any jinja files")
        return()
    endif()

    set(options PRIVILEGED NON_PRIVILEGED NO_POSTFIX)
    ...
    if (NOT (JINJA_GENERATE_API_PRIVILEGED OR JINJA_GENERATE_API_NON_PRIVILEGED))
        message(SEND_ERROR "Error: jinja_generate_api_cpp() called without argument: PRIVILEGED | NON_PRIVILEGED")
        return()
    endif()

    if (NOT JINJA_GENERATE_API_TARGET)
        message(SEND_ERROR "Error: jinja_generate_api_cpp() called without argument: TARGET")
        return()
    endif()
    ...
    if (NOT JINJA_GENERATE_API_JINJA_TEMPLATES)
        message(SEND_ERROR "Error: jinja_generate_api_cpp() called without argument: JINJA_TEMPLATES")
        return()
    endif()

    if (JINJA_GENERATE_API_NO_POSTFIX)
        set(GENERATE_API_TYPE_POSTFIX "")
    elseif (JINJA_GENERATE_API_PRIVILEGED)
        set(GENERATE_API_TYPE_POSTFIX "_privileged")
    elseif(JINJA_GENERATE_API_NON_PRIVILEGED)
        set(GENERATE_API_TYPE_POSTFIX "_non_privileged")
    endif()

    file(GLOB_RECURSE DEVICE_API_SCHEMA_FILES RELATIVE ${JINJA_GENERATE_API_DEVICE_API_CODEGEN_DIR} *.yaml)

    if (JINJA_GENERATE_API_PRIVILEGED)
        set(DEVICE_API_SPEC_FILE ${JINJA_GENERATE_API_DEVICE_API_CODEGEN_DIR}/privileged-api/device-api-privileged.yaml)
    elseif (JINJA_GENERATE_API_NON_PRIVILEGED)
        set(DEVICE_API_SPEC_FILE ${JINJA_GENERATE_API_DEVICE_API_CODEGEN_DIR}/non-privileged-api/device-api-non-privileged.yaml)
    endif()
    ...
    foreach(FIL ${JINJA_GENERATE_API_JINJA_TEMPLATES})
        ...
        set(GENERATED_FILE_NAME ${FIL_WE}${GENERATE_API_TYPE_POSTFIX}${SRC_EXT})
        set(GENERATED_DIR       "${CMAKE_CURRENT_BINARY_DIR}/esperanto/device-api")
        set(GENERATED_FILE      "${GENERATED_DIR}/${FIL_DIR}${GENERATED_FILE_NAME}")
        ...
        add_custom_command(
            ...
            COMMAND ${JINJA_GENERATE_API_DEVICE_API_CODEGEN_DIR}/device_api_codegen.py
            ARGS  --spec     ${DEVICE_API_SPEC_FILE}
                  --schema   ${JINJA_GENERATE_API_DEVICE_API_CODEGEN_DIR}/schema/device-api.schema.json
                  --template ${JINJA_TEMPLATE}
                  --output   ${GENERATED_FILE}
            DEPENDS ${JINJA_GENERATE_API_DEVICE_API_CODEGEN_DIR}/device_api_codegen.py
                    ${JINJA_GENERATE_API_DEVICE_API_CODEGEN_DIR}/schema/device-api.schema.json
