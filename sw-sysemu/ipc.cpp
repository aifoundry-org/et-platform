...
static int32_t     latency[MAXOPCODE];
static ready_t     fready[MAXFREG];
static ready_t     xready[MAXXREG];
static ready_t     mready[MAXMREG];
static uint64_t    cycle = 0;
static uint64_t    last_texsnd = 0;
static uint64_t    numins = 0;
static uint32_t    debug_ipc = 0;
static uint32_t    exit_on_first_ps = 0;
static int         lat_sampler;
static uint64_t    histogram[MAXPRODUCERS] = {0,0,0,0,0};
static const char* pnames[MAXPRODUCERS] = {"FPU", "DCACHE", "TBOX", "ALU", "NONE"};

static FILE *ipcoutFile;
...
static cache_tag_t dcache[DSETS][DWAYS];
static uint32_t lru[DSETS][DWAYS];

static uint64_t dcache_num_lookups = 0;
static uint64_t dcache_num_rdhit   = 0;
static uint64_t dcache_num_wrhit   = 0;
static uint64_t dcache_num_rddelayhit = 0;
static uint64_t dcache_num_wrdelayhit = 0;
static uint64_t dcache_num_rdmiss  = 0;
static uint64_t dcache_num_wrmiss  = 0;

static void dcache_init()
{
    if (debug_ipc)
    {
        gfprintf(ipcoutFile,"dcache_init: sets %d ways %d line size %d total capacity %d\n",
...
static void make_mru(uint32_t set, uint32_t way)
{
...
static uint32_t get_victim(uint32_t set)
{
    // The LRU way is always at the back of the LRU list
    return lru[set][DWAYS-1];
}

static uint32_t dcache_lookup(uint64_t addr, access_t acc)
{
    ...
    for (uint32_t w = 0; w < DWAYS; w++)
    {
        if (dcache[set][w].valid && dcache[set][w].tag == tag)
        {
            ...
            if (dcache[set][w].available <= cycle)
            {
                if  (debug_ipc)
                {
                    gfprintf(ipcoutFile,"dcache_lookup: %6s: cycle %llu available %llu hit on set %d way %d tag %llx addr %llx\n",
            ...
            else
            {
                ...
                if (debug_ipc)
                {
                    gfprintf(ipcoutFile,"dcache_lookup: %6s: cycle %llu available %llu wait on hit on set %d way %d tag %llx addr %llx pending %llu \n",
    ...
    if (debug_ipc)
    {
        gfprintf(ipcoutFile,"dcache_lookup: %6s: cycle %llu miss on set %d tag %llx addr %llx --> victim %d\n",
...
static uint32_t dcache_read(uint64_t addr)
{
    return dcache_lookup(addr, READ);
}

static uint32_t dcache_write(uint64_t addr)
{
...
void ipc_init(const char *type, int debug)
{
    ...
    if (ipcoutFile == NULL)
    {
        ...
        if (ipcoutFile == NULL)
        {
            throw std::runtime_error("Failed to open 'ipc.txt' for writing");
        }
    }
    if (debug_ipc)
    {
    ...
    for (i = 0; i < MAXFREG; i++) { fready[i].cycle = 0; fready[i].producer = PRODNONE; }
    for (i = 0; i < MAXXREG; i++) { xready[i].cycle = 0; xready[i].producer = PRODNONE; }
    for (i = 0; i < MAXMREG; i++) { mready[i].cycle = 0; mready[i].producer = PRODNONE; }
    ...
    {
        if (latency[i] == -1)
            throw std::runtime_error("No latency defined for opcode");
    }
}
...
static ready_t earliest(ready_t start, xreg src)
{
...
static ready_t earliest(ready_t start, freg src)
{
    if (src != fnone && fready[src].cycle > start.cycle)
    {
        // if the src is ready in the future, then return its ready cycle and
        // producer as the ealiest time we can execute
        return fready[src];
    }
    else
    {
...
static void set_dst_cycle(freg dst, uint64_t newcycle, producers p)
{
    fready[dst].cycle = newcycle;
    fready[dst].producer = p;
}

static void set_dst_cycle(xreg dst, uint64_t newcycle, producers p)
{
    xready[dst].cycle = newcycle;
    xready[dst].producer = p;
}

static void delay_stats(ready_t ready)
{
    ...
    if (ready.cycle > (cycle+1))
    {
        ...
        if (debug_ipc)
        {
            gfprintf(ipcoutFile,"histo[%d] += %llu (%llu - %llu)\n",
                     ready.producer,(ready.cycle-(cycle+1)),ready.cycle,cycle+1);
        }
    }
}

static uint64_t advance_time(ready_t ready)
{
    delay_stats(ready);
    cycle = ready.cycle;
    numins++;
...
static void debug_instruction(xreg xdst, freg fdst, xreg xsrc1, xreg xsrc2, freg fsrc1, freg fsrc2, freg fsrc3, uint64_t lat, char *dis,bool miss)
{
    char latinfo[512];
    char srcinfo[512];
    char dstinfo[512];
