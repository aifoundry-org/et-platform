...
#define require_feature_gfx() do { \
    if (shire_other_esrs[shire_index(cpu)].minion_feature & 0x1) \
        throw trap_illegal_instruction(cpu.inst.bits); \
} while (0)


#define require_feature_ml() do { \
    if (shire_other_esrs[shire_index(cpu)].minion_feature & 0x2) \
        throw trap_illegal_instruction(cpu.inst.bits); \
} while (0)
...
#define require_feature_ml_on_thread0() do { \
    if ((cpu.mhartid % EMU_THREADS_PER_MINION) || \
        (shire_other_esrs[shire_index(cpu)].minion_feature & 0x2)) \
        throw trap_illegal_instruction(cpu.inst.bits); \
} while (0)

#define require_feature_u_cacheops() do { \
    if (shire_other_esrs[shire_index(cpu)].minion_feature & 0x4) \
        throw trap_illegal_instruction(cpu.inst.bits); \
} while (0)


#define require_feature_u_scratchpad() do { \
    if (shire_other_esrs[shire_index(cpu)].minion_feature & 0x8) \
        throw trap_illegal_instruction(cpu.inst.bits); \
} while (0)


#define require_lock_unlock_enabled() do { \
    if (shire_other_esrs[shire_index(cpu)].minion_feature & 0x24) \
        throw trap_illegal_instruction(cpu.inst.bits); \
} while (0)
