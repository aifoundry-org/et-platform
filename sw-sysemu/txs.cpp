...
{
    uint32_t neigh_id = thread / EMU_THREADS_PER_NEIGH;

    LOG_NOTHREAD(DEBUG, "\t[TBOX ID from Thread] Current thread %u, EMU_THREADS_PER_NEIGH %u, neigh_id %u", thread, EMU_THREADS_PER_NEIGH, neigh_id);
...
*/
void new_sample_request(uint32_t thread, uint32_t port_id, uint32_t number_packets, uint64_t base_address)
{
    ...
    {
        val[i] = pmemread<uint64_t>(Noshire_agent{}, base_address);
    ...
    GET_TBOX(shire_id, tbox_id).set_request_header(thread, header);
    ...
    GET_TBOX(shire_id, tbox_id).set_request_pending(thread, false);

    /* Compute request */
    GET_TBOX(shire_id, tbox_id).sample_quad(thread, true); // Performs Sample Request
    ...
    for (uint32_t channel = 0; channel < num_channels; channel++)
    {
        ...
        // Put result in port
        write_msg_port_data_from_tbox(thread, port_id, shire_id * EMU_TBOXES_PER_SHIRE + tbox_id, &(data[channel].u32[0]), 1);
    }
}
