...
uint64 get_abs_integral_fxp(int64 v, uint32 int_bits, uint32 frac_bits)
{
    uint64 v_abs = (v < 0) ? uint64(-v) : uint64(v);
    uint64 v_int_abs = v_abs >> frac_bits;
    
    return v_int_abs;
}

int64 get_integral_fxp(int64 v, uint32 int_bits, uint32 frac_bits)
{
    uint64 v_int_abs = get_abs_integral_fxp(v, int_bits, frac_bits);
    return (v < 0) ? -int64(v_int_abs) : int64(v_int_abs);
}

uint64 get_fractional_fxp(int64 v, uint32 frac_bits, uint32 digits)
{
    uint64 v_abs = (v < 0) ? uint64(-v) : uint64(v);
    uint64 v_abs_frac = v_abs & ((1 << frac_bits) - 1);
    uint64 v_abs_dec_frac = 0;
...
int64 extend_sign(uint64 v, uint32 sign_bit)
{
    return int64(v) | (int64(0) - (1LL << sign_bit));
}

char *fxp_2_dec_str(char *str, int64 v, uint32 int_bits, uint32 frac_bits, uint32 frac_dec_digits)
{
    char format_str[32];
...
float32 fxp_2_float32(int64 v, uint32 int_bits, uint32 frac_bits)
{
    uint64 v_abs = (v < 0) ? uint64(-v) : uint64(v);
...
char *fxp_2_hex_str(char *str, int64 v, uint32 int_bits, uint32 frac_bits)
{
    char format_str[32];
    uint32 frac_hex_digits = frac_bits >> 2 + (((frac_bits & 0x3) > 0) ? 1 : 0);
...
void setup_triangle(float32 vpos_x[3], float32 vpos_y[3], float32 vdepth[3], int64 a[4], int64 b[4], int64 c[4])
{
    char str1[256], str2[256], str3[256], str4[256], str5[256], str6[256];

    int64 vpos_x_fxp_16_8[3];
    int64 vpos_y_fxp_16_8[3];
    int32 vdepth_unorm24[3];

    for (uint32 v = 0; v < 3; v++)
    {
        vpos_x_fxp_16_8[v] = int64(vpos_x[v] * 256.0);
        vpos_y_fxp_16_8[v] = int64(vpos_y[v] * 256.0);
        vdepth_unorm24[v] = int32(vdepth[v] * float32((1 << 24) - 1));
    ...
    int64 tri_2x_area_fxp_35_16 = c_fxp_33_16[0] + c_fxp_33_16[1] + c_fxp_33_16[2];
    ...
    int64 an_fxp_9_24[3];
    int64 bn_fxp_9_24[3];
    int64 cn_fxp_26_24[3];

    for (uint32 e = 0; e < 3; e++)
    {
        int64 recip_tri_2x_area_fxp_16_35 = ((1LL << 51) / tri_2x_area_fxp_35_16);
...
int main()
{
    ...
    uint32 depth_stencil_buffer[1024 * 1024];

    uint32 in_buffer_ptr = 0;

    for (uint32 y = 0; y < 1024; y++)
        for (uint32 x = 0; x < 1024; x++)
            depth_stencil_buffer[y * 1024 + x] = 0xFFFFFF;

    uint64 in_buffer[1024];
    uint64 *out_buffer = new uint64[1024 * 1024];

    set_rbox((uint64) in_buffer, 1024, (uint64) out_buffer, 1024 * 1024);
    ...
    rbox_state_pckt.state.depth_stencil_buffer_ptr = (uint64) depth_stencil_buffer;
    ...
    for (uint32 eq = 0; eq < 3; eq++)
    {
        ...
        tri_with_tile_64x64_pckt.tri_with_tile_64x64.edge_eqs[eq].a = (uint64) (a[eq] >> 11);
        tri_with_tile_64x64_pckt.tri_with_tile_64x64.edge_eqs[eq].b = (uint64) (b[eq] >> 11);
        tri_with_tile_64x64_pckt.tri_with_tile_64x64.edge[eq].e     = (uint64) (c[eq] >> 11);
    }
    printf("Depth eq => a = %llx b = %llx c = %llx\n", a[3], b[3], c[3]);
    tri_with_tile_64x64_pckt.tri_with_tile_64x64.depth_eq.a = uint32(a[3]);
    tri_with_tile_64x64_pckt.tri_with_tile_64x64.depth_eq.b = uint32(b[3]);
    tri_with_tile_64x64_pckt.tri_with_tile_64x64.depth      = uint32(c[3]);
    ...
    uint32 vposx[] = {0,         0,  64 * 256, 64 * 256, 318 * 256, 318 * 256};
    uint32 vposy[] = {0, 128 * 256, 128 * 256,        0,         0, 128 * 256};
    ...
    uint64 rbox_input_buffer[1024];

    uint32 rbox_packets = 0;

    set_rbox((uint64) rbox_input_buffer, 1024, (uint64) out_buffer, 1024 * 1024);
    ...
    create_rbox_packets(&setup_output, (uint8 *) &rbox_input_buffer[in_buffer_ptr], &rbox_packets, TILE_SIZE_32x32);

    printf("Number of RBOX packets generated : %d\n", rbox_packets);

    for(uint32 p = 0; p < rbox_packets; p++)
        push_packet();
    ...
    uint8 framebuffer[1024 * 1024];

    for (uint32 b = 0; b < (1024 * 1024); b++)
        framebuffer[b] = 0;
            ...
            case RBOX_OUTPCKT_QUAD_INFO :
                {
                    ...
                    uint32 x = out_quad_info_pckt.quad_info.x << 1;
                    uint32 y = out_quad_info_pckt.quad_info.y << 1;
