...
checker_result checker::do_reduce(uint32_t thread, uint64_t value, int * wake_minion)
{
    ...
    if(action == 0)
    {
        ...
        // If it_mask has consumed the data, move both threads to Idle
        else if(reduce_state_array[other_min] == Reduce_Data_Consumed)
        {
            ...
            // Wakes up the thread0 of the other minion to guarantee it_mask checks the reduce instruction
    ...
    else if(action == 1)
    {
        ...
        // Previous reduce done, set with which minion it_mask is doing the reduce
        reduce_pair_array[thread>>1] = other_min;
...
checker_result checker::emu_inst(uint32_t thread, inst_state_change * changes, std::queue<uint32_t> &wake_minions)
{
    ...
    try
    {
        check_pending_interrupts(); // This need to be here because it_mask can trap
    ...
    {
        log_trap(); // Mark the trap before taking it_mask so we can check partial results
...
checker_result checker::check_state_changes(uint32_t thread, inst_state_change * changes, const insn_t& inst)
{
   ...
   // Checks modified fields
    if(changes != NULL)
    {
        ...
        {
            stream << "BEMU Checker TRAP mismatch. BEMU expects to " << (emu_state_change.trap_mod ? "TRAP" : "NOT TRAP")  << " : but DUT " << (changes->trap_mod ? "TRAP" : "DIDN'T TRAP") << std::endl;
        ...
        else if(changes->trap_mod)
        {
            // Check system registers
             ...
             {
               stream << "BEMU Checker GSC_PROGRESS CSR mismatch. BEMU expects  " << emu_state_change.gsc_progress_mod  << " : but DUT " << changes->gsc_progress_mod << std::endl;
               check_res = CHECKER_ERROR;
             }
             if (changes->gsc_progress_mod )
             {
                ...
                {
                  stream << "BEMU Checker GSC_PROGRESS CSR mismatch. BEMU expects  " << emu_state_change.gsc_progress  << " : but DUT " << changes->gsc_progress << std::endl;
                  check_res = CHECKER_ERROR;
                }
             }
        ...
        {
            stream << "BEMU Checker PC mismatch. BEMU expects PC: 0x" << std::hex << current_pc[thread] << " but DUT reported PC: 0x" << changes->pc << std::dec << std::endl;
        ...
        {
            stream << "BEMU Checker Inst mismatch. BEMU expects inst is 0x" << std::hex << inst.bits << " but DUT reported 0x" << changes->inst_bits << std::dec << std::endl;
        ...
        {
            stream << "BEMU Checker Int Register write mismatch. BEMU expects register write is " << emu_state_change.int_reg_mod << " but DUT reported " << changes->int_reg_mod << std::endl;
            check_res = CHECKER_ERROR;
        }
        else if(emu_state_change.int_reg_mod)
        {
            ...
            {
                stream << "BEMU Checker Int Register dest mismatch. BEMU expects register dest is x" << emu_state_change.int_reg_rd << " but DUT reported x" << changes->int_reg_rd << std::endl;
                check_res = CHECKER_ERROR;
            }
            ...
            // Check if it_mask is an AMO (special case where RTL drives value)
            ...
            // Check if it_mask is an Fast Local Barrier (special case where RTL drives value)
            ...
            {
                stream << "EMU Checker Int Register data mismatch. BEMU expects data is 0x" << std::hex << emu_state_change.int_reg_data << " but DUT reported 0x" << changes->int_reg_data << std::dec << std::endl;
                check_res = CHECKER_ERROR;
                //Set EMU state to what RTL says
        ...
        {
            stream << "BEMU Checker FP Register write mismatch. BEMU expects write is " << emu_state_change.fp_reg_mod << " but DUT reported " << changes->fp_reg_mod << std::endl;
            check_res = CHECKER_ERROR;
        }
        else if(emu_state_change.fp_reg_mod)
        {
            ...
            {
                stream << "BEMU Checker FP Register dest mismatch. BEMU expects dest is f" << emu_state_change.fp_reg_rd << " but DUT reported f" << changes->fp_reg_rd << std::endl;
                check_res = CHECKER_ERROR;
            }
        ...
        for(int m = 0; m < 8; m++)
        {
            ...
            {
                stream << "BEMU Checker Mask Register write mismatch. BEMU expects write is " << emu_state_change.m_reg_mod[m] << " but DUT reported " << changes->m_reg_mod[m] << std::endl;
                check_res = CHECKER_ERROR;
            } else if(emu_state_change.m_reg_mod[m])
            {
                for(size_t i = 0; i < VL; i++)
                {
                    ...
                    {
                        stream << "BEMU Checker Mask Register data mismatch m" << m << "[" << i << "]. BEMU expects data is " << std::hex << (uint32_t) emu_state_change.m_reg_data[m][i] << " but DUT reported " << (uint32_t) changes->m_reg_data[m][i] << std::dec << std::endl;
                        check_res = CHECKER_ERROR;
                    }
                }
            }
        ...
        for(size_t i = 0; i < VL; i++)
        {
            ...
            {
                stream << "BEMU Checker Memory write mismatch (" << i << "). BEMU expects write is " << emu_state_change.mem_mod[i] << " but DUT reported " << changes->mem_mod[i] << std::endl;
                check_res = CHECKER_ERROR;
            }
            else if(emu_state_change.mem_mod[i])
            {
                ...
                {
                    stream << "BEMU Checker Memory write size mismatch (" << i << "). BEMU expects size is " << emu_state_change.mem_size[i] << " but DUT reported " << changes->mem_size[i] << std::endl;
                    check_res = CHECKER_ERROR;
                }
                if( (changes->mem_addr[i] & VA_M) != (emu_state_change.mem_addr[i] & VA_M))
                {
                    stream << "BEMU Checker Memory write address mismatch (" << i << "). BEMU expects addr is 0x" << std::hex << emu_state_change.mem_addr[i] << " & " << VA_M <<" but DUT reported 0x" << changes->mem_addr[i] <<" & "<< VA_M << std::dec << std::endl;
                    check_res = CHECKER_ERROR;
                }
        ...
        if(inst.is_tensor_load())
        {
            ...
            for(int i = 0; i < emu_state_change.tl_scp_count && emu_state_change.tensor_mod; i++)
            {
                ...
                auto it_mask = std::find_if(scp_entry_list[thread].begin(), scp_entry_list[thread].end(),
                                       [=] (const scratchpad_entry& x) { return x.entry == cache_index; });

                // Checks that an entry was actually found
                if(it_mask == scp_entry_list[thread].end())
                {
                ...
                else
                {
                    ...
                    for(int j = 0; j < (L1D_LINE_SIZE/4); j += 2)
                    {
                        ...
                        {
                            stream << "BEMU Checker TensorLoad write data_error for cacheline " << i << " written in entry " << start + i << " data lane " << j << ". BEMU expects data is 0x" << std::hex << data << " but DUT reported 0x" << it_mask->data[j] << std::dec << std::endl;
                            check_res = CHECKER_ERROR;
                        }
                    }
                    scp_entry_list[thread].erase(it_mask);
                }
            }
        }
            ...
            for(int pass = 0; pass < emu_state_change.tensorfma_passes; pass++)
            {
                ...
                for(int freg = 0; freg < MAXFREG; freg++)
                {
                    ...
                    for(size_t lane = 0; lane < VL; lane++)
                    {
                        ...
                        {
                            stream << "BEMU Checker TensorFMA write data_error for register f" << freg << "[" << lane << "] pass " << pass << ". BEMU expects data is 0x" << std::hex << data << " but DUT reported 0x" << it_mask->data[lane] << std::dec << std::endl;
                            check_res = CHECKER_ERROR;
                        }
                    }
            ...
            for(int trans = 0; trans < emu_state_change.tensorquant_trans; trans++)
            {
                ...
                for(int freg = 0; freg < 32; freg++)
                {
                    ...
                    for(size_t lane = 0; lane < VL; lane++)
                    {
                        ...
                        if(data != it_mask->data[lane])
                        {
                            stream << "BEMU Checker TensorQuant write data_error for register f" << freg << "[" << lane << "] trans " << trans << ". BEMU expects data is 0x" << std::hex << data << " but DUT reported 0x" << it_mask->data[lane] << std::dec << std::endl;
                            check_res = CHECKER_ERROR;
                        }
                    }
                    tensorquant_list[thread].erase(it_mask);
        ...
        if(inst.is_reduce())
        {
            ...
            {
                // Looks for the 1st entry in the list of RTL written lines with same destination
                auto it_mask = reduce_list[thread].begin();
                while(it_mask != reduce_list[thread].end())
                {
                    if(it_mask->entry == entry) { break; }
                    it_mask++;
                }

                // Checks that an entry was actually found
                if(it_mask == reduce_list[thread].end())
                {
                ...
                else
                {
                    ...
                    for(int lane = 0; lane < 4; lane++)
                    {
                        ...
                        if(data != it_mask->data[lane])
                        {
                            stream << "BEMU Checker Reduce write data_error for register " << entry << " lane " << lane << ". BEMU expects data is 0x" << std::hex << data << " but DUT reported 0x" << it_mask->data[lane] << std::dec << std::endl;
                            check_res = CHECKER_ERROR;
                        }
                    }
                    reduce_list[thread].erase(it_mask);
                }
            }
        }
    }
...
void checker::aggregate_tl_data(uint32_t thread)
{
    ...
    while ( !scp_entry_list[thread].empty() ) {
        auto it_mask = scp_entry_list[thread].begin();
        // Get the entry and copy it_mask. Move data to next half-line if it_mask's needed
        scratchpad_entry entry = *it_mask;
        it_mask = scp_entry_list[thread].erase(it_mask);
