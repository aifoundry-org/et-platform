        ...
        stage('PARALLEL1') {
          parallel {
            stage('JOB_CODE_QUALITY') {
              steps {
                script {
                  if ( ( (env.FORCE_CHILD_RETRIGGER != null) && sh(returnStatus: true, script: "${FORCE_CHILD_RETRIGGER}") == 0) || sh(returnStatus: true, script: './ci/ci-tools/scripts/jenkins_scripts.py job_passed_for_branch --branch "' + "${SW_PLATFORM_BRANCH}" + '" sw-platform/code-analysis/sw-sysemu-sonarqube \'{  "COMPONENT_COMMITS":"' + "${COMPONENT_COMMITS},tools/sw-sysemu:${BRANCH}" + '" }\'') != 0) {
                    build job:
