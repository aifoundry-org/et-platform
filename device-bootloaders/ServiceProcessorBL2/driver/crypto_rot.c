...
DONE:
    ...
    if (0 != public_key_parameters_asset_id) {
        if (0 != vaultip_drv_asset_delete(get_rom_identity(), public_key_parameters_asset_id)) {
            MESSAGE_ERROR_DEBUG(
                "crypto_ecdsa_verify: vaultip_drv_asset_delete(public_key_parameters_asset_id) failed!\n");
        }
    }
                                           ...
                                           size_t buffer_size, uint32_t *counter_size)
{
    ...
    if (0 != vaultip_drv_static_asset_search(get_rom_identity(), static_asset_id, &asset_id,
                                             &asset_size)) {
        MESSAGE_ERROR_DEBUG(
            "crypto_load_monotonic_counter_from_otp: vaultip_drv_static_asset_search(%u) failed!\n",
            static_asset_id);
        return -1;
    }
    ...
    *counter_size = asset_size;
    ...
    if (0 != vaultip_drv_monotonic_counter_read(get_rom_identity(), asset_id, buffer,
        ...
        MESSAGE_ERROR_DEBUG(
            "crypto_load_monotonic_counter_from_otp: vaultip_drv_monotonic_counter_read() failed!\n");
        return -1;
    }
    return 0;
}
