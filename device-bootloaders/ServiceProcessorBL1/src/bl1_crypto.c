...
int crypto_derive_kdk_key(const void * kdk_derivation_data, size_t kdk_derivation_data_size, uint32_t * kdk_asset_id) {
    ...
    if (0 != vaultip_static_asset_search(get_rom_identity(), VAULTIP_STATIC_ASSET_HUK, &huk_asset_id, &huk_asset_length)) {
        MESSAGE_ERROR_DEBUG("derive_kdk: vaultip_static_asset_search(HUK) failed!\n");
        return -1;
    }
...
int crypto_delete_key(uint32_t key) {
    if (0 != vaultip_asset_delete(get_rom_identity(), key)) {
        MESSAGE_ERROR_DEBUG("crypto_delete_key: vaultip_asset_delete() failed!\n");
        return -1;
    }
    return 0;
}
...
int crypto_hash_abort(CRYPTO_HASH_CONTEXT_t * hash_context) {
    ...
    if (hash_context->hash_alg != HASH_ALG_INVALID) {
        if (0 != vaultip_asset_delete(get_rom_identity(), hash_context->temp_digest_asset_id)) {
            MESSAGE_ERROR_DEBUG("crypto_hash_abort: vaultip_asset_delete failed!");
            return -1;
        }
    }
...
int crypto_hash_final(CRYPTO_HASH_CONTEXT_t * hash_context, const void * msg, size_t msg_size, size_t total_msg_length, uint8_t * hash) {
    ...
    if (0 == rv) {
        if (0 != vaultip_asset_delete(get_rom_identity(), hash_context->temp_digest_asset_id)) {
            MESSAGE_ERROR_DEBUG("crypto_hash_final: vaultip_asset_delete failed!");
            rv = -1;
        }
...
int crypto_mac_verify(ESPERANTO_MAC_TYPE_t mac_alg,
    ...
    if (0 != vaultip_mac_verify(get_rom_identity(), mac_alg, mack_key, data, data_size, mac)) {
        MESSAGE_ERROR_DEBUG("test_kdk: vaultip_mac_verify() failed!\n");
        return -1;
    }
...
int crypto_aes_decrypt_update(CRYPTO_AES_CONTEXT_t * aes_context, void * data, size_t data_size) {
    if (0 != vaultip_aes_cbc_decrypt(get_rom_identity(), aes_context->aes_key_asset_id, aes_context->IV, data, data_size)) {
        MESSAGE_ERROR_DEBUG("crypto_aes_decrypt_update: vaultip_aes_cbc_encrypt() failed!\n");
        return -1;
    }
...
int crypto_aes_decrypt_final(CRYPTO_AES_CONTEXT_t * aes_context, void * data, size_t data_size, uint8_t * IV) {
    if (NULL != data && data_size > 0) {
        if (0 != vaultip_aes_cbc_decrypt(get_rom_identity(), aes_context->aes_key_asset_id, aes_context->IV, data, data_size)) {
            MESSAGE_ERROR_DEBUG("crypto_aes_decrypt_final: vaultip_aes_cbc_encrypt() failed!\n");
            return -1;
        }
    }
...
static int crypto_create_ec_parameters_asset(EC_KEY_CURVE_ID_t curve_id, uint32_t * ec_parameters_asset_id) {
    ...
    if (0 != vaultip_asset_create(get_rom_identity(), public_key_parameters_asset_policy.lo, public_key_parameters_asset_policy.hi,
        ...
        MESSAGE_ERROR_DEBUG("crypto_create_ec_parameters_asset: vaultip_asset_create() failed!\n");
        return -1;
    }

    if (0 != vaultip_asset_load_plaintext(get_rom_identity(), asset_id, domain_parameters_data_ptr, domain_parameters_size)) {
        MESSAGE_ERROR_DEBUG("crypto_create_ec_parameters_asset: vaultip_asset_load_plaintext() failed!\n");
        if (0 != vaultip_asset_delete(get_rom_identity(), asset_id)) {
            MESSAGE_ERROR_DEBUG("crypto_create_ec_parameters_asset: vaultip_asset_delete() failed!\n");
        }
        return -1;
    }
...
static int crypto_create_ec_public_key_asset(HASH_ALG_t hash_alg, const PUBLIC_KEY_EC_t * ec_public_key, uint32_t * ec_public_key_asset_id) {
    ...
    if (0 != vaultip_asset_create(get_rom_identity(), public_key_asset_policy.lo, public_key_asset_policy.hi,
        ...
        MESSAGE_ERROR_DEBUG("crypto_create_ec_public_key_asset: vaultip_asset_create() failed!\n");
        return -1;
    }
    ...
    if (0 != vaultip_asset_load_plaintext(get_rom_identity(), asset_id, public_key_data_ptr, public_key_data_size)) {
        MESSAGE_ERROR_DEBUG("crypto_create_ec_public_key_asset: vaultip_asset_load_plaintext() failed!\n");
        if (0 != vaultip_asset_delete(get_rom_identity(), asset_id)) {
            MESSAGE_ERROR_DEBUG("crypto_create_ec_public_key_asset: vaultip_asset_delete() failed!\n");
        }
        return -1;
    }
...
DONE:
    ...
    if (0 != public_key_parameters_asset_id) {
        if (0 != vaultip_asset_delete(get_rom_identity(), public_key_parameters_asset_id)) {
            MESSAGE_ERROR_DEBUG("crypto_ecdsa_verify: vaultip_asset_delete(public_key_parameters_asset_id) failed!\n");
        }
    }
    if (0 != public_key_asset_id) {
        if (0 != vaultip_asset_delete(get_rom_identity(), public_key_asset_id)) {
            MESSAGE_ERROR_DEBUG("crypto_ecdsa_verify: vaultip_asset_delete(public_key_asset_id) failed!\n");
        }
    }
    return rv;
}
...
static int crypto_create_rsa_public_key_asset(HASH_ALG_t hash_alg, const PUBLIC_KEY_RSA_t * rsa_public_key, uint32_t * rsa_public_key_asset_id) {
    ...
    if (0 != vaultip_asset_create(get_rom_identity(), public_key_asset_policy.lo, public_key_asset_policy.hi,
                  public_key_asset_other_settings, 0, &asset_id)) {
        MESSAGE_ERROR_DEBUG("crypto_create_rsa_public_key_asset: vaultip_asset_create() failed!\n");
        return -1;
    }

    MESSAGE_INFO_DEBUG("crypto_create_rsa_public_key_asset: asset created, assetid=0x%x\n", asset_id);
    if (0 != vaultip_asset_load_plaintext(get_rom_identity(), asset_id, public_key_data_ptr, public_key_data_size)) {
        MESSAGE_ERROR_DEBUG("crypto_create_rsa_public_key_asset: vaultip_asset_load_plaintext() failed!\n");
        if (0 != vaultip_asset_delete(get_rom_identity(), asset_id)) {
            MESSAGE_ERROR_DEBUG("crypto_create_rsa_public_key_asset: vaultip_asset_delete() failed!\n");
        }
        return -1;
    }
...
int crypto_verify_pk_signature(const PUBLIC_KEY_t * public_key, const PUBLIC_SIGNATURE_t * signature, const void * data, size_t data_size) {
    ...
    if (data_size > 4096) {
        ...
        if (0 != vaultip_asset_create(get_rom_identity(), temp_digest_asset_policy.lo, temp_digest_asset_policy.hi, temp_digest_asset_other_settings, 0, &temp_digest_asset_id)) {
            MESSAGE_ERROR_DEBUG("crypto_verify_pk_signature: vaultip_asset_create() failed!\n");
            return -1;
        }

        // pre-hash the data
        if (0 != vaultip_hash_update(get_rom_identity(), signature->hashAlg, temp_digest_asset_id, data, prehash_length, true)) {
            MESSAGE_ERROR_DEBUG("crypto_verify_pk_signature: vaultip_hash_init() failed!\n");
            rv = -1;
            goto DONE;
        }
    ...
    MESSAGE_INFO_DEBUG("crypto_verify_pk_signature: message_length=0x%x, total_message_length=0x%x\n", data_size, total_message_length);
    switch (public_key->keyType) {
    case PUBLIC_KEY_TYPE_EC:
...
DONE:
    if (0 != temp_digest_asset_id) {
        if (0 != vaultip_asset_delete(get_rom_identity(), temp_digest_asset_id)) {
            MESSAGE_ERROR_DEBUG("crypto_verify_pk_signature: vaultip_asset_delete() failed!\n");
        }
    }
    return rv;
}
...
int crypto_load_public_key_hash_from_otp(VAULTIP_STATIC_ASSET_ID_t static_asset_id, void * buffer, size_t buffer_size, uint32_t * hash_size) {
    ...
    if (0 != vaultip_static_asset_search(get_rom_identity(), static_asset_id, &asset_id, &asset_size)) {
        MESSAGE_ERROR_DEBUG("load_public_key_hash_from_otp: vaultip_static_asset_search(%u) failed!\n", static_asset_id);
        return -1;
    }
    ...
    *hash_size = asset_size;
    ...
    if (0 != vaultip_public_data_read(get_rom_identity(), asset_id, buffer, (uint32_t)buffer_size, hash_size)) {
        MESSAGE_ERROR_DEBUG("load_public_key_hash_from_otp: vaultip_public_data_read() failed!\n");
        return -1;
    }
    return 0;
}
...
int crypto_load_monotonic_counter_from_otp(VAULTIP_STATIC_ASSET_ID_t static_asset_id, void * buffer, size_t buffer_size, uint32_t * counter_size) {
    ...
    if (0 != vaultip_static_asset_search(get_rom_identity(), static_asset_id, &asset_id, &asset_size)) {
        MESSAGE_ERROR_DEBUG("load_monotonic_counter_from_otp: vaultip_static_asset_search(%u) failed!\n", static_asset_id);
        return -1;
    }
    ...
    *counter_size = asset_size;
    ...
    if (0 != vaultip_monotonic_counter_read(get_rom_identity(), asset_id, buffer, (uint32_t)buffer_size, counter_size)) {
        MESSAGE_ERROR_DEBUG("load_monotonic_counter_from_otp: vaultip_monotonic_counter_read() failed!\n");
        return -1;
    }
    return 0;
}
