...
int SERIAL_init(uintptr_t uartRegs)
{
    ...
    iowrite32(uartRegs + UART_IIR_FCR_ADDRESS,
        ...
        UART_IIR_FCR_RT_SET(UART_IIR_FCR_RT_RT_FIFO_HALF_FULL)    | // rx threshold = RX FIFO half full
        UART_IIR_FCR_TET_SET(UART_IIR_FCR_TET_TET_FIFO_CHAR_2)    | // tx threshold = 2 characters in TX FIFO
    ...
    iowrite32(uartRegs + UART_IER_IER_ADDRESS,
        UART_IER_IER_ERBFI_SET(UART_IER_IER_ERBFI_ERBFI_DISABLED)    | // receive data available
        UART_IER_IER_ETBEI_SET(UART_IER_IER_ETBEI_ETBEI_DISABLED)    | // transmit holding register empty
        ...
        UART_IER_IER_ELCOLR_SET(UART_IER_IER_ELCOLR_ELCOLR_DISABLED) | // error bits cleared by reading LSR register or Rx FIFO (RBR read)
