                          ...
                          uint32_t *kdk_asset_id)
{
    ...
    // find HUK
    if (0 != vaultip_static_asset_search(get_rom_identity(), VAULTIP_STATIC_ASSET_HUK,
    ...
    // create KDK
    if (0 != vaultip_asset_create(get_rom_identity(), kdk_policy.lo, kdk_policy.hi,
    ...
    // derive KDK
    if (0 != vaultip_asset_load_derive(get_rom_identity(), *kdk_asset_id, huk_asset_id, NULL, 0,
                          ...
                          uint32_t *mack_asset_id)
{
    ...
    // create MACK
    if (0 != vaultip_asset_create(get_rom_identity(), mack_policy.lo, mack_policy.hi,
    ...
    // derive MACK
    if (0 != vaultip_asset_load_derive(get_rom_identity(), *mack_asset_id, kdk_asset_id, NULL, 0,
                          ...
                          size_t enck_derivation_data_size, uint32_t *enck_asset_id)
{
    ...
    // create ENCK
    if (0 != vaultip_asset_create(get_rom_identity(), enck_policy.lo, enck_policy.hi,
    ...
    // derive ENCK
    if (0 != vaultip_asset_load_derive(get_rom_identity(), *enck_asset_id, kdk_asset_id, NULL, 0,
...
int crypto_hash_init(CRYPTO_HASH_CONTEXT_t *hash_context, HASH_ALG_t hash_alg)
{
    ...
    if (0 != vaultip_asset_create(get_rom_identity(), temp_digest_asset_policy.lo,
...
int crypto_hash_update(CRYPTO_HASH_CONTEXT_t *hash_context, const void *msg, size_t msg_size)
{
    ...
    if (hash_context->init_done) {
        rv = vaultip_hash_update(get_rom_identity(), hash_context->hash_alg,
                      ...
                      size_t total_msg_length, uint8_t *hash)
{
    ...
    if (hash_context->init_done) {
        rv = vaultip_hash_final(get_rom_identity(), hash_context->hash_alg,
                                hash_context->temp_digest_asset_id, msg, msg_size, false,
...
{
    if (0 != vaultip_aes_cbc_decrypt(get_rom_identity(), aes_context->aes_key_asset_id,
                                                 ...
                                                 const ECC_DOMAIN_PARAMETERS_t *domain_parameters)
{
    ...
    if (0 != crypto_write_subvector_256(&(ec_256->curve_modulus_p), 7, 0, domain_parameters->P_p,
        ...
        MESSAGE_ERROR_DEBUG(
            "crypto_create_ec_256_parameters_asset: crypto_write_subvector_256(modulus_p) failed!\n");
        return -1;
    }
    ...
    if (0 != crypto_write_subvector_256(&(ec_256->curve_constant_a), 7, 1, domain_parameters->A_p,
        ...
        MESSAGE_ERROR_DEBUG(
            "crypto_create_ec_256_parameters_asset: crypto_write_subvector_256(const_A) failed!\n");
        return -1;
    }
    ...
    if (0 != crypto_write_subvector_256(&(ec_256->curve_constant_b), 7, 2, domain_parameters->B_p,
        ...
        MESSAGE_ERROR_DEBUG(
            "crypto_create_ec_256_parameters_asset: crypto_write_subvector_256(const_B) failed!\n");
        return -1;
    }
    ...
    if (0 != crypto_write_subvector_256(&(ec_256->curve_order_n), 7, 3, domain_parameters->Order_p,
        ...
        MESSAGE_ERROR_DEBUG(
            "crypto_create_ec_256_parameters_asset: crypto_write_subvector_256(order_n) failed!\n");
        return -1;
    }
    ...
    if (0 != crypto_write_subvector_256(&(ec_256->curve_base_point_x), 7, 4,
        ...
        MESSAGE_ERROR_DEBUG(
            "crypto_create_ec_256_parameters_asset: crypto_write_subvector_256(point_x) failed!\n");
        return -1;
    }
    ...
    if (0 != crypto_write_subvector_256(&(ec_256->curve_base_point_y), 7, 5,
        ...
        MESSAGE_ERROR_DEBUG(
            "crypto_create_ec_256_parameters_asset: crypto_write_subvector_256(point_y) failed!\n");
        return -1;
    }
    ...
    if (0 != crypto_write_subvector_32(&(ec_256->curve_cofactor), 7, 6, CoFactor, sizeof(CoFactor),
                                       bits)) {
        MESSAGE_ERROR_DEBUG(
            "crypto_create_ec_256_parameters_asset: crypto_write_subvector_32(cofactor) failed!\n");
        return -1;
    }
                                                 ...
                                                 const ECC_DOMAIN_PARAMETERS_t *domain_parameters)
{
    ...
    if (0 != crypto_write_subvector_384(&(ec_384->curve_modulus_p), 7, 0, domain_parameters->P_p,
                                        domain_parameters->PLen)) {
        MESSAGE_ERROR_DEBUG(
            "crypto_create_ec_384_parameters_asset: crypto_write_subvector_384(modulus_p) failed!\n");
        return -1;
    }
    ...
    if (0 != crypto_write_subvector_384(&(ec_384->curve_constant_a), 7, 1, domain_parameters->A_p,
                                        domain_parameters->ALen)) {
        MESSAGE_ERROR_DEBUG(
            "crypto_create_ec_384_parameters_asset: crypto_write_subvector_384(const_A) failed!\n");
        return -1;
    }
    ...
    if (0 != crypto_write_subvector_384(&(ec_384->curve_constant_b), 7, 2, domain_parameters->B_p,
                                        domain_parameters->BLen)) {
        MESSAGE_ERROR_DEBUG(
            "crypto_create_ec_384_parameters_asset: crypto_write_subvector_384(const_B) failed!\n");
        return -1;
    }
    ...
    if (0 != crypto_write_subvector_384(&(ec_384->curve_order_n), 7, 3, domain_parameters->Order_p,
                                        domain_parameters->OrderLen)) {
        MESSAGE_ERROR_DEBUG(
            "crypto_create_ec_384_parameters_asset: crypto_write_subvector_384(order_n) failed!\n");
        return -1;
    }
    ...
    if (0 != crypto_write_subvector_384(&(ec_384->curve_base_point_x), 7, 4,
                                        ...
                                        domain_parameters->ECPointXLen)) {
        MESSAGE_ERROR_DEBUG(
            "crypto_create_ec_384_parameters_asset: crypto_write_subvector_384(point_x) failed!\n");
        return -1;
    }
    ...
    if (0 != crypto_write_subvector_384(&(ec_384->curve_base_point_y), 7, 5,
                                        ...
                                        domain_parameters->ECPointYLen)) {
        MESSAGE_ERROR_DEBUG(
            "crypto_create_ec_384_parameters_asset: crypto_write_subvector_384(point_y) failed!\n");
        return -1;
    }
    ...
    if (0 != crypto_write_subvector_32(&(ec_384->curve_cofactor), 7, 6, CoFactor, sizeof(CoFactor),
                                       bits)) {
        MESSAGE_ERROR_DEBUG(
            "crypto_create_ec_384_parameters_asset: crypto_write_subvector_32(cofactor) failed!\n");
        return -1;
    }
                                                 ...
                                                 const ECC_DOMAIN_PARAMETERS_t *domain_parameters)
{
    ...
    if (0 != crypto_write_subvector_521(&(ec_521->curve_modulus_p), 7, 0, domain_parameters->P_p,
                                        domain_parameters->PLen)) {
        MESSAGE_ERROR_DEBUG(
            "crypto_create_ec_521_parameters_asset: crypto_write_subvector_521(modulus_p) failed!\n");
        return -1;
    }
    ...
    if (0 != crypto_write_subvector_521(&(ec_521->curve_constant_a), 7, 1, domain_parameters->A_p,
                                        domain_parameters->ALen)) {
        MESSAGE_ERROR_DEBUG(
            "crypto_create_ec_521_parameters_asset: crypto_write_subvector_521(const_A) failed!\n");
        return -1;
    }
    ...
    if (0 != crypto_write_subvector_521(&(ec_521->curve_constant_b), 7, 2, domain_parameters->B_p,
                                        domain_parameters->BLen)) {
        MESSAGE_ERROR_DEBUG(
            "crypto_create_ec_521_parameters_asset: crypto_write_subvector_521(const_B) failed!\n");
        return -1;
    }
    ...
    if (0 != crypto_write_subvector_521(&(ec_521->curve_order_n), 7, 3, domain_parameters->Order_p,
                                        domain_parameters->OrderLen)) {
        MESSAGE_ERROR_DEBUG(
            "crypto_create_ec_521_parameters_asset: crypto_write_subvector_521(order_n) failed!\n");
        return -1;
    }
    ...
    if (0 != crypto_write_subvector_521(&(ec_521->curve_base_point_x), 7, 4,
                                        ...
                                        domain_parameters->ECPointXLen)) {
        MESSAGE_ERROR_DEBUG(
            "crypto_create_ec_521_parameters_asset: crypto_write_subvector_521(point_x) failed!\n");
        return -1;
    }
    ...
    if (0 != crypto_write_subvector_521(&(ec_521->curve_base_point_y), 7, 5,
                                        ...
                                        domain_parameters->ECPointYLen)) {
        MESSAGE_ERROR_DEBUG(
            "crypto_create_ec_521_parameters_asset: crypto_write_subvector_521(point_y) failed!\n");
        return -1;
    }
    ...
    if (0 != crypto_write_subvector_32(&(ec_521->curve_cofactor), 7, 6, CoFactor, sizeof(CoFactor),
                                       bits)) {
        MESSAGE_ERROR_DEBUG(
            "crypto_create_ec_521_parameters_asset: crypto_write_subvector_32(cofactor) failed!\n");
        return -1;
    }
                                             ...
                                             uint32_t *ec_parameters_asset_id)
{
    ...
    case EC_KEY_CURVE_NIST_P256:
        if (0 != crypto_create_ec_256_parameters_asset(&(domain_parameters_data.ec_256),
                                                       &ECurve_NIST_P256)) {
            MESSAGE_ERROR_DEBUG(
                "crypto_create_ec_parameters_asset: crypto_create_ec_256_parameters_asset() failed!\n");
            return -1;
        }
    ...
    case EC_KEY_CURVE_NIST_P384:
        if (0 != crypto_create_ec_384_parameters_asset(&(domain_parameters_data.ec_384),
                                                       &ECurve_NIST_P384)) {
            MESSAGE_ERROR_DEBUG(
                "crypto_create_ec_parameters_asset: crypto_create_ec_384_parameters_asset() failed!\n");
            return -1;
        }
    ...
    case EC_KEY_CURVE_NIST_P521:
        if (0 != crypto_create_ec_521_parameters_asset(&(domain_parameters_data.ec_521),
                                                       &ECurve_NIST_P521)) {
            MESSAGE_ERROR_DEBUG(
                "crypto_create_ec_parameters_asset: crypto_create_ec_521_parameters_asset() failed!\n");
            return -1;
        }
    ...
    if (0 != vaultip_asset_create(get_rom_identity(), public_key_parameters_asset_policy.lo,
    ...
    if (0 != vaultip_asset_load_plaintext(get_rom_identity(), asset_id, domain_parameters_data_ptr,
                                          domain_parameters_size)) {
        MESSAGE_ERROR_DEBUG(
            "crypto_create_ec_parameters_asset: vaultip_asset_load_plaintext() failed!\n");
        if (0 != vaultip_asset_delete(get_rom_identity(), asset_id)) {
            MESSAGE_ERROR_DEBUG(
                "crypto_create_ec_parameters_asset: vaultip_asset_delete() failed!\n");
        }
        return -1;
    }
                                                        ...
                                                        const PUBLIC_KEY_EC_t *public_key)
{
    ...
    if (0 != crypto_write_subvector_256(&(ec_25519->point_x), 1, 0, public_key->pX,
                                        public_key->pXsize)) {
        MESSAGE_ERROR_DEBUG(
            "crypto_create_ec_curve25519_public_key_asset: crypto_write_subvector_256(pX) failed!\n");
        return -1;
    }
                                               ...
                                               const PUBLIC_KEY_EC_t *public_key)
{
    ...
    if (0 != crypto_write_subvector_256(&(ec_25519->point_x), 1, 0, public_key->pX,
                                        public_key->pXsize)) {
        MESSAGE_ERROR_DEBUG(
            "crypto_create_ec_edwards25519_public_key_asset: crypto_write_subvector_256(pX) failed!\n");
        return -1;
    }
                                             ...
                                             uint32_t *ec_public_key_asset_id)
{
    ...
    case EC_KEY_CURVE_NIST_P256:
        if (HASH_ALG_SHA2_256 != hash_alg) {
            MESSAGE_ERROR_DEBUG(
                "crypto_create_ec_public_key_asset: curve p256 can only be used with SHA256 hash!\n");
            return -1;
        }
        if (0 != crypto_create_ec_p256_public_key_asset(&(public_key_data.p256), ec_public_key)) {
            MESSAGE_ERROR_DEBUG(
                "crypto_create_ec_public_key_asset: crypto_create_ec_p256_public_key_asset() failed!\n");
            return -1;
        }
    ...
    case EC_KEY_CURVE_NIST_P384:
        if (HASH_ALG_SHA2_256 != hash_alg && HASH_ALG_SHA2_384 != hash_alg) {
            MESSAGE_ERROR_DEBUG(
                "crypto_create_ec_public_key_asset: curve p384 can only be used with SHA256 or SHA384 hash!\n");
            return -1;
        }
        if (0 != crypto_create_ec_p384_public_key_asset(&(public_key_data.p384), ec_public_key)) {
            MESSAGE_ERROR_DEBUG(
                "crypto_create_ec_public_key_asset: crypto_create_ec_p384_public_key_asset() failed!\n");
            return -1;
        }
    ...
    case EC_KEY_CURVE_NIST_P521:
        if (0 != crypto_create_ec_p521_public_key_asset(&(public_key_data.p521), ec_public_key)) {
            MESSAGE_ERROR_DEBUG(
                "crypto_create_ec_public_key_asset: crypto_create_ec_p521_public_key_asset() failed!\n");
            return -1;
        }
    ...
    case EC_KEY_CURVE_CURVE25519:
        ...
        if (HASH_ALG_SHA2_256 != hash_alg) {
            MESSAGE_ERROR_DEBUG(
                "crypto_create_ec_public_key_asset: curve 25519 can only be used with SHA256 hash!\n");
            return -1;
        }
        if (0 != crypto_create_ec_curve25519_public_key_asset(&(public_key_data.curve25519),
                                                              ec_public_key)) {
            MESSAGE_ERROR_DEBUG(
                "crypto_create_ec_public_key_asset: crypto_create_ec_curve25519_public_key_asset() failed!\n");
            return -1;
        }
    ...
    case EC_KEY_CURVE_EDWARDS25519:
        if (HASH_ALG_SHA2_512 != hash_alg) {
            MESSAGE_ERROR_DEBUG(
                "crypto_create_ec_public_key_asset: curve ed25519 can only be used with SHA512 hash!\n");
            return -1;
        }
        if (0 != crypto_create_ec_edwards25519_public_key_asset(&(public_key_data.edwards25519),
                                                                ec_public_key)) {
            MESSAGE_ERROR_DEBUG(
                "crypto_create_ec_public_key_asset: crypto_create_ec_edwards25519_public_key_asset() failed!\n");
            return -1;
        }
    ...
    if (0 != vaultip_asset_create(get_rom_identity(), public_key_asset_policy.lo,
                                  public_key_asset_policy.hi, public_key_asset_other_settings, 0,
                                  &asset_id)) {
    ...
    if (0 != vaultip_asset_load_plaintext(get_rom_identity(), asset_id, public_key_data_ptr,
                                          public_key_data_size)) {
        MESSAGE_ERROR_DEBUG(
            "crypto_create_ec_public_key_asset: vaultip_asset_load_plaintext() failed!\n");
        if (0 != vaultip_asset_delete(get_rom_identity(), asset_id)) {
            MESSAGE_ERROR_DEBUG(
                "crypto_create_ec_public_key_asset: vaultip_asset_delete() failed!\n");
        }
        return -1;
    }
                               ...
                               const void *data, size_t data_size, uint32_t total_data_size)
{
    ...
    if (0 != vaultip_public_key_ecdsa_verify(
...
DONE:
    ...
    if (0 != public_key_parameters_asset_id) {
        if (0 != vaultip_asset_delete(get_rom_identity(), public_key_parameters_asset_id)) {
            MESSAGE_ERROR_DEBUG(
                "crypto_ecdsa_verify: vaultip_asset_delete(public_key_parameters_asset_id) failed!\n");
        }
    }
    if (0 != public_key_asset_id) {
        if (0 != vaultip_asset_delete(get_rom_identity(), public_key_asset_id)) {
            MESSAGE_ERROR_DEBUG(
                "crypto_ecdsa_verify: vaultip_asset_delete(public_key_asset_id) failed!\n");
        }
    }
    return rv;
}
                                                   ...
                                                   const PUBLIC_KEY_RSA_t *public_key)
{
    ...
    if (0 != crypto_write_subvector_3072(&(rsa_3072->modulus), 2, 0, public_key->pubMod,
                                         public_key->pubModSize)) {
        MESSAGE_ERROR_DEBUG(
            "crypto_create_rsa_3072_public_key_asset: crypto_write_subvector_3072(modulus) failed!\n");
        return -1;
    }
    ...
    if (0 != crypto_write_subvector_64(&(rsa_3072->exponent), 2, 1, public_key->pubExp,
                                       public_key->pubExpSize)) {
        MESSAGE_ERROR_DEBUG(
            "crypto_create_rsa_3072_public_key_asset: crypto_write_subvector_64(exponent) failed!\n");
        return -1;
    }
                                                   ...
                                                   const PUBLIC_KEY_RSA_t *public_key)
{
    ...
    if (0 != crypto_write_subvector_4096(&(rsa_4096->modulus), 2, 0, public_key->pubMod,
                                         public_key->pubModSize)) {
        MESSAGE_ERROR_DEBUG(
            "crypto_create_rsa_4096_public_key_asset: crypto_write_subvector_4096(modulus) failed!\n");
        return -1;
    }
    ...
    if (0 != crypto_write_subvector_64(&(rsa_4096->exponent), 2, 1, public_key->pubExp,
                                       public_key->pubExpSize)) {
        MESSAGE_ERROR_DEBUG(
            "crypto_create_rsa_4096_public_key_asset: crypto_write_subvector_64(exponent) failed!\n");
        return -1;
    }
                                              ...
                                              uint32_t *rsa_public_key_asset_id)
{
    ...
    case 2048:
        if (0 !=
            ...
            MESSAGE_ERROR_DEBUG(
                "crypto_create_rsa_public_key_asset: crypto_create_rsa_2048_public_key_asset() failed!\n");
            return -1;
        }
    ...
    case 3072:
        if (0 !=
            ...
            MESSAGE_ERROR_DEBUG(
                "crypto_create_rsa_public_key_asset: crypto_create_rsa_3072_public_key_asset() failed!\n");
            return -1;
        }
    ...
    case 4096:
        if (0 !=
            ...
            MESSAGE_ERROR_DEBUG(
                "crypto_create_rsa_public_key_asset: crypto_create_rsa_4096_public_key_asset() failed!\n");
            return -1;
        }
    ...
    if (0 != vaultip_asset_load_plaintext(get_rom_identity(), asset_id, public_key_data_ptr,
                                          public_key_data_size)) {
        MESSAGE_ERROR_DEBUG(
            "crypto_create_rsa_public_key_asset: vaultip_asset_load_plaintext() failed!\n");
        if (0 != vaultip_asset_delete(get_rom_identity(), asset_id)) {
            MESSAGE_ERROR_DEBUG(
                "crypto_create_rsa_public_key_asset: vaultip_asset_delete() failed!\n");
        }
        return -1;
    }
                             ...
                             const void *data, size_t data_size, uint32_t total_data_size)
{
    ...
    // verify the signature
    if (0 != vaultip_public_key_rsa_pss_verify(
                               ...
                               const void *data, size_t data_size)
{
    ...
    if (data_size > 4096) {
        ...
        if (0 != vaultip_asset_create(get_rom_identity(), temp_digest_asset_policy.lo,
                                      temp_digest_asset_policy.hi, temp_digest_asset_other_settings,
                                      0, &temp_digest_asset_id)) {
            MESSAGE_ERROR_DEBUG("crypto_verify_pk_signature: vaultip_asset_create() failed!\n");
            return -1;
        }

        // pre-hash the data
        if (0 != vaultip_hash_update(get_rom_identity(), signature->hashAlg, temp_digest_asset_id,
                                     data, prehash_length, true)) {
    ...
    MESSAGE_INFO_DEBUG(
        "crypto_verify_pk_signature: message_length=0x%x, total_message_length=0x%x\n", data_size,
    ...
    case PUBLIC_KEY_TYPE_EC:
        ...
        break;
    case PUBLIC_KEY_TYPE_RSA:
        rv = crypto_rsa_verify(&(public_key->rsa), signature, temp_digest_asset_id, data, data_size,
                               total_message_length);
        break;
    default:
        MESSAGE_ERROR_DEBUG("crypto_verify_pk_signature: not supported key type!\n");
                                         ...
                                         size_t buffer_size, uint32_t *hash_size)
{
    ...
    if (0 !=
        vaultip_static_asset_search(get_rom_identity(), static_asset_id, &asset_id, &asset_size)) {
        MESSAGE_ERROR_DEBUG(
            "load_public_key_hash_from_otp: vaultip_static_asset_search(%u) failed!\n",
            static_asset_id);
        return -1;
    }
    ...
    *hash_size = asset_size;
    ...
    if (0 != vaultip_public_data_read(get_rom_identity(), asset_id, buffer, (uint32_t)buffer_size,
                                      hash_size)) {
                                           ...
                                           size_t buffer_size, uint32_t *counter_size)
{
    ...
    if (0 !=
        vaultip_static_asset_search(get_rom_identity(), static_asset_id, &asset_id, &asset_size)) {
        MESSAGE_ERROR_DEBUG(
            "load_monotonic_counter_from_otp: vaultip_static_asset_search(%u) failed!\n",
            static_asset_id);
        return -1;
    }
    ...
    *counter_size = asset_size;
    ...
    if (0 != vaultip_monotonic_counter_read(get_rom_identity(), asset_id, buffer,
                                            (uint32_t)buffer_size, counter_size)) {
        MESSAGE_ERROR_DEBUG(
            "load_monotonic_counter_from_otp: vaultip_monotonic_counter_read() failed!\n");
        return -1;
    }
    return 0;
}
