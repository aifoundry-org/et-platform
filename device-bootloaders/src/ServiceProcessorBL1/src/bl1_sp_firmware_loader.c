...
static int verify_bl2_image_file_header(ESPERANTO_IMAGE_FILE_HEADER_t * sp_bl2_file_header, uint32_t sp_bl2_size) {
    ...
    if (0 != (sp_bl2_file_header->info.file_header_flags & ESPERANTO_IMAGE_FILE_HEADER_FLAGS_ENCRYPTED)) {
        ...
        if (0 != crypto_derive_kdk_key(gs_sp_bl2_kdk_derivation_data, sizeof(gs_sp_bl2_kdk_derivation_data), &gs_sp_bl2_kdk)) {
        ...
        if (0 != crypto_derive_mac_key(sp_bl2_file_header->info.mac_type, gs_sp_bl2_kdk, gs_sp_bl2_mac_derivation_data, sizeof(gs_sp_bl2_mac_derivation_data), &gs_sp_bl2_mack)) {
        ...
        if (0 != crypto_mac_verify(sp_bl2_file_header->info.mac_type, gs_sp_bl2_mack, &(sp_bl2_file_header->info), sizeof(sp_bl2_file_header->info), sp_bl2_file_header->MAC)) {
        ...
        if (0 != crypto_derive_enc_key(gs_sp_bl2_kdk, gs_sp_bl2_enc_derivation_data, sizeof(gs_sp_bl2_enc_derivation_data), &gs_sp_bl2_enck)) {
        ...
        if (0 != crypto_aes_decrypt_update(&gs_sp_bl2_aes_context, &(sp_bl2_file_header->info.image_info_and_signaure.info.secret_info), sizeof(sp_bl2_file_header->info.image_info_and_signaure.info.secret_info))) {
...
static int load_bl2_code_and_data(const ESPERANTO_IMAGE_FILE_HEADER_t * sp_bl2_file_header) {
    ...
    const ESPERANTO_IMAGE_INFO_t * image_info = &(sp_bl2_file_header->info.image_info_and_signaure.info);
    ...
    if (0 != (sp_bl2_file_header->info.file_header_flags & ESPERANTO_IMAGE_FILE_HEADER_FLAGS_ENCRYPTED)) {
        ...
        if (0 != constant_time_memory_compare(hash, sp_bl2_file_header->info.encrypted_code_and_data_hash, code_and_data_hash_size)) {
...
DONE:
    ...
    if (gs_sp_bl2_enck_created) {
        if (0 != crypto_delete_key(gs_sp_bl2_enck)) {
            printx("load_bl2_firmware: crypto_delete_key(enck) failed!\n");
            rv = -1;
        }
    }
    if (gs_sp_bl2_mack_created) {
        if (0 != crypto_delete_key(gs_sp_bl2_mack)) {
            printx("load_bl2_firmware: crypto_delete_key(mack) failed!\n");
            rv = -1;
        }
    }
    if (gs_sp_bl2_kdk_created) {
        if (0 != crypto_delete_key(gs_sp_bl2_kdk)) {
