...
static int spi_controller_tx32_data(volatile Ssi_t * spi_regs, const uint8_t * spi_command, uint32_t spi_command_length, uint8_t * tx_data, uint32_t tx_data_size) {
    ...
    //         printf(" %02x", tx_data[n]);
    //     }
    //     printf("\n");
    // }

    if (4 != spi_command_length) {
        printf("spi_controller_tx32_data: command_length (%u) is not 4!\n", spi_command_length);
        return -1;
    }

    if (0 != (tx_data_size & 0x3)) {
        printf("spi_controller_tx32_data: tx_data_size (%u) is not a multiple of 4!\n", tx_data_size);
        return -1;
    }

    if (0 != (((const size_t)spi_command) & 0x3)) {
        printf("spi_controller_tx32_data: command is not 32-bit aligned!\n");
        return -1;
    }

    if (0 != (((size_t)tx_data) & 0x3)) {
        printf("spi_controller_tx32_data: tx_data is not 32-bit aligned!\n");
        return -1;
    }
    ...
    while (rx32_count > 0) {
        ...
        if (rxflr.B.RXTFL > 0) {
            ...
            if (timeout > TX_TIMEOUT) {
                rv = -1;
                printf("TX ERR: SR=0x%x, TXTFL=0x%x, RXTFL=0x%x\n", spi_regs->SR.R, spi_regs->TXFLR.R, rxflr.R);
                printf("        CTRLR0=0x%x, SSIENR=0x%x, SER=0x%x\n", spi_regs->CTRLR0.R, spi_regs->SSIENR.R, spi_regs->SER.R);
                printf("        BAUDR=0x%x, TXFTLR=0x%x, RXFTLR=0x%x\n", spi_regs->BAUDR.R, spi_regs->TXFTLR.R, spi_regs->RXFTLR.R);
                //printf("SR! 0x%x TXFLR! 0x%x\n", sr.R, txflr.R);
                goto DONE;
            }
        }
    }
...
static int spi_controller_tx_data(volatile Ssi_t * spi_regs, const uint8_t * spi_command, uint32_t spi_command_length, uint8_t * tx_data, uint32_t tx_data_size) {
    ...
    printf("SPI_TX: cmd=%02x, len=%u, tx_data=0x%x, tx_size=%u\n", spi_command[0], spi_command_length, tx_data, tx_data_size);
    for (n = 1; n < spi_command_length; n++) {
        printf(" %02x", spi_command[n]);
    }
    if (spi_command_length > 1) {
    ...
    if (combined_size > 256) {
        printf("spi_controller_tx_data: TX size (%u + %u = %u)too large!\n", spi_command_length, tx_data_size, combined_size);
        return -1;
    }
    ...
    while (txflr.B.TXTFL > 0) {
        ...
        if (timeout > TX_TIMEOUT) {
            rv = -1;
            printf("TX ERR 2: SR=0x%x, TXTFL=0x%x\n", spi_regs->SR.R, txflr.R);
            goto DONE;
        }
        txflr.R = spi_regs->TXFLR.R;
    }
...
static int spi_controller_rx32_data(volatile Ssi_t * spi_regs, const uint8_t * spi_command, uint32_t spi_command_length, uint32_t read_frames, uint32_t skip_read_size, uint8_t * rx_data, uint32_t rx_data_size) {
    ...
    if (4 != spi_command_length) {
        printf("spi_controller_rx32_data: command_length is not 4!\n");
        return -1;
    }

    if (0 != (rx_data_size & 0x3)) {
        printf("spi_controller_rx32_data: rx_data_size is not a multiple of 32-bit!\n");
        return -1;
    }

    if (0 != (((const size_t)spi_command) & 0x3)) {
        printf("spi_controller_rx32_data: command is not 32-bit aligned!\n");
        return -1;
    }

    if (0 != (((const size_t)rx_data) & 0x3)) {
        printf("spi_controller_rx32_data: rx_data is not 32-bit aligned!\n");
        return -1;
    }
    ...
    while (read_frames > 0) {
        ...
        if (rxflr.B.RXTFL > 0) {
            ...
            if (timeout > RX_TIMEOUT) {
                rv = -1;
                printf("RX ERR: SR=0x%x, TXTFL=0x%x, RXTFL=0x%x\n", spi_regs->SR.R, spi_regs->TXFLR.R, rxflr.R);
                printf("        CTRLR0=0x%x, SSIENR=0x%x, SER=0x%x\n", spi_regs->CTRLR0.R, spi_regs->SSIENR.R, spi_regs->SER.R);
                printf("        BAUDR=0x%x, TXFTLR=0x%x, RXFTLR=0x%x\n", spi_regs->BAUDR.R, spi_regs->TXFTLR.R, spi_regs->RXFTLR.R);
                //printf("SR! 0x%x\n", sr.R);
                goto DONE;
...
int spi_controller_command(SPI_CONTROLLER_ID_t id, uint8_t slave_index, SPI_COMMAND_t * command) {
    ...
    if (0 == command->data_receive) {
        ...
        if (0 != command->dummy_bytes) {
            printf("spi_controller_command: support for tx with dummy bytes not implemented!\n");
            return -1;
        }
        if (0 == (true_write_size & 0x3) && 4 == spi_command_length) {
            ...
            printf("spi_controller_command: tx command_length (%u) and data_size (%u) not supported!\n", spi_command_length, command->data_size);
            return -1;
        }
#endif
    } else {
        ...
        if (4 == spi_command_length) {
            ...
            printf("spi_controller_command: rx command_length (%u) not supported!\n", spi_command_length);
            return -1;
        }
#else
    ...
    if (command->data_receive) {
        ...
        baud_rate.R = (Ssi_BAUDR_t){ .B = { .SCKDV = bl2_data->spi_controller_tx_baudrate_divider }}.R;
        spi_regs->BAUDR.R = baud_rate.R;
        spi_regs->CTRLR0.R = (Ssi_CTRLR0_t){ .B = { 
                                                // .DFS = 0,
                                                .FRF = SSI_CTRLR0_FRF_FRF_MOTOROLA_SPI,
