                             ...
                             const ESPERANTO_IMAGE_FILE_HEADER_t *image_file_header)
{
    ...
    {
        if (0 != verify_esperanto_image_certificate(image_type,
                                                    &(image_file_header->info.signing_certificate)))
        {
            Log_Write(LOG_LEVEL_ERROR,
                      "verify_image_file_header: image certificate is not valid!\n");
            return -1;
        }
        else
        {
...
static int verify_image_file_header_flags_derive_keys(
    ESPERANTO_IMAGE_FILE_HEADER_t *image_file_header, const uint8_t *kdk_derivation_data,
    size_t kdk_derivation_data_size, const uint8_t *mack_derivation_data,
    size_t mack_derivation_data_size, const uint8_t *enck_derivation_data,
    size_t enck_derivation_data_size)
{
        ...
        (image_file_header->info.file_header_flags & ESPERANTO_IMAGE_FILE_HEADER_FLAGS_ENCRYPTED))
    {
        ...
        {
            Log_Write(LOG_LEVEL_ERROR,
                      "verify_image_file_header: crypto_derive_kdk_key() failed!\n");
            return -1;
        }
        gs_kdk_created = true;
        ...
        {
            Log_Write(LOG_LEVEL_ERROR,
                      "verify_image_file_header: crypto_derive_mack_key() failed!\n");
            return -1;
        }
        ...
        {
            Log_Write(LOG_LEVEL_ERROR,
                      "verify_image_file_header: crypto_derive_enck_key() failed!\n");
            return -1;
        }
        ...
        {
            Log_Write(LOG_LEVEL_ERROR,
                      "verify_image_file_header: crypto_aes_decrypt_init() failed!\n");
            return -1;
        }
        ...
        {
            Log_Write(LOG_LEVEL_ERROR,
                      "verify_image_file_header: crypto_aes_decrypt_update() failed!\n");
            return -1;
        }
    }
                                    ...
                                    uint32_t image_file_size)
{
    ...
    {
        Log_Write(LOG_LEVEL_ERROR,
                  "verify_image_file_header: Unexpected image type! (%08x, expected %08x)!\n",
    ...
    if (0 != verify_image_file_header_flags_derive_keys(
                 image_file_header, kdk_derivation_data, kdk_derivation_data_size,
                 mack_derivation_data, mack_derivation_data_size, enck_derivation_data,
                 enck_derivation_data_size))
    {
                                    ...
                                    const ESPERANTO_IMAGE_FILE_HEADER_t *image_file_header)
{
    ...
    for (region_no = 0; region_no < image_info->secret_info.load_regions_count; region_no++)
    {
        ...
        {
            Log_Write(LOG_LEVEL_ERROR,
                      "Invalid region %u: load=0x%x, addr=0x%lx, fsize=0x%x, msize=0x%x\n",
