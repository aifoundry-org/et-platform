                                                       ...
                                                       active_power_management_e state)
{
    ...
    if (STATUS_SUCCESS !=
        SP_Host_Iface_CQ_Push_Cmd((char *)&dm_rsp, sizeof(struct device_mgmt_default_rsp_t)))
    {
        Log_Write(LOG_LEVEL_ERROR,
...
static void pwr_svc_set_module_voltage(uint16_t tag, uint64_t req_start_time, void *buffer)
{
    ...
    {
        case MODULE_MINION:
            vminLUT.mnn_voltage = &set_voltage_cmd->value;
            ...
            break;
        case MODULE_L2CACHE:
            vminLUT.sram_voltage = &set_voltage_cmd->value;
            status = flash_fs_set_vmin_lut_boot_voltages(&vminLUT);
            if (STATUS_SUCCESS != status)
            {
                Log_Write(LOG_LEVEL_ERROR,
            ...
            break;
        case MODULE_NOC:
            vminLUT.noc_voltage = &set_voltage_cmd->value;
            status = flash_fs_set_vmin_lut_boot_voltages(&vminLUT);
            if (STATUS_SUCCESS != status)
            {
                Log_Write(LOG_LEVEL_ERROR,
        ...
        case MODULE_PCIE_LOGIC:
            vminLUT.pcl_voltage = &set_voltage_cmd->value;
            status = flash_fs_set_vmin_lut_boot_voltages(&vminLUT);
            if (STATUS_SUCCESS != status)
            {
                Log_Write(LOG_LEVEL_ERROR,
            ...
            break;
        case MODULE_DDR:
            vminLUT.ddr_voltage = &set_voltage_cmd->value;
            status = flash_fs_set_vmin_lut_boot_voltages(&vminLUT);
            if (STATUS_SUCCESS != status)
            {
                Log_Write(LOG_LEVEL_ERROR,
                                                         ...
                                                         power_throttle_state_e throttle_state)
{
    ...
    if (STATUS_SUCCESS != SP_Host_Iface_CQ_Push_Cmd(
                              (char *)&dm_rsp, sizeof(struct device_mgmt_throttle_residency_rsp_t)))
    {
        Log_Write(LOG_LEVEL_ERROR,
                                                      ...
                                                      power_state_e power_state)
{
    ...
    if (STATUS_SUCCESS != SP_Host_Iface_CQ_Push_Cmd(
                              (char *)&dm_rsp, sizeof(struct device_mgmt_power_residency_rsp_t)))
    {
        Log_Write(LOG_LEVEL_ERROR,
