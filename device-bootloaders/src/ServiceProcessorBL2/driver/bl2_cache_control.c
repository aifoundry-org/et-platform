...
int32_t icache_err_interrupt_enable(uint64_t shire_mask, uint32_t interrupt_mask) {
   ...
   */
  FOR_EACH_MINSHIRE(
     for (uint8_t neigh = 0; neigh < 4; neigh++) {
         icache_esr = read_esr_new(
             PP_MACHINE, minshire, REGION_NEIGHBOURHOOD, neigh,
             ETSOC_NEIGH_ESR_ICACHE_ERR_LOG_CTL_ADDRESS, 0);
         icache_esr =
             ETSOC_NEIGH_ESR_ICACHE_ERR_LOG_CTL_ERR_INTERRUPT_ENABLE_MODIFY(
                 icache_esr, interrupt_mask);
         write_esr_new(PP_MACHINE, minshire, REGION_NEIGHBOURHOOD,
                       neigh, ETSOC_NEIGH_ESR_ICACHE_ERR_LOG_CTL_ADDRESS, icache_esr, 0);
     }
  )
...
int32_t icache_enable_all_interrupts(uint64_t shire_mask) {
    ...
    interrupt_mask = BIT(SINGLE_BIT_ECC) |
                     BIT(DOUBLE_BIT_ECC) |
                     BIT(ECC_ERROR_COUNTER_SATURATION);
    icache_err_interrupt_enable(shire_mask, interrupt_mask);
  ...
  FOR_EACH_MINSHIRE(
     for (uint8_t neigh = 0; neigh < 4; neigh++) {
         icache_esr = read_esr_new(
             ...
             ETSOC_NEIGH_ESR_ICACHE_ERR_LOG_CTL_ADDRESS, 0);
         interrupt_mask =
             ~interrupt_mask & ETSOC_NEIGH_ESR_ICACHE_ERR_LOG_CTL_ERR_INTERRUPT_ENABLE_GET(icache_esr);
         icache_esr =
             ETSOC_NEIGH_ESR_ICACHE_ERR_LOG_CTL_ERR_INTERRUPT_ENABLE_MODIFY(
                 icache_esr, interrupt_mask);
         write_esr_new(PP_MACHINE, minshire, REGION_NEIGHBOURHOOD,
...
static void icache_error_handler(uint8_t minshire) {
  ...
  error_code =
      ETSOC_NEIGH_ESR_ICACHE_ERR_LOG_INFO_SBE_DBE_CODE_GET(error_log_info);
...
int32_t sram_enable_all_interrupts(uint64_t shire_mask) {
    ...
    interrupt_mask = BIT(SINGLE_BIT_ECC) |
                     BIT(DOUBLE_BIT_ECC) |
                     BIT(ECC_ERROR_COUNTER_SATURATION) |
                     BIT(DECODE_AND_SLAVE_ERRORS) |
                     BIT(PERFORMANCE_COUNTER_SATURATION);
    sc_err_interrupt_enable(shire_mask, interrupt_mask);
  ...
  FOR_EACH_MINSHIRE(
      for (uint8_t bank = 0; bank < 4; bank++) {
          sc_esr = read_esr_new(
              ...
              ETSOC_SHIRE_CACHE_ESR_SC_ERR_LOG_CTL_ADDRESS, bank);
          interrupt_mask =
              ~interrupt_mask & ETSOC_SHIRE_CACHE_ESR_SC_ERR_LOG_CTL_ESR_SC_ERR_INTERRUPT_ENABLE_GET(sc_esr);
          sc_esr =
              ETSOC_SHIRE_CACHE_ESR_SC_ERR_LOG_CTL_ESR_SC_ERR_INTERRUPT_ENABLE_MODIFY(
                  sc_esr, interrupt_mask);
          write_esr_new(PP_MACHINE, minshire, REGION_OTHER,
