...
static int i2c_set_connection_params(ET_I2C_DEV_t* dev, ET_I2C_SPEED_t speed)
{
    ...
    const uint32_t et_ic_con_mask = I2C_IC_CON_IC_SLAVE_DISABLE_SET(
     ...
     switch (speed)
     {
        case ET_I2C_SPEED_100k:
            dev->regs->IC_CON = et_ic_con_mask | I2C_IC_CON_SPEED_SET(
                                                    I2C_IC_CON_SPEED_SPEED_STANDARD);
            return ET_I2C_OK;
        case ET_I2C_SPEED_400k:
            dev->regs->IC_CON = et_ic_con_mask | I2C_IC_CON_SPEED_SET(
                                                    I2C_IC_CON_SPEED_SPEED_FAST);
            return ET_I2C_OK;
...
int i2c_init(ET_I2C_DEV_t* dev, ET_I2C_SPEED_t speed, uint8_t addr_slave)
{
    ...
    if (speed == ET_I2C_SPEED_100k) dev->regs->IC_SDA_HOLD =
