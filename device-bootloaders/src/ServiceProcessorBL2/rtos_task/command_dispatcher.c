...
TaskHandle_t g_mm_cmd_hdlr_handle;
...
static void create_mm_cmd_hdlr_task(void)
{
    ...
    g_mm_cmd_hdlr_handle = xTaskCreateStatic(mm_cmd_hdlr_task, "MM_CMD_Handle_Task",
                                             VQUEUE_STACK_SIZE, NULL, VQUEUE_TASK_PRIORITY,
                                             g_mm_cmd_hdlr_stack, &g_mm_cmd_hdlr_ptr);
                                                   ...
                                                   void *shared_mem_ptr, uint64_t vq_used_space)
{
    ...
    {
        Log_Write(LOG_LEVEL_ERROR, "pc_vq_task:ERROR:Host cmd processing failed:%d\r\n",
                  pop_ret_val);
...
static void mm2sp_get_fw_version_cmd_handler(const void *cmd_buffer)
{
    ...
    else if (cmd->fw_type == MM2SP_MACHINE_MINION_FW)
    {
        ...
        Log_Write(LOG_LEVEL_DEBUG,
                  "mm_cmd_hldr: Machine FW version:major:%d:minor:%d:revision:%d\n", rsp.major,
                  rsp.minor, rsp.revision);
    }
...
void launch_host_sp_command_handler(void)
{
    ...
    /* Launch the MDI Breakpoint notify task */
