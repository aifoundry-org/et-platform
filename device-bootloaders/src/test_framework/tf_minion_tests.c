...
int8_t Minion_Step_Clk_PLL_Program_Cmd_Handler(void *test_cmd)
{
    const struct tf_cmd_minion_step_clock_pll_program_t *cmd =
        (struct tf_cmd_minion_step_clock_pll_program_t *)test_cmd;
    ...
    cmd_rsp_hdr.rsp_hdr.payload_size =
        TF_GET_PAYLOAD_SIZE(struct tf_rsp_minion_step_clock_pll_program_t);
    printf("\n** STEPPLL4 mode = %d **\r\n", cmd->cmd_payload);
    cmd_rsp_hdr.status = (uint32_t)configure_sp_pll_4(cmd->cmd_payload, HPDPLL_LDO_KICK);
    printf("\n** STEPPLL4 done **\r\n");
...
int8_t Minion_Shire_Enable_Cmd_Handler(void *test_cmd)
{
    const struct tf_cmd_minion_shire_enable_t *cmd =
        (struct tf_cmd_minion_shire_enable_t *)test_cmd;
    ...
    cmd_rsp_hdr.status = (uint32_t)Initialize_Minions(cmd->shire_mask);
    // enable the neighborhoods
    cmd_rsp_hdr.status |= (uint32_t)Minion_Configure_Minion_Shire_PLL(
        cmd->shire_mask, cmd->pll4_mode, cmd->pll4_mode, true);
    printf("\n** Sh EN done **\r\n");
