...
int8_t Minion_Shire_Enable_Cmd_Handler(void* test_cmd)
{
    struct tf_shire_cmd_t* cmd = (struct tf_shire_cmd_t*)test_cmd;
    struct tf_cmd_rsp_t cmd_rsp_hdr;
    ...
    cmd_rsp_hdr.status = (uint32_t)Minion_Enable_Compute_Minion(cmd->shire_mask, cmd->pll4_mode);
    // enable the neighborhoods
    cmd_rsp_hdr.status |= (uint32_t)Minion_Enable_Neighborhoods(cmd->shire_mask);
    printf("\n** Sh EN done **\r\n");
    TF_Send_Response(&cmd_rsp_hdr, sizeof(struct tf_cmd_rsp_t));
...
int8_t Minion_ESR_Read_Cmd_Handler(void* test_cmd)
{
    struct tf_esr_cmd_t* cmd = (struct tf_esr_cmd_t*)test_cmd;
...
int8_t Minion_ESR_Write_Cmd_Handler(void* test_cmd)
{
    ...
    rsp_hdr.id = TF_RSP_MINION_ESR_WRITE;
    rsp_hdr.flags = TF_RSP_ONLY;
    rsp_hdr.payload_size =  0;
...
int8_t Minion_ESR_RMW_Cmd_Handler(void* test_cmd)
{
    ...
    rsp_hdr.id = TF_RSP_MINION_ESR_RMW;
    rsp_hdr.flags = TF_RSP_ONLY;
    rsp_hdr.payload_size =  0;
