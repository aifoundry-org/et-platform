...
int8_t Sp_Trace_Get_Buffer_Cmd_Handler(const void *test_cmd)
{
    ...
    rsp.rsp_hdr.flags = TF_RSP_WITH_PAYLOAD;
    rsp.rsp_hdr.payload_size = (uint32_t)(sizeof(uint32_t) + rsp.bytes_read);

    TF_Send_Response_With_Payload(&rsp, (uint32_t)(sizeof(tf_rsp_hdr_t) + sizeof(uint32_t)),
...
int8_t CM_Trace_Get_Info_Cmd_Handler(const void *test_cmd)
{
    ...
    {
        struct trace_buffer_std_header_t trace_header;
        memcpy(&trace_header, (void *)CM_SMODE_TRACE_BUFFER_BASE,
    ...
    else
    {
        ...
        memcpy(&trace_header,
               (void *)(CM_SMODE_TRACE_BUFFER_BASE +
                        (cmd->buffer_id * CM_SMODE_TRACE_BUFFER_SIZE_PER_HART)),
               sizeof(struct trace_buffer_size_header_t));
        rsp.offset = trace_header.data_size;
        rsp.base =
            CM_SMODE_TRACE_BUFFER_BASE + (cmd->buffer_id * CM_SMODE_TRACE_BUFFER_SIZE_PER_HART);
    }
