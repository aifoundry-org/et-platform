...
class EsperantoBootLoadersConan(ConanFile):
    ...
    def requirements(self):
        # header-only libs
        self.requires("deviceApi/[>=0.5.0 <1.0.0]")
        self.requires("esperantoTrace/[>=0.5.0 <1.0.0]")
        self.requires("signedImageFormat/1.0")
        self.requires("tf-protocol/[>=0.1.0 <1.0.0]")
        self.requires("esperanto-flash-tool/[>=1.0.0 <2.0.0]") # we only consume a header
        # libs
        self.requires("etsoc_hal/[>=1.0.0 <2.0.0]")
        self.requires("et-common-libs/[>=0.7.0 <1.0.0]")
