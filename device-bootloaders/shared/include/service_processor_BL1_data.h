#ifndef __SERVICE_PROCESSOR_BL1_DATA_H__
#define __SERVICE_PROCESSOR_BL1_DATA_H__
...
#define SERVICE_PROCESSOR_BL1_DATA_VERSION 0x00000001

typedef struct ESPERANTO_PARTITION_BL1_INFO_s {
...
} ESPERANTO_PARTITION_BL1_INFO_t;

typedef struct FLASH_FS_BL1_INFO_s {
    union {
        SPI_FLASH_ID_t flash_id;
        uint32_t flash_id_u32;
    };
    uint32_t flash_size;
    ESPERANTO_PARTITION_BL1_INFO_t partition_info[2];
...
typedef struct SERVICE_PROCESSOR_BL1_DATA_s {
    uint32_t service_processor_bl1_data_size;
