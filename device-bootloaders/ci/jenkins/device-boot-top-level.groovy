        ...
        stage('PARALLEL0') {
          parallel {
            ...
            stage('DM_TESTS_PCIE_SYSEMU') {
              steps {
                script {
                  if ( ( (env.FORCE_CHILD_RETRIGGER != null) && sh(returnStatus: true, script: "${FORCE_CHILD_RETRIGGER}") == 0) || sh(returnStatus: true, script: './ci/ci-tools/scripts/jenkins_scripts.py job_passed_for_branch --branch "' + "${SW_PLATFORM_BRANCH}" + '" sw-platform/virtual-platform/pipelines/dm-tests-pcie-sysemu-1dev \'{  "COMPONENT_COMMITS":"' + "${COMPONENT_COMMITS},device-software/device-bootloaders:${BRANCH}" + '" }\'') != 0) {
                    build job:
            ...
            stage('DM_TESTS_ZEBU_BEMU') {
              steps {
                script {
                  if ( ( (env.FORCE_CHILD_RETRIGGER != null) && sh(returnStatus: true, script: "${FORCE_CHILD_RETRIGGER}") == 0) || sh(returnStatus: true, script: './ci/ci-tools/scripts/jenkins_scripts.py job_passed_for_branch --branch "' + "${SW_PLATFORM_BRANCH}" + '" sw-platform/system-sw-integration/pipelines/dm-tests-zebu-bemu \'{  "COMPONENT_COMMITS":"' + "${COMPONENT_COMMITS},device-software/device-bootloaders:${BRANCH}" + '" }\'') != 0) {
                    build job:
