...
static std::vector<int> addKernelRandomData(int numElems, int* vecA, int* vecB) {
  ...
  for (int deviceIdxUberKer = 0; deviceIdxUberKer < deviceCountUberKer; deviceIdxUberKer++) {
    ...
    for (uint8_t queueIdxUberKer = 0; queueIdxUberKer < queueCountUberKer; queueIdxUberKer++) {
      ...
      streamUberKer.push_back(
        IDevOpsApiCmd::createCmd<DataReadCmd>(device_ops_api::CMD_FLAGS_BARRIER_ENABLE, devAddrBufLayer0, hostVirtAddr,
      ...
      streamUberKer.push_back(
        IDevOpsApiCmd::createCmd<DataReadCmd>(device_ops_api::CMD_FLAGS_BARRIER_ENABLE, devAddrBufLayer1, hostVirtAddr,
                                              vResultStorageUberKer.back().size() * sizeof(vResultUberKer[0]),
                                              device_ops_api::DEV_OPS_API_DMA_RESPONSE_COMPLETE));
  ...
  for (int deviceIdx = 0; deviceIdx < deviceCount; deviceIdx++) {
    ...
    for (uint8_t queueIdx = 0; queueIdx < queueCount; queueIdx++) {
      ...
      streamEmptyKer.push_back(IDevOpsApiCmd::createCmd<DataReadCmd>(
        device_ops_api::CMD_FLAGS_BARRIER_ENABLE, devAddrKernelResult, hostVirtAddr, vResultStorage.back().size(),
        device_ops_api::DEV_OPS_API_DMA_RESPONSE_COMPLETE));
  ...
  for (int deviceIdxExceptKer = 0; deviceIdxExceptKer < deviceCountExceptKer; deviceIdxExceptKer++) {
    ...
    for (uint8_t queueIdxExceptKer = 0; queueIdxExceptKer < queueCountExceptKer; queueIdxExceptKer++) {
      ...
      streamExceptKer.push_back(IDevOpsApiCmd::createCmd<DataReadCmd>(
        device_ops_api::CMD_FLAGS_BARRIER_ENABLE, kernelExceptionDevAddr, hostVirtAddr, vResultStorage.back().size(),
        device_ops_api::DEV_OPS_API_DMA_RESPONSE_COMPLETE));
    ...
    for (int queueIdx = 0; queueIdx < queueCount; queueIdx++) {
      for (int kernelIdx = 0; kernelIdx < kerCountStorage[key(deviceIdx, queueIdx)]; kernelIdx++) {
        ASSERT_EQ(sumOfVectorAB[i], resultOfVectorAB[i])
          << "Vectors result mismatch for device: " << deviceIdx << ", queue: " << queueIdx << " at Index: " << i;
        i++;
      }
    }
  }
  ...
  for (const auto& perQueueKernels : kernels) {
    ...
    for (auto kerType : perQueueKernels.kernels) {
      ...
      case KernelTypes::EXCEP_KERNEL_TYPE: {
        ...
        loadElfToDevice(deviceIdxDiffKer, readersDiffKer[kerCount], excepKerElfPath, streamDiffKer, excepKerEntryAddr);
        // allocate 1MB space for kernel error/exception buffer
        auto devAddrkernelException = getDmaWriteAddr(deviceIdxDiffKer, 0x100000);
...
std::map<KernelTypes, std::string> TestDevOpsApiKernelCmds::kernelELFNames_ = {
  {KernelTypes::ADD_KERNEL_TYPE, "add_vector.elf"}, {KernelTypes::EXCEP_KERNEL_TYPE, "exception.elf"},
  {KernelTypes::HANG_KERNEL_TYPE, "hang.elf"},      {KernelTypes::UBER_KERNEL_TYPE, "uberkernel.elf"},
  {KernelTypes::EMPTY_KERNEL_TYPE, "empty.elf"},    {KernelTypes::CMUMODE_KERNEL_TYPE, "cm_umode_test.elf"},
};

uint64_t TestDevOpsApiKernelCmds::loadElf(int deviceIdx, KernelTypes kerType, device_ops_api::dma_write_node& node) {
  auto kernelElfName = kernelELFNames_[kerType];
  if (kernelElfStore_.count(kernelElfName) == 0) {
    auto kernelElf = std::make_unique<KernelElf>();
    ELFIO::elfio reader;
    auto elfPath = (fs::path(FLAGS_kernels_dir) / fs::path(kernelElfName)).string();

    if (!reader.load(elfPath)) {
      throw Exception("Unable to load elf file");
    }
    ...
    TEST_VLOG(1) << std::endl
                 << "Loading ELF: " << elfPath << std::endl
                 << "  ELF Entry: 0x" << std::hex << entry << "  Segment [0]: "
                 << "vAddr: 0x" << std::hex << vAddr << ", pAddr: 0x" << std::hex << pAddr << ", Mem Size: 0x"
                 << memSize << ", File Size: 0x" << fileSize;

    kernelElf->segment0.resize(fileSize);
    memcpy(kernelElf->segment0.data(), segment0->get_data(), kernelElf->segment0.size());
    kernelElf->entryOffset = entry - vAddr;
    kernelElfStore_.try_emplace(kernelElfName, std::move(kernelElf));
  }

  // Allocate device buffer for the ELF segment
  const auto& savedElf = kernelElfStore_[kernelElfName];
  uint64_t deviceElfSegment0Buffer = getDmaWriteAddr(deviceIdx, ALIGN(savedElf->segment0.size(), 0x1000));
  auto kernelEntryAddr = deviceElfSegment0Buffer + savedElf->entryOffset;
  ...
  auto mmapMem = static_cast<void*>(getMmapBuffer(deviceIdx, ALIGN(savedElf->segment0.size(), 0x1000)));
  memcpy(mmapMem, savedElf->segment0.data(), savedElf->segment0.size());
  node = {templ::bit_cast<uint64_t>(mmapMem), 0, deviceElfSegment0Buffer,
          static_cast<uint32_t>(savedElf->segment0.size())};

  return kernelEntryAddr;
}

std::vector<KernelTypes> TestDevOpsApiKernelCmds::generateKernelTypes(KernelTypes kernelType, uint64_t totalKernel,
                                                                      int numKernelTypes) const {
...
uint64_t TestDevOpsApiKernelCmds::kernelExceptionSpace(int deviceIdx) {
  return getDmaWriteAddr(deviceIdx, kExceptionSize);
}

void TestDevOpsApiKernelCmds::validataAddKernel(std::vector<AddKerInfo> ExpectedResultAB,
  ...
  for (int i = 0; i < ExpectedResultAB.size(); ++i) {
    ASSERT_EQ(memcmp(ExpectedResultAB[i].data.data(), addKernelResultAB[i], addKerNumElems_ * sizeof(int)), 0)
      ...
      << ", queue: " << ExpectedResultAB[i].queueIdx << " at Index: " << i;
  }

  TEST_VLOG(1) << "====> ADD TWO VECTORS KERNEL RESPONSE DATA VERIFIED <====" << std::endl;
}

void TestDevOpsApiKernelCmds::addKernelResultReadBackPerQueue(int deviceIdx, std::vector<int*>& addKernelResultsVector,
                                                              std::vector<uint64_t> perQueueResultDevAddrs,
                                                              std::vector<CmdTag>& stream) {

  auto bufSize = addKerNumElems_ * sizeof(int);
  auto alignedBufSize = ALIGN(bufSize, kCacheLineSize);
  std::array<device_ops_api::dma_read_node, kNodeCount> readNodes;

  uint8_t nodesFilled = 0;
  for (size_t i = 0; i < perQueueResultDevAddrs.size(); ++i) {
    ...
    readNodes[nodesFilled] = {templ::bit_cast<uint64_t>(readPtr), 0, perQueueResultDevAddrs[i],
                              static_cast<uint32_t>(bufSize)};
    nodesFilled++;

    if (nodesFilled == kNodeCount || i == perQueueResultDevAddrs.size() - 1) {
      // create DMA read list command
      stream.push_back(IDevOpsApiCmd::createCmd<DmaReadListCmd>(
        i < kNodeCount ? // Barrier only for first read to make sure that all kernels execution done
          device_ops_api::CMD_FLAGS_BARRIER_ENABLE
                       : device_ops_api::CMD_FLAGS_BARRIER_DISABLE,
        readNodes.data(), nodesFilled, device_ops_api::DEV_OPS_API_DMA_RESPONSE_COMPLETE));
      nodesFilled = 0;
    }
  }
}

uint64_t TestDevOpsApiKernelCmds::handleAddKernelDMAListCmd(int deviceIdx, uint64_t shireMask,
                                                            std::vector<int>& sumOfAB, std::vector<CmdTag>& stream) {
  ...
  wrNodes[1] = {templ::bit_cast<uint64_t>(vectorA), 0, vectorADevAddr, static_cast<uint32_t>(bufSize)};

  // DMA node for vector B
  wrNodes[2] = {templ::bit_cast<uint64_t>(vectorB), 0, vectorBDevAddr, static_cast<uint32_t>(bufSize)};
  ...
  stream.push_back(IDevOpsApiCmd::createCmd<DmaWriteListCmd>(device_ops_api::CMD_FLAGS_BARRIER_DISABLE, wrNodes.data(),
                                                             wrNodes.size(),
                                                             device_ops_api::DEV_OPS_API_DMA_RESPONSE_COMPLETE));

  // Add kernel launch command
  stream.push_back(IDevOpsApiCmd::createCmd<KernelLaunchCmd>(
    device_ops_api::CMD_FLAGS_BARRIER_ENABLE, kernelEntryAddr, devKernelArgsAddr, kernelExceptionSpace(deviceIdx),
    shireMask, 0, templ::bit_cast<uint64_t*>(&params), sizeof(params), "",
    device_ops_api::DEV_OPS_API_KERNEL_LAUNCH_RESPONSE_KERNEL_COMPLETED));
...
void TestDevOpsApiKernelCmds::printExceptionContext(const std::vector<ExcepContextInfo> exceptionContexts) const {
  for (auto context : exceptionContexts)
    printErrorContext(context.queueIdx, templ::bit_cast<std::byte*>(context.data), exceptionShireMask_, context.tagId);
}

void TestDevOpsApiKernelCmds::handleExceptionOrAbortKernelDMAListCmd(int deviceIdx, int queueIdx, uint64_t shireMask,
                                                                     KernelTypes kertype,
                                                                     std::vector<ExcepContextInfo>& contexts,
                                                                     std::vector<CmdTag>& stream) {
...
void TestDevOpsApiKernelCmds::emptyKernelReadBackResults(int deviceIdx, std::vector<uint64_t> dummyResultAddrs,
                                                         std::vector<CmdTag>& stream) {

  std::array<device_ops_api::dma_read_node, kNodeCount> rdNodes;
  uint8_t nodesFilled = 0;
  for (size_t i = 0; i < dummyResultAddrs.size(); ++i) {
    rdNodes[nodesFilled] = {templ::bit_cast<uint64_t>(getMmapBuffer(deviceIdx, kEmptyKerDummyRead, false)), 0,
                            dummyResultAddrs[i], kEmptyKerDummyRead};
    nodesFilled++;

    if (nodesFilled == kNodeCount || i == dummyResultAddrs.size() - 1) {
      // create DMA read list command
      stream.push_back(IDevOpsApiCmd::createCmd<DmaReadListCmd>(
        i < kNodeCount ? // Barrier only for first read to make sure that all kernels execution done
          device_ops_api::CMD_FLAGS_BARRIER_ENABLE
                       : device_ops_api::CMD_FLAGS_BARRIER_DISABLE,
        rdNodes.data(), nodesFilled, device_ops_api::DEV_OPS_API_DMA_RESPONSE_COMPLETE));
      nodesFilled = 0;
    }
  }
}

uint64_t TestDevOpsApiKernelCmds::handleEmptyKernelDMAListCmd(int deviceIdx, uint64_t shireMask,
                                                              std::vector<CmdTag>& stream) {

  std::array<device_ops_api::dma_write_node, 1> wrNodes;
  ...
  stream.push_back(IDevOpsApiCmd::createCmd<KernelLaunchCmd>(
    ...
    kernelEntryAddr, kernelArgsAddr, 0, shireMask, 0, templ::bit_cast<uint64_t*>(&dummyKernelArgs),
    sizeof(dummyKernelArgs), "", device_ops_api::DEV_OPS_API_KERNEL_LAUNCH_RESPONSE_KERNEL_COMPLETED));
...
void TestDevOpsApiKernelCmds::handleUberKernelDMAListCmd(int deviceIdx, uint64_t shireMask,
                                                         std::vector<uint64_t*>& layersResult,
                                                         std::vector<CmdTag>& stream) {

  std::array<device_ops_api::dma_write_node, 1> wrNodes;
  std::array<device_ops_api::dma_read_node, 2> rdNodes;
  ...
  // Read back data written by layer 0
  layersResult.push_back(
    static_cast<uint64_t*>(static_cast<void*>(getMmapBuffer(deviceIdx, kUberKerLayerBuf0, false))));
  rdNodes[0] = {templ::bit_cast<uint64_t>(layersResult.back()), 0, devAddrLayer0, kUberKerLayerBuf0};

  // Read back data written by layer 1
  layersResult.push_back(
    static_cast<uint64_t*>(static_cast<void*>(getMmapBuffer(deviceIdx, kUberKerLayerBuf1, false))));
  rdNodes[1] = {templ::bit_cast<uint64_t>(layersResult.back()), 0, devAddrLayer1, kUberKerLayerBuf1};

  // create DMA read list command
  stream.push_back(IDevOpsApiCmd::createCmd<DmaReadListCmd>(device_ops_api::CMD_FLAGS_BARRIER_ENABLE, rdNodes.data(),
                                                            rdNodes.size(),
                                                            device_ops_api::DEV_OPS_API_DMA_RESPONSE_COMPLETE));
}
...
kernelContainer_t TestDevOpsApiKernelCmds::buildKernelsInfo(std::vector<KernelTypes> totalKer, bool singleDevice,
                                                            bool singleQueue, bool calculateDmaListSize) {
  int kernelsCount = 0;
  ...
  for (auto devIdx = 0; devIdx < devCount; ++devIdx) {
    ...
    for (auto queueIdx = 0; queueIdx < queueCount; ++queueIdx) {
      if (totalKer.size() > 1) {
        ...
        int perQueueKersCount = (devIdx == (devCount - 1) && queueIdx == (queueCount - 1))
...
void TestDevOpsApiKernelCmds::launchKernelDMAListCmds(uint64_t shireMask, std::vector<KernelTypes> totalKer,
                                                      bool singleDevice, bool singleQueue) {
  // Add kernel parameters
...
void TestDevOpsApiKernelCmds::kernelLaunchCmd_NegativeTesting(
  bool invalidAddr, int size, uint64_t shireMask, device_ops_api::dev_ops_api_kernel_launch_response_e status) {
  std::vector<CmdTag> stream;
  std::vector<std::vector<ELFIO::elfio>> readersStorage;
  std::vector<ELFIO::elfio> readers;
  auto elfPath = (fs::path(FLAGS_kernels_dir) / fs::path("empty.elf")).string();

  std::vector<uint8_t> dummyArgs(size, 0xFF);
  int deviceCount = getDevicesCount();
  for (int deviceIdx = 0; deviceIdx < deviceCount; deviceIdx++) {
    int queueCount = getSqCount(deviceIdx);
    readers.resize(queueCount);
    for (uint8_t queueIdx = 0; queueIdx < queueCount; queueIdx++) {
      // Load ELF
      uint64_t kernelEntryDevAddr;
      loadElfToDevice(deviceIdx, readers[queueIdx], elfPath, stream, kernelEntryDevAddr);
      if (invalidAddr) {
        kernelEntryDevAddr = 0xbeef;
      }
      // allocate space for kernel args
      auto devAddrKernelArgs =
        getDmaWriteAddr(deviceIdx, ALIGN(dummyArgs.size() * sizeof(unsigned long int), kCacheLineSize));
      // allocate 1MB space for kernel error/exception buffer
      auto kernelExceptionDevAddr = getDmaWriteAddr(deviceIdx, 0x100000);
      // kernel launch
      stream.push_back(IDevOpsApiCmd::createCmd<KernelLaunchCmd>(
        device_ops_api::CMD_FLAGS_BARRIER_DISABLE, kernelEntryDevAddr, devAddrKernelArgs, kernelExceptionDevAddr,
        shireMask, 0, templ::bit_cast<uint64_t*>(dummyArgs.data()), static_cast<int>(dummyArgs.size()), "", status));

      // Save stream against deviceIdx and queueIdx
      insertStream(deviceIdx, queueIdx, std::move(stream));
      stream.clear();
    }
    readersStorage.push_back(std::move(readers));
  }
  execute(true);
  deleteStreams();
}

void TestDevOpsApiKernelCmds::kernelLaunchCmd_InvalidAddressNegativeTesting(uint64_t shireMask) {
  kernelLaunchCmd_NegativeTesting(true, 8, shireMask,
                                  device_ops_api::DEV_OPS_API_KERNEL_LAUNCH_RESPONSE_INVALID_ADDRESS);
}

void TestDevOpsApiKernelCmds::kernelLaunchCmd_InvalidArgSizeNegativeTesting(uint64_t shireMask) {
  kernelLaunchCmd_NegativeTesting(false, 128, shireMask,
                                  device_ops_api::DEV_OPS_API_KERNEL_LAUNCH_RESPONSE_INVALID_ARGS_PAYLOAD_SIZE);
}

void TestDevOpsApiKernelCmds::kernelLaunchCmd_InvalidShireMaskNegativeTesting(uint64_t shireMask) {
  kernelLaunchCmd_NegativeTesting(false, 8, shireMask,
                                  device_ops_api::DEV_OPS_API_KERNEL_LAUNCH_RESPONSE_SHIRES_NOT_READY);
}
