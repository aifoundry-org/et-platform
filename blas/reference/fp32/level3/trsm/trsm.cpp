...
int entryPoint_0(KernelArguments* args) {
  ...
  for (int col = 0; col < args->n; ++col) {
    for (int row = 0; row < args->m; ++row) {
      args->b[row + static_cast<size_t>(col) * args->ldb] *= args->alpha;
    }
  }

  if (left) {
    const int rowStart = effectiveUpper ? (args->m - 1) : 0;
    const int rowStop = effectiveUpper ? -1 : args->m;
    const int rowStep = effectiveUpper ? -1 : 1;
    for (int row = rowStart; row != rowStop; row += rowStep) {
      if (!unit) {
        const float diagonal = triangularElement(args->a, args->lda, row, row, upper, transpose, unit);
        for (int col = 0; col < args->n; ++col) {
          args->b[row + static_cast<size_t>(col) * args->ldb] /= diagonal;
        }
      }
      for (int inner = effectiveUpper ? 0 : (row + 1); inner < (effectiveUpper ? row : args->m); ++inner) {
        const float coeff = triangularElement(args->a, args->lda, inner, row, upper, transpose, unit);
        if (coeff == 0.0f) {
          continue;
        }
        for (int col = 0; col < args->n; ++col) {
          args->b[inner + static_cast<size_t>(col) * args->ldb] -=
            coeff * args->b[row + static_cast<size_t>(col) * args->ldb];
        }
      }
      if (effectiveUpper) {
        for (int inner = row + 1; inner < args->m; ++inner) {
          const float coeff = triangularElement(args->a, args->lda, inner, row, upper, transpose, unit);
          if (coeff == 0.0f) {
            continue;
          }
    ...
    const int colStart = effectiveUpper ? 0 : (args->n - 1);
    const int colStop = effectiveUpper ? args->n : -1;
    const int colStep = effectiveUpper ? 1 : -1;
    for (int col = colStart; col != colStop; col += colStep) {
      if (!unit) {
        const float diagonal = triangularElement(args->a, args->lda, col, col, upper, transpose, unit);
        for (int row = 0; row < args->m; ++row) {
          args->b[row + static_cast<size_t>(col) * args->ldb] /= diagonal;
        }
      }
      if (effectiveUpper) {
        for (int inner = col + 1; inner < args->n; ++inner) {
          const float coeff = triangularElement(args->a, args->lda, col, inner, upper, transpose, unit);
          if (coeff == 0.0f) {
            continue;
          }
          for (int row = 0; row < args->m; ++row) {
            args->b[row + static_cast<size_t>(inner) * args->ldb] -=
              args->b[row + static_cast<size_t>(col) * args->ldb] * coeff;
          }
        }
      } else {
