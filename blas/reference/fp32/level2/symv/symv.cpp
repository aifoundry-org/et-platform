...
#include "symv_kernel_arguments.h"
...
float symmetricElement(const float* a, int lda, int row, int col, bool upper) {
  if (upper) {
    return row <= col ? a[row + static_cast<size_t>(col) * lda] : a[col + static_cast<size_t>(row) * lda];
  }
  return row >= col ? a[row + static_cast<size_t>(col) * lda] : a[col + static_cast<size_t>(row) * lda];
}
...
int entryPoint_0(KernelArguments* args) {
  ...
  for (int row = 0; row < args->n; ++row) {
    ...
    for (int col = 0; col < args->n; ++col) {
      sum += symmetricElement(args->a, args->lda, row, col, upper) * args->x[xStart + col * args->incx];
    }
