...
#include "syr2_kernel_arguments.h"
...
int entryPoint_0(KernelArguments* args) {
  ...
  for (int col = 0; col < args->n; ++col) {
    ...
    const float yCol = args->y[yStart + col * args->incy];
    const int rowBegin = upper ? 0 : col;
    const int rowEnd = upper ? (col + 1) : args->n;
    for (int row = rowBegin; row < rowEnd; ++row) {
      const float xRow = args->x[xStart + row * args->incx];
      const float yRow = args->y[yStart + row * args->incy];
      args->a[row + static_cast<size_t>(col) * args->lda] += args->alpha * xRow * yCol + args->alpha * yRow * xCol;
    }
  }
