...
int64_t main(const Parameters *const kernel_params_ptr) {
  ...
  if (use_trace) {
    TRACE_kernel_launch(
        LOG_LEVELS_ERROR, kernel_params_ptr->tensor_a,
        kernel_params_ptr->tensor_b, kernel_params_ptr->tensor_c,
        kernel_params_ptr->tensor_d, kernel_params_ptr->tensor_e,
        kernel_params_ptr->tensor_f, kernel_params_ptr->tensor_g,
        kernel_params_ptr->tensor_h,
  ...
  // Odd harts just run a dummy loop and then wait for even harts to finish and
  // unblock them
  if (hart_id & 1) {
  ...
  for (uint64_t iter = 0; iter < NUM_ITER; iter++) {
    ...
    // Tensor FMA
    tensor_fma(0,
               tfma_configs[tfma_minion_idx + TFMA_BCOLS +
                            tfma_iter_idx],  // tfma_bcols,
               tfma_configs[tfma_minion_idx + TFMA_AROWS +
                            tfma_iter_idx],  // tfma_arows,
               tfma_configs[tfma_minion_idx + TFMA_ACOLS +
                            tfma_iter_idx],  // tfma_acols,
               tfma_configs[tfma_minion_idx + TFMA_ASTART_COL +
                            tfma_iter_idx],  // tfma_astart_col,
               tfma_configs[tfma_minion_idx + TFMA_TENB +
                            tfma_iter_idx],  // tfma_tenb,
               ...
               tfma_configs[tfma_minion_idx + TFMA_TYPE +
                            tfma_iter_idx],  // tfma_type,
               1);                           // tfma_clear_rf);

    tensor_reduce(
        reduce_configs[reduce_minion_idx + REDUCE_START_REG_IDX +
                       reduce_iter_idx],
        reduce_configs[reduce_minion_idx + REDUCE_OP_IDX + reduce_iter_idx],
        reduce_configs[reduce_minion_idx + REDUCE_NUM_REGS_IDX +
                       reduce_iter_idx],
        reduce_configs[reduce_minion_idx + REDUCE_PARTNER_IDX +
                       reduce_iter_idx],
        reduce_configs[reduce_minion_idx + REDUCE_ACTION_IDX +
                       reduce_iter_idx]);
  }
