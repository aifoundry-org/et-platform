...
if (hart_id == 1) //Only Thread1 (or prefetch threads to do prefetch)
   {
	   for(int i = 0; i < N_TIMES ; i++) //N_TIMES = # of times the thread1 performs the prefetch CSR
	   {
		   ...
		   {
			   VA = (uint64_t)(((0x82ULL)<<32) + ((0xaU)<<28U) + (shire_id << 23) + (minion_id << 18) + ((0x01*(uint64_t)i)<<13) + (0x1 << 6));
		   }
		   /****** Prefetches from these shires should get data always form mem-sire 2*******/
		   else if ( (shire_id == 2) || (shire_id == 18) || (shire_id == 26) )
		   {
			   VA = (uint64_t)(((0x82ULL)<<32) + ((0xaU)<<28U) + (shire_id << 23) + (minion_id << 18) + ((0x01*(uint64_t)i)<<13) + (0x2 << 6));
		   }
		   ...
		   {
			   VA = (uint64_t)(((0x82ULL)<<32) + ((0xaU)<<28U) + (shire_id << 23) + (minion_id << 18) + ((0x01*(uint64_t)i)<<13) + (0x7 << 6));
		   }
		   ...
		   prefetch_va(false,     0,   (uint64_t)(VA),  15,         1024,      0, 0 );
		   //WAIT_PREFETCH_0;
	   }
           ...
           for(int i = 0; i < N_TIMES ; i++) //N_TIMES = # of times the thread1 performs the prefetch CSR
	   {
		   ...
		   if(op == 1) flush_va(false,     dst,   (uint64_t)(VA),  15,         stride,      0, 0 );
                   if(op == 2) evict_va(false,     dst,   (uint64_t)(VA),  15,         stride,      0, 0 );
		   //WAIT_CACHEOPS;
	   }
           return 0;
   }
