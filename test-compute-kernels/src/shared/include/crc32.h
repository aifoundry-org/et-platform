...
static const uint32_t crc32tables[8][256] __attribute__((aligned(64))) =
{
    {
...
// static void generate_crc32_tables(void)
// {
//     // Generate 1st crc32 lookup table
...
//     // Generate tables 1-7 by transforming table 0
//     for (uint32_t i = 0; i <= 0xFF; i++)
//     {
...
//         crc32tables[4][i] = (crc32tables[3][i] >> 8) ^ crc32tables[0][crc32tables[3][i] & 0xFF];
//         crc32tables[5][i] = (crc32tables[4][i] >> 8) ^ crc32tables[0][crc32tables[4][i] & 0xFF];
//         crc32tables[6][i] = (crc32tables[5][i] >> 8) ^ crc32tables[0][crc32tables[5][i] & 0xFF];
//         crc32tables[7][i] = (crc32tables[6][i] >> 8) ^ crc32tables[0][crc32tables[6][i] & 0xFF];
//     }
// }

// Slice-by-8 CRC32
static uint32_t crc32_8bytes(const void* data_ptr, uint64_t length, uint32_t previousCrc32);

static uint32_t crc32_8bytes(const void* data_ptr, uint64_t length, uint32_t previousCrc32)
{
    const uint32_t* current = (const uint32_t*)data_ptr;
    ...
    while (length--)
    {
        crc = (crc >> 8) ^ crc32tables[0][(crc & 0xFF) ^ *currentChar++];
    }
