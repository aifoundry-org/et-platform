    ...
    stage('WRAPPER') {
      ...
      stages {
        ...
        stage('CLANG_CHECK') {
          steps {
            script {
              if ( ( (env.FORCE_CHILD_RETRIGGER != null) && sh(returnStatus: true, script: "${FORCE_CHILD_RETRIGGER}") == 0) || sh(returnStatus: true, script: './ci/ci-tools/scripts/jenkins_scripts.py job_passed_for_branch --branch "' + "${SW_PLATFORM_BRANCH}" + '" sw-platform/system-sw-integration/pipelines/linux-driver-clang-tests \'{  "COMPONENT_COMMITS":"' + "${COMPONENT_COMMITS},host-software/linuxDriver/etsoc1-pcie-driver:${BRANCH}" + '" }\'') != 0) {
                build job:
