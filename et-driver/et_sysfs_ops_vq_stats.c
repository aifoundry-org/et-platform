...
msg_count_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < vq_data->vq_common.hp_sq_count; i++) {
		bytes += sysfs_emit_at(
			...
			atomic64_read(&vq_data->hp_sqs[i]
			   ...
			   size_t count)
{
	...
	for (i = 0; i < vq_data->vq_common.hp_sq_count; i++) {
		for (j = 0; j < ARRAY_SIZE(vq_data->hp_sqs[i].stats); j++)
			atomic64_set(&vq_data->hp_sqs[i].stats[j], 0);
	}
