#!/usr/bin/env python3
"""Run the nested-star GP-SDK silicon suite directly, without pytest."""
...
from nested_star_cases import ALL_CASES, POSITIVE_CASES  # noqa: E402


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the nested-star GP-SDK silicon suite.")
    ...
    parser.add_argument("--topology-probe-kernel", type=Path,
                        default=REPO_ROOT / "build/device-prefix/src/device-build/tests/shire_latency_probe.elf_dbg")
...
def build_env(repo_root: Path) -> dict[str, str]:
    ...
    env["ET_SKIP_MEMCPY_DEVICE_CHECK"] = "1"
    return env
...
def build_command(args: argparse.Namespace, case) -> list[str]:
    ...
    return [
        ...
        *case.extra_args,
        ...
        "--erbium_sim",
        f"--topology_probe_kernel={args.topology_probe_kernel}",
    ]
