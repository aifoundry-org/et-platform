/*
 * Test: decoder_u8
 * Fills a trace with n_entries u8 values.
...
int main(int argc, const char **argv)
{
    ...
    srand(uargs.seed);
    {
        ...
        const struct trace_value_u8_t *entry = NULL;
        uint64_t i = 0;
        while (1) {
            entry_header = Trace_Decode(buf, entry_header);
            if (!entry_header)
                break;
            CHECK_EQ(entry_header->type, TRACE_TYPE_VALUE_U8);
            entry = (const struct trace_value_u8_t *)entry_header;
