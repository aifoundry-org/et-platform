...
static ET_I2C_DEV_t i2c_devices[] = {
                                   {.regs =  (I2c*)R_SP_I2C0_BASEADDR, .isInitialized = false},
                                   {.regs =  (I2c*)R_SP_I2C1_BASEADDR, .isInitialized = false},
                                   {.regs =  (I2c*)R_PU_I2C_BASEADDR,  .isInitialized = false}
                                  };


ET_I2C_DEV_t* i2c_get_device(ET_I2C_CONTROLLER_t controllerId) {
     return &i2c_devices[controllerId];
}
...
static void i2c_clock_init(ET_I2C_DEV_t* dev) {
    ...
    val_ss_scl_hcnt =  ((uint16_t) (((ET_SS_MIN_SCL_HIGH * ET_IC_CLK_MHZ) / ET_CLOCK_SPEED_DIVIDER) + 1));
    val_ss_scl_lcnt =  ((uint16_t) (((ET_SS_MIN_SCL_LOW * ET_IC_CLK_MHZ) / ET_CLOCK_SPEED_DIVIDER) + 1));
    val_fs_scl_hcnt =  ((uint16_t) (((ET_FS_MIN_SCL_HIGH * ET_IC_CLK_MHZ) / ET_CLOCK_SPEED_DIVIDER) + 1));
    val_fs_scl_lcnt =  ((uint16_t) (((ET_FS_MIN_SCL_LOW * ET_IC_CLK_MHZ) / ET_CLOCK_SPEED_DIVIDER) + 1));
...
static int i2c_set_connection_params(ET_I2C_DEV_t* dev, ET_I2C_SPEED_t speed) {
    //set speed and
    //set address format and
    //enable restart on the bus and
    //enable master FSM,
    const uint32_t et_ic_con_mask = I2C_IC_CON_IC_SLAVE_DISABLE_SET(I2C_IC_CON_IC_SLAVE_DISABLE_IC_SLAVE_DISABLE_SLAVE_DISABLED) |
                                    I2C_IC_CON_IC_RESTART_EN_SET(I2C_IC_CON_IC_RESTART_EN_IC_RESTART_EN_ENABLED) |
                                    I2C_IC_CON_IC_10BITADDR_MASTER_SET(I2C_IC_CON_IC_10BITADDR_MASTER_IC_10BITADDR_MASTER_ADDR_7BITS) |
     ...
     switch (speed) {
        ...
        case ET_I2C_SPEED_100k:
            dev->regs->IC_CON = et_ic_con_mask | I2C_IC_CON_SPEED_SET(I2C_IC_CON_SPEED_SPEED_STANDARD);
            return ET_I2C_OK;
...
int i2c_init(ET_I2C_DEV_t* dev, ET_I2C_SPEED_t speed, uint8_t addr_slave) {
    ...
    if (speed == ET_I2C_SPEED_100k) dev->regs->IC_SDA_HOLD = 0x50005; // this is a magic number, I do not know where it comes from
...
int i2c_write(ET_I2C_DEV_t* dev, uint8_t regAddr, uint8_t* txDataBuff, uint8_t txDataCount) {
    ...
    while (I2C_IC_STATUS_MST_ACTIVITY_GET(dev->regs->IC_STATUS) == I2C_IC_STATUS_MST_ACTIVITY_MST_ACTIVITY_ACTIVE);
...
int i2c_read(ET_I2C_DEV_t* dev, uint8_t regAddr, uint8_t* rxDataBuff, uint8_t rxDataCount) {
 ...
 //Now read what we have written previously
    ...
    dev->regs->IC_DATA_CMD = (uint32_t)(I2C_IC_DATA_CMD_RESET_VALUE |
                          I2C_IC_DATA_CMD_CMD_SET(I2C_IC_DATA_CMD_CMD_CMD_READ) |
                          I2C_IC_DATA_CMD_STOP_SET(I2C_IC_DATA_CMD_STOP_STOP_ENABLE)); //this is what will init an actual read

    //check RX fifo for received bytes
    while(I2C_IC_RXFLR_RXFLR_GET(dev->regs->IC_RXFLR) < rxDataCount); //wait until we get the bytes we want in the RX fifo.
